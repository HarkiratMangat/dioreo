...
// ==========================================
// Patch Notes operates on a single "current entry" model (the last item in patchNotes[]) rather than
// a search-and-pick flow -- see .claude/rules/manage-panel.md's Patch Notes bullet for the full
// design reasoning. Split out of the former handlers/manage.js on 2026-08-14 (stage 2 of
...
// stays synced to currentSeasonTitle. If none exists yet at all (fresh install, or right after a
// Wipe Season), whichever of these 3 is submitted first creates it.
...
async function setDateInfo(interaction) {
    ...
    // parseReleaseDateTime (not the plain parseAdminDate every other admin date field uses) -- patch
    // notes release times are typed in the admin's own local clock whenever a time is included at
    // all; see the function's own comment in adminParser.js for why this field alone gets that
    // treatment.
    ...
    // Manual title override -- blank clears it, which reverts the effective display title back to
    // the auto-synced `title` (currentSeasonTitle).
    current.titleOverride = interaction.fields.getTextInputValue('season_title')?.trim() || '';
    await seasonalDoc.save();
    // Keep this patch's cached images' Cloudinary metadata (release date/season) in sync -- the
    // release date just changed and applies to every image of this entry.
...
// --- URLS 1 / URLS 2 --- custom_id: modal_patch_urls_{1|2}
// Each of the 5 URLs is its own field -- a blank field means "no image in this slot". URLs 1 owns
// images[0..4], URLs 2 owns images[5..9] -- each submit only replaces its own half, preserving
// whatever the other slot has saved.
async function setUrls(interaction) {
    ...
    // Re-host each submitted URL into this season's own Cloudinary folder, keyed by this patch
    // note's own _id -- NOT the season title, since a title can be renamed later without touching
    // this doc's _id. slotOffset maps each field to its ABSOLUTE position in images[] (URLs 1 = 0-4,
    // URLs 2 = 5-9) so re-submitting a slot overwrites the same cached asset in place rather than
    // accumulating duplicates. A Cloudinary hiccup must never block the save -- cachePatchImage()
    // falls back to the raw URL on failure.
...
// --- ADD NEW SEASON --- custom_id: modal_patch_addseason
// Always PUSHES a brand-new patchNotes[] entry, becoming the new "current" entry (the old current
// entry automatically becomes a past season). URLs come in as two multi-line fields instead of 5
// individually-addressable Short fields, matching the original mockup shape.
...
// --- PAST SEASONS: EDIT --- custom_id: modal_patch_editseason_{id}
// Updates ONE SPECIFIC existing entry in place (picked via handlePatchSeasonPick below), never
// touches which entry is "current".
...
// --- PAST SEASONS PICK --- custom_id: mng_patchseason_pick (select menu, rendered on the Patch
// Notes page itself). Its options ARE the full list already, so picking one goes straight to its
// edit modal -- no search step, unlike the generic mng_pick_ disambiguation select.
