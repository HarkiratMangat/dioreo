...
// --- START NEW SEASON, STEP 1 --- custom_id: modal_wipe_season Used to wipe draws+calendar the INSTANT this modal was submitted -- no confirmation at all, unlike every other destructive action in this panel. Now just stashes the entered title in pendingSeasonWipes and shows the same Confirm/Cancel step Purge uses -- the actual wipe only happens from handleWipeButton below.
...
// --- START NEW SEASON, STEP 2 --- custom_id: mng_wipeconfirm_{token} / mng_wipecancel_{token} Snapshots the pre-wipe state so this can be undone -- a season reset is at least as destructive as any single Purge and deserves the same safety net.
...
// --- SEASON TITLES + DEADLINES (merged) --- custom_id: modal_season_titles_deadlines Each of the 3 deadline fields carries both a title and an end date on one line ("Battle Pass, August 28"), split apart via adminParser.js's splitTitleDate(). A blank line leaves that title+date pair untouched.
async function setTitlesDeadlines(interaction) {
    ...
    // Unrecognized date text (e.g. a "TDB" typo for "TBD") returns null rather than silently falling back to the literal current instant, so an unparseable date is treated the same as a blank one (field left untouched) rather than corrupted, and the admin gets told which line was skipped.
    const skippedDates = [];
    // Typing the literal word "TBD" (case-insensitive) is a real, explicit directive -- clears the Date field and sets its matching `${dateField}TBD` flag; seasonend.js/calendar.js both read that flag to show "TBD" and treat the deadline as indefinitely running.
    ...
    // patchNotes[].title is captured independently at "Add Patch Notes" time (so older, past-season entries keep their own historical title forever) -- but the MOST RECENT entry always represents the season that's currently live, so keep it in sync here. Without this, renaming a typo'd season title silently left the current patch notes entry (and therefore the default /patch notes view) showing the old name.
    ...
    await seasonalDoc.save();
    // The most-recent patch's season title just changed -- re-sync its cached images' Patch_Season metadata to match. Best-effort; keyed by the patch _id, which the rename never touches.
    if (seasonalDoc.patchNotes.length > 0) {
...
// --- NEXT SEASON DRAFT: PROMOTE / DISCARD --- custom_id: mng_draftpromoteconfirm / mng_draftpromotecancel / mng_draftdiscardconfirm / mng_draftdiscardcancel. Promote snapshots the pre-swap LIVE values (not the draft) so Undo restores exactly what was live before -- the draft itself is simply cleared, not restorable via Undo (it's reachable again the normal way: just re-stage it).
async function handleDraftButton(interaction) {
    ...
    if (customId === 'mng_draftpromoteconfirm') {
        ...
        // A `if (draft.bpEnd)` truthy check alone would silently skip promoting a TBD deadline -- TBD leaves draft.bpEnd explicitly null, so the TBD flag has to be checked FIRST.
        ...
        // No staging UI exists for draft banners yet (schema-only forward compat) -- these are no-ops today, but copy the same "truthy draft value wins, blank leaves live untouched" convention titles use above, so a future staging modal doesn't ALSO need this promote logic added separately.
        ...
        // Most-recent patchNotes[] entry stays synced to the live title, same as the manual Season Titles & Deadlines flow above.
