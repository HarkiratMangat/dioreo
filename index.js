...
// The three modules this file wires together. Everything that used to live inline here moved into
// them on 2026-08-13 17:20 EDT (docs/ROADMAP.md's index.js split) -- this file is the entrypoint
// again: connect, construct, wire, log in. Nothing else.
...
//
// ⚠️ PRESENCE IS DELIVERED ONLY TO USERS WHO SHARE A GUILD with the account, and prod sits at
// guild_count 0, so on prod this renders to an audience of nobody. That is a DISTRIBUTION fact, not a
// code bug -- do not "fix" it by adding fields, moving it, or setting it again post-ready. Confirmed
// on the dev bot 2026-08-09 11:27 EDT by a controlled add/kick/re-add cycle. It lights up by itself
// for any guild the app joins under the v3 guild-install work (docs/ROADMAP.md).
const BOT_PRESENCE = {
    status: "online",
...
// ⚠️ registerLifecycle installs `client.on("error", ...)`, which is NOT optional and is NOT covered
// by the router's own try/catch -- discord.js's captureRejections reroutes a rejected async listener
// into a client `error` event that bypasses it entirely. See .claude/rules/interaction-router.md.
...
// (see .claude/rules/accent-and-colors.md). The acquireInstanceLock() query is issued via Mongoose,
// which buffers commands until the mongoose.connect() call above actually finishes -- no extra
// wait-for-connection logic needed here.
(async () => {
