...
function register(route) {
    ...
    route('GET', /^\/api\/analytics$/, requireAdmin(async (req, res, url, session) => {
        ...
        // 🔴 "WHO" WAS A ROW OF DIGITS. Harkirat, pin pmtvqy8du, 2026-09-10 12:33 EDT: "the who column is
        // useless right now because wtf does '632283' tell me? nothing." This codebase stores NO usernames --
        // portal/ui/shell.js:217 says so plainly, which is why the identity chip shows an id too -- so the only
        // honest labels are the ones it does hold: the owner is the owner, and a granted admin has whatever
        // note the owner wrote when granting (models/AdminUser.js calls it "a label the owner can add ... e.g.
        // a name"). Everything else keeps its shortened id rather than inventing a name.
        // ⚠️ ONE query per page load, not one per row: a river of 100 rows has one or two distinct actors.
