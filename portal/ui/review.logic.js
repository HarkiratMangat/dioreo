...
//
// Same contract as track.logic.js and board.logic.js: a classic <script> before app.js makes these
// top-level declarations globals an ESM module reads without an import, and Node's require() reads
// the same file as CommonJS. buildPortal emits the script tag for every *.logic.js automatically.

// A blocker is something standing between the staged work and the commit button. Each one names
// what to do about it — a gate that only says "no" teaches nothing.
//
// 🔴 THE NOUN AGREES WITH THE NUMBER, and it is not "gates open". The mockup shipped a masthead
// reading "1 GATES OPEN", warn-coloured, plural on a count of one, meaning the REVERSE of what it
// says: "gates open" reads as cleared, you may pass. On the screen where everything is written.
function blockersFor(ops, changesets, resolved, confirmText) {
    ...
    // 🔴 NEVER THE WORD "DELETE" — muscle memory carries you straight through that one. The word is
    // the changeset's own id fragment, which the server independently expects, so typing it means
    // you looked at the thing you are committing.
