...
//
// ⚠️ IT LEAVES NO TIP BEHIND. A tooltip whose host is removed mid-hover — which the Track does routinely — would otherwise sit on screen pointing at nothing. `pointerout` and `focusout` both kill it, and so does a scroll, because a fixed-position tip does not travel with the thing it describes. ⚠️ NO IMPORT OF THE .logic SIBLING, and the missing line is the point. Every *.logic.js here loads as a CLASSIC script before this module graph evaluates, so its top-level function declarations are GLOBALS — `import { tipPlacement } from './tips.logic.js'` throws "does not provide an export named" in every real browser and takes the whole page down with it. Written that way first, and the page rendered blank; season.js made the same mistake before this, which is why every .logic.js sibling carries the warning in its own header.

let installed = false;

export function installTips() {
    // 🔴 OFF ACROSS THE WHOLE PORTAL, 2026-09-11 14:20 EDT, Harkirat: "fuck off with these pop-up hints. They're ugly, terribly implemented and integrated, and they're more annoying than they are helpful. Completely remove them across the entire portal until i decide to fully revamp their design." The runtime is disabled rather than every `data-tip` stripped: one line to reverse when the redesign happens, and no attribute churn across ten realm files in the meantime. Filed as near-future work.
    return;
}
