...
export function RealmShell({ realm, session, error, slow, onRetry, skeleton = {}, label = 'Loading' }) {
    if (error && (error.kind === 'expired' || error.kind === 'forbidden')) return html`<${NoAccess} error=${error} />`;
    return html`
