                ...
                <div class="cols">
                    ${['draft', 'staged', 'blocked', 'ready'].map(key => html`
                        <div class=${'col' + (key === 'ready' ? ' gate' : '')}>
                            ...
                            ${cols[key].map(c => html`<${Card} changeset=${c} onExport=${onExport} onOpen=${toReview} onDiscard=${key !== 'ready' ? onDiscard : null} selected=${false} />`)}
                            ${key === 'ready' && readyCount ? html`
                                <button class="commit" onClick=${toReview}>
