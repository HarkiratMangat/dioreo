...
function oneWayItems(live, draft) {
    ...
    // 🔴 THE DESIGN'S STRIP IS FIVE OPERATIONS AND THIS IS SEVEN, WITH DIFFERENT WORDS ON THE FIVE THEY SHARE. `draws-all` and `promote` are portal additions, and the notes were rewritten. Both may well be improvements — promote in particular has nowhere else to live — but they are a redesign, and the redesigns come back after every realm matches. Under the conformance flag the strip is the design's, verbatim: same order, same titles, same units, same sentences. 🔴 SEVEN OPERATIONS, NOT THE DESIGN'S FIVE — and this is a REFUSAL, not a leftover. The design's
    //    strip omits `draws-all` and `promote`, which are real destructive operations an admin can perform
    //    here; conforming would have deleted two of the seven irreversible ops from the only screen that
    //    offers them. Same call as the rest of this surface: a conformance pass moves composition, never a
    //    capability. The design's WORDING for the five they share was also refused with it, because those
    //    lines gate on a session export this console does not have — see oneway.js's own comment.
    return [
