// portal/ui/analytics.js — ESM. The Analytics realm: Health, Usage, Timing, Reach and Search. 🔴 The event river and its revert moved to portal/ui/history.js on 2026-09-13 17:41 EDT (batch-2 spec §4) — Analytics is a surface for looking; History carries the one capability.
//
...
import { HISTORY_FILTER_KEY } from './history.js';
...
export function AnalyticsRealm({ session }) {
    ...
    // 🔴 THE RIVER LIVES IN HISTORY NOW (2026-09-13 17:41 EDT, batch-2 spec §4), AND THESE LINKS MOVED WITH IT RATHER THAN GOING DEAD. Health's tiles and level rows filtered the river on this page and scrolled to it; the manifest left Analytics whole because it carries revert, a capability, while this realm is for looking. Routing is an exact hash match, so the filter cannot ride in the URL — it is handed over in sessionStorage and History consumes it once on mount. Storage blocked means History opens unfiltered, which is still the right page.
    function filterRiver(filters) {
        try { sessionStorage.setItem(HISTORY_FILTER_KEY, JSON.stringify(filters)); } catch { /* see above */ }
