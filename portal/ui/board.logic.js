...
//
// Board is the CHANGESET PIPELINE (spec F3 / §8.2), not a third content view -- its columns are
// pipeline stages (Draft -> Staged -> Blocked -> Ready), derived from a Changeset document's own
// state/tier/exportedAt, never assigned by hand.
//
// 🔴 Task 5's Files note says these three .logic.js files "import nothing" -- so gateCommit's logic
// is inlined here rather than required from portal/api/policy.js (this file must also load as a
// plain classic <script> in the browser, where require() does not exist). This is a deliberate,
// tiny duplication of a 4-line pure function, not the drift utils/manageActions.js's header warns
// against -- scripts/portalApi.test.js and scripts/portalUi.test.js both assert the identical
// tier-3 behaviour, so the two copies disagreeing would fail a test immediately, not silently.
...
// Which Board column a changeset belongs in. 'blocked' is a REAL column stating why (spec §8.2),
// derived from the same gate the server enforces -- never a separate client-side guess that could
// disagree with what commit would actually do.
