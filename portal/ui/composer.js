...
//
// ⚠️ Debounced, and every reply is checked against the value that is in the field NOW. Typing "sep" then "sep 21" fires two requests, and nothing guarantees they come back in that order; without the guard the slower "sep" reply overwrites the newer "sep 21" answer and the echo contradicts the field it sits under. 🔴 EXPORTED 2026-09-10 17:43 EDT, AND THE REASON IS PIN pmtvp9ur7. Harkirat asked for *"a pop-up date picker on every date field portal-wide"*. The portal already has something strictly better than a picker and it was reachable from exactly TWO fields, both inside this file: seven other date inputs across Season and Broadcast were raw `<input type="date">`, which cannot take "in 3 weeks" and cannot take a paste out of a patch note. So the answer to that pin is not a new control — it is this one, everywhere. ⚠️ `chrome` EXISTS BECAUSE THE CLASSES ARE THE COMPOSER'S, NOT THE FIELD'S. `nw-l` / `nw-i` / `nw-date-echo` are the composer's own form vocabulary; a drawer's is `.dwfield` + `label` + `input`. Carrying the composer's classes into a drawer would put a second form language inside one drawer, which is the defect `.dwfield` was created to end. Same component, same parser, the host's own chrome.
