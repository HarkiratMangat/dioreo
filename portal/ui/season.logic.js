...
//
// 🔴 THE MANIFEST ROW'S `lane` ('newDraws'/'returningDraws'/'calendar' — the SeasonalData ARRAY
// PATH name, see season.js's toManifestRows) is NOT the vocabulary core/ops/draws.js's validate()
// accepts for payload.category/target.category ('new'/'returning' — see LANE_TO_CATEGORY below).
// Passing the lane straight through fails validation silently differently than expected; this
// mapping is the fix, found by reading core/ops/draws.js's validateOne() before writing this file.
...
function buildSeasonAddOp(kind, fields) {
    ...
    if (entity === 'draw') {
        // core/ops/draws.js validates payload.date (matching the SeasonalData schema's newDraws/
        // returningDraws[].date field, and utils/adminParser.js's parseBulkDrawList -- draws have no
        // separate start/end, unlike calendar events, whose schema genuinely has both).
...
// needs the full record, not a partial patch (core/ops/draws.js, core/ops/calendar.js). Dates are
// passed as bare ISO date strings (YYYY-MM-DD) rather than a full ISO datetime, since validate()
// re-parses them through chrono-node's parseAdminDate() (an op arriving as JSON over HTTP never
// satisfies the "already a real Date instance" fast path those functions also support) and a bare
// date is the form that parser is built for.
...
function buildSeasonEditOp(row, columnKey, newValue) {
    ...
    // The Manifest row's own display field is called `window`/`endDate` regardless of entity (see
    // toManifestRows in season.js), but a draw's real schema/op field is `date`, not `endDate` -- a
    // row edit on the Manifest's synthetic `endDate` display key must be routed onto the correct
