...
// Discord serves animated assets in two different containers and ffmpeg needs to be TOLD which one
// for APNG (see the `-f apng` note above), while it auto-detects GIF correctly. Hardcoding `-f apng`
// therefore breaks a GIF source outright, which is why this sniffs the real bytes instead: APNG is a
// PNG signature plus an `acTL` chunk (always before the first IDAT, so a bounded search is enough),
// GIF is its own magic. Anything else returns null and ffmpeg is left to work it out itself.
...
// How many frames does this animation actually have? Needed because the frame STRIDE has to be
// derived from the real length -- see the span bug fixed below. Returns null (rather than throwing)
// if ffprobe is unavailable or the input is a single still, so the caller degrades to stride 1
...
// ⚠️ SPAN BUG FIXED (2026-08-11 01:45 EDT). The stride here used to be the literal constant 3 with a
// 9-tile grid, so this only ever sampled frames 0,3,...,24 -- the first 25 frames, no matter how long
// the animation was. The comment claimed it "spans the whole cycle"; that was only true for
// animations of 27 frames or fewer, and it silently shortchanged everything longer INCLUDING the
// 60-frame decoration this function was written for.
// Measured on a real 85-frame banner whose pale sky blooms into crimson flowers only after frame ~34:
// a single frame lost 83% of the image's peak chroma and THIS FUNCTION lost 84% -- i.e. the montage
// was no better than the single frame it replaced. Deriving the stride from the real frame count
// recovers the crimson (#A90824). Harkirat predicted this failure from the source GIF before it was
// measured, exactly as he did for the decoration's gold sparkle.
// Still deterministic: the frame count, stride and grid are all fixed functions of the input, with no
...
async function extractFrameMontage(sourceUrl, frames = MONTAGE_FRAMES) {
    ...
    try {
        ...
        // ceil (not floor) so the selection reaches the END of the animation: 85 frames over 9 tiles
        // gives stride 10 -> frames 0,10,...,80, covering 94% of the cycle. floor would give stride 9
        // and stop at 81 having emitted 10 candidates for 9 tiles, wasting one.
        ...
        await new Promise((resolve, reject) => {
            ...
            args.push('-i', inputPath,
                ...
                //
                // ⚠️ SCALE BEFORE TILING, always. A 3x3 grid multiplies the source's own resolution
                // NINEFOLD: measured 2026-08-11 01:50 EDT, a 999x427 animated banner tiled out to
                // 2997x1281 = 3.8M pixels for Jimp to decode SYNCHRONOUSLY -- precisely the event-loop
                // stall that produced the bot-wide 10062 "Unknown interaction" wave (see
                // .claude/rules/accent-and-colors.md). This never bit before because the only caller
                // was decoration, whose assets are ~160px square; it becomes a real hazard the moment
                // banners go through here. k-means samples ~2500 pixels regardless of resolution, so
                // capping each tile at 160px wide costs nothing in quality. `min` is escaped because
                // an unescaped comma would be read as a filter separator, silently breaking the chain.
