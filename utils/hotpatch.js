...
// ==========================================
// THE INVARIANT: a reload is sound iff its dependent closure terminates at a LATE-BOUND BOUNDARY and every member of that closure is either stateless or declares a __hotSwap contract.
//
// A BOUNDARY is a module that resolves its dependency at CALL time, not at require time, so it never holds a stale reference and never needs reloading. Two exist:
...
// A boundary is where the closure ENDS. It is never a member, so its own module state is irrelevant -- which is exactly why router.js's interactionCooldowns Map does not block all 15 handlers.
//
// ⚠️ The graph is STATIC (read off disk), not built from require.cache. The runtime cache only records requires that have actually EXECUTED, so a lazy require() in a cold branch is missing from it -- fine for soundness (a lazy dependent re-resolves anyway and needs no invalidation) but it makes the answer depend on what the bot happened to run, un-testable offline, and unusable for `scripts/hotpatch.mjs --dry-run` with no bot attached. The static graph counts every edge as early-bound: closures come out LARGER, so the error is always toward REFUSING. That is the safe direction, and it is a deliberate trade, not an oversight.
...
// Structural refusals. index.js and bot/* own the process lifecycle (crash handlers, the client, the gateway listeners, the instance lock) -- re-requiring them would register a SECOND set. models/* is here for a different reason that is just as absolute: models export mongoose.model('Name', schema), which THROWS OverwriteModelError on a second call. No __hotSwap contract can make either category safe. Verified 2026-08-20 11:39 EDT that no models/*.js currently requires a utils/ file, so a model reaching a closure is not possible today -- this guard exists so that a future edge cannot make it possible silently.
...
function buildReverseGraph(modules) {
    ...
    for (const mod of modules) {
        ...
        for (const match of src.matchAll(/require\((['"])(\.[^'"]+)\1\)/g)) {
            ...
            // ⚠️ Resolve a DIRECTORY require too. `require("./manage")` from handlers/router.js means handlers/manage/index.js; a naive `+ ".js"` looks for a handlers/manage.js that does not exist and silently DROPS the edge. `./manage` is the only true directory require in the tree today (verified 2026-08-20 11:47 EDT), which is exactly why it would go unnoticed.
...
// Module-scope state that MUTATES, plus live timers and process listeners. A `new Set([...])` used as a constant lookup table is NOT state -- treating it as such is a false positive that would wrongly refuse utils/colorNames.js (DROPPED_NAMES) and utils/search.js (GENERIC_DRAW_WORDS). The test asserts exactly that case.
...
function planHotpatch({ files }) {
    ...
    // `unresolved` = named a path that is not a scanned module at all (deleted by the pull, a non-runtime file, a typo). Kept separate from runHotpatch's `stray` -- both used to be called `unknown` and the panel printed one message for two different situations.
...
// A commands/*.js must satisfy the SAME predicate bot/registry.js's loader uses, or a swapped module would register into client.commands in a shape the router cannot dispatch.
const isCommandShape = m => m && typeof m === 'object' && 'data' in m && 'execute' in m;

/**
 * Verify-then-swap, ALL OR NOTHING. On any failure the old module objects are put back into require.cache and client.commands, so the process never runs a half-updated tree -- a bot that LOOKS fine while running mixed code is strictly worse than one that is plainly restarting.
 */
// ONE AT A TIME. Discord and the CLI can both fire; two interleaved delete/re-require passes would defeat the snapshot/restore. ✅ Consequence worth keeping: this `let` makes utils/hotpatch.js classify as stateful, and bot/lifecycle.js requires it, so hotpatch is REFUSE_STRUCTURAL on its OWN engine and can never patch itself. That is correct -- do not work around it.
let inFlight = false;
...
async function applyHotpatchInner(plan, { client }) {
    ...
    // 3. Let any module that declares a contract hand its live state forward. ⚠️ planHotpatch's __hotSwap detection is a SOURCE REGEX, so a comment mentioning the name would satisfy it. This is the authoritative check: if the plan waved a module through on a contract it does not actually have, fail here rather than silently dropping its state.
    ...
    try {
        ...
        for (const [rel, mod] of fresh) {
            ...
            // 🔴 DISCORD-FACING SHAPE. The command list Discord shows comes from ONE REST PUT at boot. Swapping the module changes what execute() DOES, never what Discord SHOWS -- so a new command would land in client.commands with no way to invoke it, and a changed option or description would leave every user on the old shape, both under a ✅ Applied message. Refuse instead. Re-PUTting from here drags in rate limits, applyGunsmithsScopeChoices's builder mutation and gateableCommandNames -- far more than "minor hot fix" warrants.
        ...
        // 7. Re-wire the one boundary that needs it. Handlers need none -- the router resolves them at call time (Task 2), which is the whole point of that change.
        if (client?.commands) {
            for (const [rel, mod] of fresh) {
...
// The commit whose code this PROCESS is running -- from utils/logger.js, which resolved it at require time and which noteHotpatch() keeps current across successive patches. 🔴 NOT `git rev-parse HEAD`. Harkirat pulls separately and then hotpatches, so HEAD is ALREADY the new commit by the time this runs: diffing against it yields an empty changed set and a cheerful "nothing to patch" over a process still running old code. Verified 2026-08-20 11:47 EDT that DIORS_COMMIT is never set by anything, so there is no env-var shortcut either.
function runningCommit() { return require('./logger').currentCommit(); }

// Cheap: one `git diff`, no graph, no file scanning. Autocomplete calls THIS, never runHotpatch -- building the graph reads 128 files, and autocomplete fires per keystroke inside Discord's 3s budget on a shared-core e2-micro. commands/settings.js:54 records what happened the last time something heavy sat on that path: unrelated interactions missed the ACK window and died with 10062.
...
async function runHotpatch({ client, files = [], pull = false, dryRun = false }) {
    ...
    if (result.ok) {
        // OBSERVABILITY. logBootBanner()/.restart-reason/BootRecord all key on a PROCESS START, so a hotpatch is invisible to every one of them -- and "attribute every journal line to a version+commit" is the property the whole ops layer rests on. These three lines are what keep it true. Do not tidy them away; see .claude/rules/hotpatch.md.
