...
// server-admin visibility policy (2026-08-10 15:50 EDT, utils/guildPolicy.js). A server rule that
// forces ephemeral is applied at two choke points instead -- the reply/deferReply/followUp clamp on
// the interaction itself, and sendV2Payload's strip of the "Show Everyone" row -- neither of which
// any command has to remember. Threading a `guildPolicy` argument through here as well was built
// and then removed: it would have meant an optional parameter at nine call sites where forgetting
// it looks identical to passing it, which is the failure mode the choke points exist to prevent.
