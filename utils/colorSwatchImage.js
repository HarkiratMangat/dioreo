...
// A solid swatch is deterministic per hex and never changes -- but the View Colors panel regenerates
// up to 4 of them on EVERY render (every page/subpage switch re-encodes the exact same PNGs). Memoize
// by hex so each distinct color's PNG is encoded once per process instead of once per render, taking
// that redundant synchronous PNG encoding off the panel's hot path (2026-07-14 CPU pass). Bounded so
// a long-running process can't grow it without limit -- realistically only a few dozen distinct hexes
// ever appear across a user's sources, so this rarely evicts. The cached Buffer is only ever READ
// (uploaded as an attachment), never mutated, so sharing one reference across renders is safe.
