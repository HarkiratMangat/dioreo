...
//     ```
// Harkirat, on receiving it: *"absolutely no clue what it meant"*. That alert is technically perfect
// and practically useless — it says WHERE in the websocket library the failure surfaced, and nothing
// about whether the bot is down, whether users are affected, or whether he needs to get up and do
// something. An alert exists to drive a DECISION; one that can't be read can't drive one.
//
...
//
// ⚠️ THE TECHNICAL DETAIL IS NEVER REMOVED, only demoted. The stack frames are what make an alert
// debuggable later; the failure was that they came FIRST and alone. Plain language on top, evidence
// underneath — both readers served, neither at the other's expense.
//
// ⚠️ Keep the wording free of jargon that only makes sense if you already know the answer. "The
// websocket dropped" explains nothing to someone asking what a websocket is. "Discord's connection
// blipped" does. If a line needs a term of art, define it inline in the same sentence.

// Discord gateway close codes that are NOT self-healing — the connection will never come back on its
// own, because the problem is the credentials or the app's configuration, not the network. Separating
// these out is the whole point of the selfFix line: 4004 and 4014 look identical to a transient blip in
// the raw alert (both are just "disconnected, close code NNNN"), but one clears itself in seconds and
// the other means the bot is DOWN until someone changes something. Source: Discord Gateway docs.
...
// Network-level causes worth naming, keyed by a fragment that appears in the error text. These are the
// ordinary ways the internet fails, and every one of them is transient — which is exactly the fact the
// raw message fails to convey. `503` was the live case that prompted this whole module.
...
// The bot survives everything in here — it stays running and keeps answering commands. Stated
// explicitly because "🔴 error" reads as "the bot is down" when usually it is not.
...
// Pull the close code out of a shardDisconnect detail string ("... close code 4004)"). Returns a
// number or null. Deliberately tolerant of the surrounding wording so a future rephrase of the
// detail line does not silently turn every disconnect back into "probably transient".
function closeCode(detail) {
    const m = /close code (\d{4})/i.exec(detail || '');
    return m ? Number(m[1]) : null;
}

// DISPLAY NAMES — presentation only. The canonical `title` stays exactly as the call sites pass it,
// because it is a KEY: sendAlert throttles on `${level}:${title}`, AlertLog stores it, and `/alerts`
// reads it back. Renaming the key would fragment the stored history and silently reset every throttle.
// So the human label is applied to the embed title and the notification line ONLY.
//
// This matters more than it looks: the notification content line is `<@user> 🔴 **<title>**`, which on a
// phone at 03:00 is the ENTIRE message — the embed is not shown in a push notification. "Gateway shard
// error" is the jargon that started this; "Discord connection error" is the same fact in words.
const DISPLAY_TITLE = {
    ...
    // 'Bot online', 'Back online', 'Reconnecting to Discord' and 'Daily health check' are already plain
    // English — an entry mapping them to themselves would just be a line to keep in sync.
};
...
//
// Returns null for titles with nothing useful to add (a plain-language explanation of "Bot online" is
// just the title again). null means "render this alert exactly as before" — the layer is additive and
// an unknown title must never lose its technical detail.
    ...
    switch (title) {
        case 'Gateway shard error': {
            ...
            return {
                ...
                // ⚠️ Every alert name quoted in this file must be a DISPLAY_TITLE, i.e. what he will
                // actually see. An earlier draft pointed at "Discord connection lost" while the alert
                // still displayed as "Gateway disconnected" — sending someone to look for a message
                // that does not exist under that name is the same failure this module exists to fix.
        ...
        // 'Gateway resumed' shares this branch. It is normally `silent` (logged, never posted), so it
        // renders only if that ever changes — but the two tables in this file must not drift apart:
        // a title with a display name and no explanation would print a polished heading above a bare
        // stack trace, which is half of the original problem. The unit test asserts they stay in step.
        case 'Back online':
        ...
        // Deliberately no entry: 'Bot online' and 'Daily health check' already say exactly what they
        // mean in their own titles and bodies. Adding an explanation block to them would be noise, and
        // noise is what trains someone to stop reading alerts.
        default:
            return null;
    }
}
