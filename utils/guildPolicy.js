...
//
// THE MODEL, in one line: a server can quiet the bot, never make someone louder than they chose to
// be. An 'ephemeral' verdict is a CEILING -- it overrides the user's `visibility` option and their
// saved UserPreference. A 'public' verdict only PERMITS a public response; the user's own choice
// still decides. So a server rule can never rewrite a personal preference, only constrain where and
// how loudly the bot speaks. Full design + the rejected alternatives:
// docs/superpowers/specs/2026-08-10-server-admin-visibility-policy-design.md
//
// ⚠️ NOTHING HERE REFUSES A COMMAND, and that is a platform limit, not a scoping decision. A bot
// cannot hide a global slash command inside one guild: guild-scoped registration does not delete
// global commands, Discord merges both sets, and per-command permissions can only be written with
// an OAuth2 bearer token from a MANAGE_GUILD user. Admins already hold that lever natively in
// Server Settings -> Integrations. So every tier below resolves to public-or-ephemeral, and a
// future session should not "finish" this by adding a refusal path -- see models/GuildSettings.js.
//
...
// interaction itself, and utils/sendV2Payload.js strips the "Show Everyone" row on the way out.
// Same reasoning as utils/logger.js's patchConsole(): one patch that cannot be forgotten beats a
// rule that must be remembered.
const { PermissionFlagsBits } = require('discord.js');
const GuildSettings = require('../models/GuildSettings');

// MessageFlags.Ephemeral. Spelled as a literal rather than imported because it gets OR-ed into the
// Components V2 flag (32768) below, and seeing `| 64` next to `32768` makes the arithmetic legible.
...
// Returns the guild's settings document, or `null` when that server has never configured anything.
// A null is CACHED deliberately: the overwhelmingly common case is a server with no rules at all,
// and caching only the hits would leave exactly that case hitting Mongo on every interaction.
...
// The member's role ids, from whichever shape the interaction carries. discord.js gives a real
// GuildMember (roles.cache) when the guild is cached, and a raw APIInteractionGuildMember (roles as
// a plain id array) otherwise -- the raw shape is the norm for a user-installed invocation in a
// server the bot is not a member of, which is precisely a case this feature has to cover.
...
// "Is this invoker a server admin?" -- answered from the interaction payload, which already carries
// the member's COMPUTED permissions, so the common path costs zero REST calls and needs no
// privileged intent. (The GUILD_MEMBERS intent gates *enumerating* members; this never does that.)
// The ownerId fallback covers a server owner whose permission bits somehow do not include the
...
//   0. ADMIN BYPASS -- owner / Administrator / MANAGE_GUILD skip tier 1 entirely and resolve from
//      tier 2 down. Deliberately narrow: it exempts them from the COMMAND rule only, never from the
//      channel or role rules, because those they set for the whole server including themselves.
...
//   5. defaultVisibility
// A same-tier conflict (the member holds two roles disagreeing at the same level) resolves to
...
function forceEphemeralResponses(interaction) {
    const clamp = options => {
        ...
        next.ephemeral = true;
        // Preserve an explicit flags number (Components V2 sends pass 32768) rather than replacing
        // it -- dropping that flag would render a V2 payload as a blank message.
...
async function attachGuildPolicy(interaction) {
    ...
    const visibility = resolveVisibility(settings, {
        ...
        // ephemerality Discord fixed when it was first sent, so a command rule has nothing left to
        // decide there -- the tier is simply skipped and resolution falls through to role/channel.
