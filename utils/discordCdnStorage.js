// utils/discordCdnStorage.js -- posts a rendered animated-preview file ONCE to a dedicated, permanent storage channel (Harkirat's "dioreoland" server, added 2026-08-10 12:08 EDT specifically for this), capturing the resulting cdn.discordapp.com attachment url so future renders can reference it directly instead of re-fetching+re-attaching on every single view.
//
// WHY this exists at all (the full investigation is recorded in utils/nameplateWebpCache.js's/ utils/decorationWebpCache.js's own headers and in linksee memory): a Components V2 media-gallery item referencing an EXTERNAL url (Cloudinary or otherwise) gets proxied through Discord's own images-ext-N.discordapp.net and RE-ENCODED to lossy before any client sees it -- confirmed live, and confirmed NOT fixable via a query-string hint (breaks the reference entirely) or by using GIF instead of WebP (also gets reprocessed, not byte-identical). The ONLY confirmed-working bypass is referencing Discord's OWN cdn.discordapp.com domain, which is never proxied. A signed attachment url's `ex`/`is`/`hm` params DO expire, but per Discord's own docs (developers/reference.mdx, "Signed Attachment CDN URLs"), passing a stale one back into an API field (exactly what this does) gets it auto-refreshed by Discord server-side -- so no expiry-tracking/refresh logic is needed here.
//
// This is a genuinely DIFFERENT capability from everything else this bot does: CLAUDE.md's hard invariant is that this bot has ZERO standing guild permissions and only ever acts through an interaction-response webhook, never a raw proactive channel post (that fails 50001 Missing Access everywhere else). This module's target channels are the one deliberate exception -- Harkirat invited the bot to that specific server with real channel permissions for exactly this purpose. Never reuse this pattern to post anywhere else without the same explicit setup.
...
// Same walk as findFirstMediaUrl, generalized to collect EVERY resolved cdn.discordapp.com media url instead of stopping at the first (2026-08-15 10:35 EDT, added for scripts/bulkCacheCollectibles.js's grouped multi-variant cache-channel messages -- "keeping the channel tidy" per Harkirat's request). Visits arrays/object values in the SAME deterministic order findFirstMediaUrl always has, which is what makes positional zipping safe: the caller builds `files` and `components` in one loop over its variants (file[i]'s `attachment://filename` sits in components entry i), and Discord neither reorders components in its response nor gives any other reliable way to map a resolved url back to which attachment it came from (`message.attachments` is frequently `[]` for a `attachment://` ref used only inside a component -- see findFirstMediaUrl's own comment). So `findAllMediaUrls(msg.components)[i]` is that same i-th variant's resolved url, as long as the caller preserves that ordering -- it is the caller's responsibility, not enforced here.
