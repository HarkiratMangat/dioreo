// Shared weapon-lookup + render path for /dmz and /gunsmiths search -- previously duplicated as commands/dmz.js's execute() and handlers/router.js's ~110-line MP fallback (the dynamically generated /all + /<category> commands had no 'data'+'execute' module in commands/, so interaction.client.commands.get() always missed and control fell through to that block). Both copies are gone; this is the one path both callers now share, differing only by `mode`.
...
async function lookupAndRenderWeapon(interaction, { mode, rawQuery, requestedBuild, visibilityChoice }) {
    ...
    // Normalized the same way autocomplete's own weaponKey values already are -- harmless for a picked suggestion (already normalized), but lets a free-typed exact name with different casing/spacing still hit exactly.
    const weaponKey = rawQuery.toLowerCase().replace(/\s+/g, '');

    // Same "Weapon Builds" toggle every loadout lookup shares (Option A pattern, matches `seasonalVisibility`). The builds query doesn't depend on prefs at all, so it's kicked off alongside prefs instead of after -- only `prefs` is actually awaited before deferReply, to keep the 3-second ack window fast. .lean() since these builds are only ever read here, never saved.
    ...
    if (!builds || builds.length === 0) {
        // NOTE: awaited + wrapped in its own try/catch -- an unawaited `return interaction. followUp(...)` inside a try block can reject AFTER the block has already exited, which can crash the whole process. See CLAUDE.md's crash-resilience notes.
    ...
    // `build` lets a user jump straight to a specific build number (1-based, matching the "Build N of M" footer text) instead of always landing on the first. Clamped into range rather than rejected outright if it's out of bounds.
    const buildIndex = requestedBuild ? Math.min(Math.max(requestedBuild - 1, 0), builds.length - 1) : 0;

    // Category-wide build list for the "Browse other builds" dropdown. MP scopes to the found weapon's own category (matches every /<category> command's old per-category scope); DMZ has no per-category split (models/Loadout.js -- categoryRank is never set for DMZ builds), so its dropdown scope is every DMZ build, same as /dmz always did.
