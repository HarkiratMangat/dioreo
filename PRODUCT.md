...
- **The design authority is a set of mockups, and conformance to them is measured, not judged.** `npm run portal:audit` and the overlay tooling produce a numeric difference against the mockup; a realm is closed when that number is small, not when it looks right. `?conform=1` stands pending redesigns down so the portal renders the mockup's version.
