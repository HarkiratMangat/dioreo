...
// ==========================================
// Two subcommands, two doors onto one engine (see utils/loadoutScopes.js's scope descriptor):
// `search` is "I know the weapon" (MP-only — DMZ stays reachable via `list scope:DMZ` and the
// standalone /dmz), `list` is "show me a set" (11 named scopes: 7 categories + All MP + Meta MP +
// Meta DMZ + DMZ). Discord forbids a command from mixing top-level options with subcommands, which
// is why there is no shared `category:`/`mode:` option anywhere -- every browsable set is one
// whole named `scope:` choice instead, so no invalid combination can ever be assembled.
...
// Static default so `data.toJSON()` is a valid, fully-shaped command at require time (no DB round
// trip needed to import this module, or to run scripts/gunsmithsCommandShape.test.js standalone).
// bot/registry.js's applyGunsmithsScopeChoices() overwrites these with the live DB-derived list
// post-ClientReady, the same 7 categories today per the spec's measured facts -- this list only
// matters as a fallback/default and to keep this module deterministically testable.
...
// Matches a scope descriptor back to its human label -- a FIXED_SCOPES entry's own label for the
// 4 fixed scopes, or the display-cased category name for one of the 7 category scopes.
...
// Shared by execute()'s `list` branch, a `~cat~` search row, and every gsb~ next/prev/jump click
// in handlers/loadouts.js -- the single render path for every scoped browse view.
async function renderScopeBrowse(interaction, scope, flatIndex, { isUpdate = false } = {}) {
    let isEphemeral;
    if (isUpdate) {
        // No execute() to re-resolve from on a gsb~ click -- read it off the message being edited,
        // exactly like the existing mp/dmz pagination handler does. Getting this wrong makes the
        // Share button silently disappear after the first click (see .claude/rules/rendering-and-ui.md).
