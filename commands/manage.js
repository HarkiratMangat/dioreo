...
// Moved to utils/owner.js (2026-08-20, portal core Task 0b) -- utils/adminAccess.js used to reach into this file just for this constant, which transitively pulled discord.js/jimp into any process that needed permission checks. Re-exported here so nothing outside this task's file list breaks.
...
function buildAnnouncementListBlocks(announcementDocs) {
    ...
    announcementDocs.forEach((a, i) => {
        ...
        blocks.push({ type: 10, content: `### ${emojis.announcements} Announcement ${i + 1}\n-# Posted <t:${postedTs}:R> — ${expiryText}\n${preview}` });
        blocks.push({
            type: 1, components: [
        ...
        groups: [
            {
                ...
                blocks: [
                    `### ${emojis.mngInfo} How Images Work\n-# **Fastest path: ${mentionCommand(client, '/autobuild')}** — hand it a Gunsmith screenshot (or an image URL) and it uploads the image to Cloudinary AND creates the loadout for you, no image key to type. Use the manual entry below when you already have the image hosted or want fine control.\n-# **Paste a URL (this panel):** the image field on Add and Edit takes an image **URL** as well as a Public ID — paste one (a Discord attachment link works) and the bot uploads it to Cloudinary for you under a \`WeaponKey-BuildNum\` key, no dashboard trip and nothing to type. On **Edit** it re-uploads over that build's EXISTING key, so replacing a picture never breaks anything pointing at it.\n-# **Or type a Public ID:** if the image is already on Cloudinary, type its Public ID exactly — by default your file's original name (e.g. \`IMG_5630\`) unless you renamed it. The bot builds the card straight from that key and warns you on save if nothing is actually there. Rename to \`WeaponKey-BuildNum\` (e.g. \`BAL-27-1\`, \`FSS-HURRICANE-1\`) to match the rest of the collection. (${mentionCommand(client, '/autobuild')} already names it this way automatically.)`
                ],
                buttons: []
            },
            {
...
function buildLoadoutsBulkAddModal(mode) {
    // FORMAT REDESIGNED 2026-08-22 -- see utils/adminParser.js's parseBulkLoadoutList docblock for the shape and the reasoning. The label states the whole principle in one line, because the complaint that drove this was that the old format had no statable principle at all: identity on line one, one optional field per line, everything else is an attachment. NO Mode field any more, and it is not an omission: the old format REQUIRED Mode as pipe-segment 3 while the save path overwrote it with this page's `mode` regardless -- so a DMZ block pasted here saved as MP with no warning. The page has always been what decides; the format now matches.
    const modal = new ModalBuilder().setCustomId(`modal_loadouts_bulk_add_${mode}`).setTitle(`Bulk Add ${mode} Loadouts`);
    modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('bulk_text').setLabel('Weapon | Category, then one field per line').setStyle(TextInputStyle.Paragraph)
            .setPlaceholder(`BAL-27 | AR\nBuild: Flex\nImage: BAL-27-1\nCode: 1I2C6B8A9D\nBadges: meta\n- Gauge-9 Mono`)
            .setRequired(true))
    );
    return modal;
}
...
function buildAddLoadoutModal(mode) {
    ...
    modal.addComponents(
        ...
        // `Build | Share Code` is pipe-delimited, same convention as the Category|Badges field two rows down -- there's no room for a 6th field (Discord's hard per-modal cap), and Edit never had a real Share Code field to show at all until this (2026-08-22 19:40 EDT click-test fix; see models/Loadout.js's own comment on why the two were conflated). Share Code is genuinely optional -- a bare build name with no `|` at all OMITS the key entirely (preserves an /autobuild-set code untouched), matching the existing must-omit-to-preserve contract in core/ops/loadouts.js.
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('build').setLabel('Build Name (optional) | Share Code (optional)').setStyle(TextInputStyle.Short).setPlaceholder('Aggressive Flex | ABCD-1234-EFGH').setRequired(true)),
        ...
        // Label + placeholder rewritten (2026-07-18) -- the old placeholder (`bp50_flex_v1`) wasn't even the convention actually used anywhere in the live collection (real Cloudinary keys are `WeaponKey-BuildNum`, all-caps-with-hyphens, e.g. `BAL-27-1`/`FSS-HURRICANE-1` -- confirmed by querying the live Cloudinary account, not guessed). 2026-08-22: this field now takes EITHER a Public ID or a pasted http(s) image URL, which is why it is no longer "the ONE field in this modal that depends on a step happening OUTSIDE the bot first" -- a URL is uploaded to Cloudinary under a derived `WEAPON-N` key on submit (handlers/manage/loadouts.js's resolveImageInput). The two forms are told apart by the URL scheme, so this costs no extra modal slot -- which matters, since Discord caps a modal at 5 and this one uses all 5.
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('image').setLabel('Cloudinary Image Key, or an Image URL').setStyle(TextInputStyle.Short).setPlaceholder('e.g. BP50-1 -- or paste an image URL to upload it').setRequired(true)),
...
function buildEditLoadoutModal(targetLoadout, targetId) {
    ...
    modal.addComponents(
        ...
        // Pre-fills the real shareCode (if any) after the pipe -- see Add Loadout's comment on this same convention. Reopening Edit with no shareCode set pre-fills just the bare build name (no trailing ` | `), so re-submitting unchanged still omits the key and doesn't clobber an autobuild-set code with an empty string.
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('build').setLabel('Build Name (optional) | Share Code (optional)').setStyle(TextInputStyle.Short).setValue(targetLoadout.buildName + (targetLoadout.shareCode ? ` | ${targetLoadout.shareCode}` : ''))),
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('attachments').setLabel('Attachments (One per line)').setStyle(TextInputStyle.Paragraph).setPlaceholder('Gauge-9 Mono\nCrown-H3 Barrel\nOWC Skeleton Stock').setValue(targetLoadout.attachments.join('\n'))),
        // Same undefined-to-setValue() guard as buildEditDrawModal's thumbnailUrl fix above. Label + placeholder matched to Add Loadout's wording -- see that modal's comment for both the Public ID convention and the 2026-08-22 URL intake. Pasting a URL OVER the pre-filled key is how "replace this build's image" works -- the upload deliberately targets the key already sitting in this field, so the stored public_id never changes and nothing referencing it breaks. That was the explicit ask in the deferred item.
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('image').setLabel('Cloudinary Image Key, or an Image URL').setStyle(TextInputStyle.Short).setPlaceholder('e.g. BP50-1 -- or paste an image URL to upload it').setValue(targetLoadout.imageKey || '')),
...
function buildPatchAddSeasonModal() {
    ...
    modal.addComponents(
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('description').setLabel('Additional Info (optional)').setStyle(TextInputStyle.Paragraph).setPlaceholder('Per-line: # Weapon, Attachment, b:/n:/f: text...\nb/n/f: = buff/nerf/fix emojis (See Guide button)').setRequired(false)),
...
function buildPatchEditSeasonModal(entry, userTimezone) {
    ...
    modal.addComponents(
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('description').setLabel('Additional Info (optional)').setStyle(TextInputStyle.Paragraph).setPlaceholder('Per-line: # Weapon, Attachment, b:/n:/f: text...\nb/n/f: = buff/nerf/fix emojis (See Guide button)').setValue(entry.description || '').setRequired(false)),
...
function buildAnnouncementModal(announcementDoc) {
    ...
    modal.addComponents(
        ...
        // Added 2026-08-22 19:40 EDT (click-test fix) -- core/ops/announcements.js has validated/applied a real `startsAt` since the portal work landed, and utils/announcement.js's display query already withholds an unstarted announcement; only this modal never had a field to set one. Blank means "show now" on both Post and Edit -- same overwrite-not-merge convention `expiry` already uses, not a "leave unchanged" no-op.
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('starts_at').setLabel('Starts On (blank = show now)').setStyle(TextInputStyle.Short).setPlaceholder('e.g. July 15').setValue(isEdit && announcementDoc.startsAt ? formatAdminDate(announcementDoc.startsAt) : '').setRequired(false))
    );
    return modal;
}
