...
//
// 🔴 WHY. §0.7d says the deliverable is not a report and not a percentage: it is the two pages side by side, looked
// at. §0.5b says a Part ships frames that are CAPTURES OF TWO RUNNING PAGES and never a reconstruction. Both were
// true and neither was runnable for a single CONTROL — `portalDiff` shoots whole pages, `portalAudit` prints rows,
// and a question like "the design's commit chip against the portal's" had no answer short of two screenshots taken
// by hand and cropped by eye. A decision that is Harkirat's cannot be put to him as a table of computed styles.
//
// ⚠️ IT ANSWERS "WHAT DOES IT LOOK LIKE", NEVER "WHICH IS RIGHT". No pairing, no diff, no verdict — two PNGs of the
// same control under the same frozen clock at the same viewport. The judgement is the reader's, which is the point.
//
...
//
// ⚠️ THE TWO SIDES OFTEN NEED DIFFERENT SELECTORS and that is not a workaround — `--triggers` already reports that
// the mockup draws `+ Grant access` where the portal draws `+ Grant access N`. `--sel` sets both; `--mk-sel` and
// `--pt-sel` override one.
import fs from 'fs';
...
// The mockup's own fixture day. `portalDiff` freezes both sides here and `fixtures.js` hardcodes it, so a shot taken
// at any other instant is not comparable to anything else this suite has recorded.
...
// 🔴 TWO ROOTS, NOT ONE. The harness's import map and its asset paths are written against `portal/public` AS THE
// SERVER ROOT — served from the repo root instead, every module 404s and the page renders an empty div, which
// reads exactly like "the portal has no commit chip". The mockup package needs the repo root because its own
// paths are repo-relative. So: one server each.
function serve(root) {
...
async function shoot(browser, port, side, url, selector, clickSel) {
    ...
    // Frozen before any page script runs, and storage cleared — both are settled requirements of this suite, and
    // both were learned the expensive way (a moving clock, and sessionStorage leaking between passes).
    ...
    // 2600ms, the same settle `portalDiff` uses. 1400 was enough for the static mockup and not for the harness,
    // whose realm renders after a stubbed fetch resolves — so the first run reported the portal had no commit
    // chip when it simply had not drawn one yet. A too-short settle reads exactly like an absent element.
    ...
    if (!box) {
        // Name what IS there. "nothing matched" alone cannot distinguish a wrong selector from a page that
        // never rendered, and those need opposite fixes.
...
// The mockup's staged store is sessionStorage and this tool clears it, so a realm carrying a staged surface must be
// seeded or the two sides hold different data — the same refusal `portalDiff` makes, from the same one list.
