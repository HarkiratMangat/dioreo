...
(function(){
  ...
  [].slice.call(document.querySelectorAll('.seg')).forEach(function(seg){
    ...
    var L=0,R=0,tL=0,tR=0,m=0,mTgt=0,raf=0,idx=home===null?0:home;
    ...
    /* Real measured geometry, not 1/n of the track. offsetLeft and an absolutely
       ...
       agree exactly and the pill fits "Contributing" as well as "Notice". */
    function box(i){ var a=tabs[i]; return [a.offsetLeft, a.offsetLeft+a.offsetWidth]; }
    function paint(){
      ink.style.transform='translateX('+L+'px)';
      ink.style.width=Math.max(0,R-L)+'px';
      seg.style.setProperty('--c2',cols[idx]);
      if(home===null){
        ...
        seg.style.setProperty('--c1',cols[idx]);
        seg.style.setProperty('--m',0);
        seg.style.setProperty('--cm',cols[idx]);
      }else{
        seg.style.setProperty('--m',m);
        seg.style.setProperty('--cm',blend(cols[home],cols[idx],m));
      }
    }
    /* The morph. An edge opening the pill moves fast; an edge closing it lags.
       Travelling right, the right edge runs ahead while the left edge trails, so
       the pill elongates into the gap and then catches up behind itself. */
    function frame(){
      var FAST=0.32, SLOW=0.15;
      L+=(tL-L)*(tL<L?FAST:SLOW);
      R+=(tR-R)*(tR>R?FAST:SLOW);
      m+=(mTgt-m)*0.16;
      paint();
      if(Math.abs(tL-L)>0.4||Math.abs(tR-R)>0.4||Math.abs(mTgt-m)>0.004){
        raf=requestAnimationFrame(frame);
      }else{ L=tL; R=tR; m=mTgt; paint(); raf=0; }
    }
    function aim(i,snap){
      idx=i;
      var b=box(i); tL=b[0]; tR=b[1];
      mTgt=home===null?0:(i===home?0:1);
      if(snap||still){ L=tL; R=tR; m=mTgt; paint(); return; }
...
.seg-ink{position:absolute;z-index:0;left:0;top:2px;bottom:2px;width:0;
  border-radius:999px;overflow:hidden;background:var(--paper);
  border:1px solid color-mix(in srgb,var(--cm,var(--c1)) 62%,transparent);
  box-shadow:0 2px 10px -4px rgba(0,0,0,.6),
    0 0 16px -6px color-mix(in srgb,var(--cm,var(--c1)) 75%,transparent);
  will-change:transform,width}
/* The mesh: two soft lobes per colour, cross-faded by --m. Both colours are
   on screen at once mid-hover, which is the whole point — you see the page
   you are leaving bleeding into the one you are about to open. */
