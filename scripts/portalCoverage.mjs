...
function emittedFrom(t) {
    const out = new Set();
    {
        ...
        //
        // ⚠️ BOUNDED BY USE, NOT BY SHAPE. Only identifiers that actually appear inside a `class=` expression are resolved, and only their own declaration is read — so an unrelated `const MESSAGES = ['saved', 'failed']` is never mistaken for a class list. CLASS_RE still filters what survives, and the same pass runs over the mockup, which builds its classes the same way.
        //
        // 🔴 IDENTITY IS READ; TRANSIENT STATE IS NOT. `el.className = 'sub'` names what an element IS — tips.js builds its tooltip as DOM nodes on purpose, because a tip's text is a build name and not ours to trust as markup — so it is markup by another spelling, and it is read. `classList.toggle('is-slow', on)` names what an element is DOING right now, and it is not.
        //
        // ⚠️ AN EARLIER VERSION OF THIS COMMENT STATED A DIFFERENT AND WRONG RULE, and the correction is worth more than the line it replaces. It said toggles are skipped because the mockup is imperative vanilla JS and the portal is declarative Preact, so reading them measures the style rather than the design — argued from THREE examples off the top of a list of about thirty. Enumerating the tail refuted it: of the classes that scan surfaced, `rolling`, `fdelta`, `errmsg`, `mark`, `ptc`, `stack`, `smask` and `nw-date-echo` were EIGHT REAL MISSING SURFACES, every one of them subsequently built. The scan was right; the reasoning that dismissed it was a generalisation from a sample of three. What survives is the narrower line above — which is why a direct assignment is read here and a toggle is not: `is-slow`/`is-refreshing` are emitted by async.js from a returned hostClass, the Track collapses lanes as `lnc`, and `zero`/`stg-clear` encode a rule shell.js deliberately reversed. Those are state, and state is where the two codebases legitimately differ.
...
console.log('   It is NECESSARY, never SUFFICIENT. Use it to find places nobody has looked — never let it');
console.log('   choose the ANSWER: a class name says nothing about whether the surface is any good.');
console.log('');
console.log('   ⚠️  AND DO NOT READ THE MOCKUP AS DEMOTED. A 2026-08-26 audit built a whole thesis on');
console.log('   CLAUDE.md\'s "no longer a build-from specification" line and concluded that matching the');
console.log('   mockup\'s COMPOSITION was itself a mistake. Harkirat, same night, in his own words: "the old');
console.log('   design is essentially retired at this point. the design is the mockup." The line means you');
console.log('   may be AHEAD of the sketchpad in places, not that following it is wrong. What went wrong');
console.log('   that night was dropping a working feature WHILE porting a good idea — Armory\'s two add');
console.log('   chips were right and lost their keyboard shortcut in transit. Port the design; keep what');
console.log('   the portal already does better.');
