// scripts/paletteShape.test.js Regression tests for PALETTE SHAPE in utils/colorExtract.js -- how many swatches come back, and which ones. Two mechanisms decide that and they are tested together because they are one concern:
...
// Added 2026-08-12 14:59 EDT (restrict), extended 2026-08-12 18:43 EDT (ladder). Run: `node scripts/paletteShape.test.js` (also via `npm test`).
//
// ⚠️ WHY THIS EXISTS. Every failure mode here is SILENT, and worse, every one of them produces output that looks entirely reasonable on its own:
...
let passed = 0;
const pending = [];
// Counted, never hardcoded -- a literal total in the summary is a copy of state that rots the moment a case is added.
function t(name, fn) {
    pending.push(async () => {
        try {
            await fn();
...
// --- Synthetic images. getColorPalette duck-types an already-decoded image on `.bitmap.data`, so a plain object is enough and no file, network or ffmpeg is involved -- the test stays fast and has no dependency on any particular picture existing.
...
// A pure greyscale ramp: eight genuinely distinct lightnesses, zero chroma anywhere. This is exactly what the restrict exists for -- eight swatches here would be eight shades of nothing.
...
// The same structure, tinted brown. ⚠️ THE TINT IS CALIBRATED, NOT EYEBALLED: its most chromatic pixels measure 0.037, which lands it inside the band the real corpus's misjudged images occupy (0.0213 to 0.0395) -- below CHROMA_FLOOR (0.04), so the old gate called it colourless, and well above LOW_COLOUR_CHROMA (0.01), so the new one does not. A stronger tint was tried first and was USELESS as a fixture: at +18/-14 it cleared 0.04 outright, so the case passed under the old gate too and proved nothing. Anything that changes these numbers should re-check that band.
...
t('a MUTED but genuinely tinted image is NOT restricted', async () => {
    // The whole point of giving the gate its own floor. Every centroid here is under CHROMA_FLOOR, so the old candidate-chroma test called this colourless and threw away most of the palette.
...
t('the gate floor is well below the accent pool floor', () => {
    // If someone "tidies" these back into one constant, the sepia case above regresses and the only symptom is a shorter palette. Naming the relationship is what makes that a test failure. ⚠️ Read off the real export now that there is one (2026-08-12 20:10 EDT). It used to be a hardcoded local copy of 0.04, which is duplicated state: the shipped constant could move and this assertion would keep passing against the old value it was meant to be checking.
...
t('the restrict decision is identical at every requested count', async () => {
    // The boundary flip this replaced: the old gate read the EXTRACTED centroids, which change with `count`, so one image could be colourful at 6 and colourless at 8. The new gate reads the pixel tail, which does not.
    //
    // ⚠️ Read off the counts ABOVE LOW_COLOUR_COUNT only. At count === LOW_COLOUR_COUNT the restrict is a no-op by construction (Math.min), so "did the length drop?" answers a different question there and reports a flip that did not happen -- which is exactly what the first draft of this assertion did.
...
t('nameplate/decoration counts are never RAISED by the restrict', async () => {
    // Math.min in the restrict is what guarantees this; the sources that already ask for 4 must not come back with more just because they happen to be colourful.
...
//
// The property has two halves and they are checked in the two different places they can break. `spanLightness` can only ever hand back things it was given, so the selection itself is safe by construction and asserted as such. What is NOT safe by construction is the SIZE OF THE POOL it is handed: clustering over-asks by 50%, so `ordered` normally runs longer than `count`, and passing it whole lets a colour the full palette would never show appear in the restricted one. That was a real defect in the first draft, and it is invisible from the outside -- the palette simply contains a plausible extra grey. Pinned by reading the call site, because there is no output to read it from.
...
t('spanLightness spans the axis instead of taking the head', () => {
    // A ramp whose first four entries are all bunched together: a plain slice returns four neighbours, which is exactly the failure this replaces. The extremes must come back instead.
...
t('spanLightness is deterministic, including on ties', () => {
    // Determinism is the Refresh Colors button's contract -- without it that button reports "found new colors" on every click. Equidistant candidates must resolve the same way every time.
...
t('a restricted greyscale palette actually spans lightness', async () => {
    // The end-to-end version of the two cases above: the shipped path, on the image the restrict is for, must not come back as four neighbouring greys.
    const p = await getColorPalette(greyscale(), 8);
...
//
// ⚠️ Same silent-failure argument as above, with one addition specific to this half: a palette that has been laddered looks EXACTLY like one that came out that length naturally. A broken rung map, a derived swatch that duplicates a real one, or a ladder that wrongly fires on a nameplate all produce output that is completely unremarkable to look at.
...
t('off-rung lengths move to the agreed rung', () => {
    // 3 -> 4, 5 -> 4, 7 -> 8. Hardcoded on purpose: this is the agreed mapping, and a test that recomputed it from LADDER would pass no matter what LADDER said.
...
t('going DOWN keeps salience order and drops the tail', () => {
    // ⚠️ REGRESSION TEST. The first version used spanLightness here and it silently discarded small vivid accents: measured 2026-08-12 18:40 EDT, a 2% pure red on a beige/brown ground was dropped outright for sitting near the browns in LIGHTNESS -- the exact colour the accent pool exists to surface. Lightness coverage is right for a colourless image and wrong for a colourful one, and a colourless image never reaches the ladder (the restrict has already put it on a rung).
...
t('the derived entry is never a near-duplicate of a real one', () => {
    // A palette already spanning the axis evenly has no gap wide enough to hold a distinguishable colour. Returning null -- three honest swatches -- beats returning a fourth that is a twin.
...
t('the ladder is deterministic', () => {
    // Refresh Colors compares before and after; a ladder that picked differently between runs would report "found new colors" on every click.
...
t('nameplate and decoration are a CEILING, never laddered', async () => {
    // Harkirat 2026-08-12 17:38 EDT: those palettes are a deterministic composition (1 bed + 3 art) and padding one would invent a colour for a design that simply has fewer. The gate is count === KMEANS_COUNT, so a 4-asking caller must come back untouched.
    const p = await getColorPalette(sepia(), PALETTE_COUNTS.nameplate);
...
//
// ⚠️ Same silent-failure argument again, and this one hid the longest: the palette that was WRONG came back full, on-rung, perceptually deduped and passing every check in this file. Harkirat found it by looking at a picture of his own avatar and noticing the sunglasses were not in it.
//
// ⚠️ AND THE TESTS HERE ASSERT MEMBERSHIP, NEVER SHAPE. A conservation check over counts cannot see a SUBSTITUTION -- the 401-image run reported a clean distribution all the way through the accent- deleting regression this session shipped and caught. Every case below names a hex or an object.
console.log('\nthe lightness-range reservation — "the darkest thing in the picture" reaches a slot\n');

// Eight close taupe/brown ramp tones, plus a ninth entry that is the variable under test. This is the reproduction from the live bug, reduced: the real avatar's own palette topped out at #26221C and the black behind it was cut by the slice.
const RAMP = [
...
t('a 1.5% BLACK among eight ramp tones reaches the palette', async () => {
    // The bug, end to end. Before the reservation this returned eight swatches whose darkest was #26221C, and the black sat at rank 9 of 9 where the slice to 8 cut it.
...
t('the 2% pure RED control is untouched, and still tagged `accent`', async () => {
    // The conservation case, at the smallest scale that can show it: the reservation must not pay for range with the very colours the accent pool exists to surface. Measured over 401 corpus images as well -- 0 accent swatches lost at every threshold tried -- but that number lives in the rules file, and this is the case that FAILS if the victim rule is ever loosened.
...
// --- The three price rules, each of which was bought with a measurement and each of which is invisible from the outside if it regresses.
...
t('the reservation never sells an ACCENT', () => {
    ...
    //
    // ⚠️ THE ACCENTS HERE ARE DELIBERATELY DULL, and that is what makes this case test anything. A vivid accent is already protected by the colour rule below, so a fixture built from one passes whether or not this guard exists -- verified by deleting the guard and watching it stay green. A low-chroma accent is not contrived either: mergePerceptual keeps the `accent` tag when a pool entry folds onto a duller centroid, so this is a shape the real extractor produces.
...
t('the reservation never sells the OTHER extreme', () => {
    // Without this the light pass sold the black the dark pass had just bought (Starmancer.gif went out #000000, in #C4F6FF) -- an entry cannot be worth reserving and worth spending in one palette.
    //
    // ⚠️ THE DARK MUST BE LAST IN SALIENCE ORDER for this case to test anything: the victim search runs from the END of the kept list, so a fixture that puts anything duller after the dark never reaches it and passes with the guard deleted -- verified by deleting it and watching it stay green. Last is also where the real one sits, since a small black is exactly what ranks lowest.
...
t('an extreme within LIGHTNESS_GAP of a kept entry is NOT reserved', () => {
    // The threshold is what stops this firing on a near-duplicate. MERGE_DELTA_E is where this module stops calling two entries the same colour, so the gap must clear it with headroom.
...
t('the reservation can only return entries it was given, and never grows the palette', () => {
    // Same subset property the restrict carries: buying range must not invent a colour, and must not quietly hand back a ninth swatch that the ladder would then read as off-rung.
...
t('a near-colourless image cannot normalise its own noise to a full accent bonus', () => {
    // `chroma/maxChroma` is RELATIVE. On the reproduction above maxChroma is 0.0276 -- a nearly-neutral taupe -- so before the floor that taupe scored the identical 1.0000 chroma term a PURE RED scores, and every mid-grey was paid the accent premium for chroma that is not there.
...
t('on a colourless ramp, prevalence decides the order again', async () => {
    // The behavioural half of the same claim. These two entries differ by 1 point of share and by rounding-level chroma; the unfloored denominator inverted them. Read off the returned order.
    const p = await getColorPalette(ramp([[6, 6, 6], 1.5]), 8);
