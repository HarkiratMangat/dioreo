...
const makeFixture = () => {
  ...
  write(
    ...
    "# Map\n\n| File | What |\n|---|---|\n| `CHANGELOG.md` | log |\n| `CHANGELOG-SUMMARY.md` | summary |\n" +
      ...
      // Named as a DIRECTORY (readme-map's coverage unit): the fixture carries a plans/ file so plan-audit-log examines something, and without this row readme-map fires on the BASELINE.
  ...
  // A post-convention plan, so plan-audit-log examines something on the baseline instead of scoring a vacuous pass. The date must be >= PLAN_AUDIT_FROM in docs-audit.mjs or the check skips it.
