...
//
// 🔴 IT RUNS BEFORE THE WORK, NOT AS A VICTORY LAP AFTER IT. The mockup and the harness are BOTH
// fixture-driven, so they agree with each other automatically and everything they corroborate about
// data shape, they corroborate vacuously. `TL.days` returned NaN for every real record — "ends NaN
// days after the battle pass" — and no fixture could ever have shown it.
//
// ⚠️ THIS IS NOT A CONFORMANCE CHECK. The real portal carries no `?conform=1`, so every stand-down is
// ON and a pixel comparison against the mockup measures the redesigns rather than the gap. What only
// this can see: real data volumes, the door, genuine empty and error states, the 401/409 paths, and
...
const views = flag('--views', 'Track,Board,Repairs').split(',').filter(Boolean);
const noAuth = has('--no-auth');           // the falsifier: proves the door assertion can fire

// What a rendered page must never say to a reader. `null` and `undefined` are included as WORDS
// because that is how they reach a screen — a template that interpolated a missing field.
...
try {
    ...
    // 🔴 IDENTICAL READINGS ACROSS VIEWS THAT MUST DIFFER IS THE SIGNATURE OF NEVER HAVING ARRIVED,
    // and it is far more convincing than zeroes because the numbers look plausible. This is the check
    // that would have caught the 784/0/784/0/784/0 reading without anyone noticing the lone 401.
    ...
    // ⚠️ AND THE CONSOLE MESSAGE DOES NOT NAME THE URL. Chrome's subresource failure reads exactly
    // "Failed to load resource: the server responded with a status of 404 (Not Found)" — no path — so
    // filtering console text on the word favicon cannot work, and the first attempt at this exemption
    // silently did nothing. Those messages are always a MIRROR of a network response, and the
    // responses are checked below with their URLs intact, where the exemption can be exact. What
    // survives here is what only the console can report: thrown exceptions and explicit console.error.
    ...
    // A MISSING FAVICON IS NOT A DEFECT IN THE REALM, and it arrives twice — once as a >=400 response
    // and once as the console error that response generates. The first version exempted only the
    // response, so the walk failed on a 404 for /favicon.ico while reporting three clean views. A gate
    // that fires on something nobody will fix is a gate that gets ignored, which is the third time
    // today a false positive nearly cost more than the defect it guarded.
    console.log('');
} catch (e) {
