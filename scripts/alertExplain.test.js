// scripts/alertExplain.test.js
// Regression test for utils/alertExplain.js — the plain-language layer added 2026-08-06 14:58 EDT after
// a live `🔴 Gateway shard error` alert that Harkirat had "absolutely no clue" how to read.
// Run: `node scripts/alertExplain.test.js` (also runs via `npm test`, which CI calls).
//
...
// 1/min per `${level}:${title}`, so a second 'Gateway disconnected' with a DIFFERENT close code is
// silently dropped: the transient-vs-fatal branch, the single most valuable distinction in this module,
// cannot be exercised that way at all. It looked covered and was not. Calling explain() directly is the
// only way those branches get proven.
...
t('a shard error states the bot kept running', () => {
    // The reason this line matters: a red 🔴 alert reads as "the bot is down" when it usually is not,
    // and that misread is what turns a nothing-event into a panic at 03:00.
...
t('every fatal close code produces a No', () => {
    // Guards the table itself: adding a code to FATAL_CLOSE_CODES without it flowing through the
    // fatal branch would silently reintroduce "probably transient" for a permanent outage.
...
t('an unknown title returns null so the old layout is kept', () => {
    // This is the safety property of the whole feature: explain() is additive. If it ever started
    // returning a stub object for unknown titles, sendAlert would move the technical detail into a
    // field and print an empty explanation above it — strictly worse than before the layer existed.
...
t('every alert name quoted in an explanation is a real DISPLAY title', () => {
    // A draft pointed the reader at "Discord connection lost" while the alert still displayed as
    // "Gateway disconnected" — telling someone to look for a message that does not exist under that
    // name is the exact failure this module exists to remove. This asserts it structurally instead of
    // trusting a proofread.
...
t('no display title is mapped to itself', () => {
    // A self-mapping is a line that has to be kept in sync and buys nothing; the fallthrough in
    // displayTitle() already handles it.
...
t('every explanation answers all three questions, non-empty', () => {
    ...
    for (const title of titles) {
        ...
        // ⚠️ NO MINIMUM LENGTH on `action`. The first version of this test demanded >10 chars for all
        // three fields and failed on `action: 'Nothing.'` — which is the RIGHT answer for a recovery
        // alert, and the one a reader most wants to see. The assertion was measuring verbosity and
        // calling it usefulness. `what` and `selfFix` do have to explain something; `action` is allowed
        ...
        // ...and it may end in `?` — the database action legitimately ends "is the VM's IP still
        // allow-listed?". This assertion has now been wrong twice in the same direction (first
        // demanding length, then demanding a full stop), which is a standing warning about this kind
        // of test: it is easy to encode a style preference and read the failure as a content bug.
...
t('explanations stay within Discord\'s 1024-char field cap', () => {
    // selfFix and action render as embed FIELDS, capped at 1024 by Discord. sendAlert slices as a
    // backstop, but a silently truncated sentence is a bad answer — keep them genuinely short.
    const titles = [...Object.keys(DISPLAY_TITLE), 'Back online', 'Reconnecting to Discord'];
    for (const title of titles) {
