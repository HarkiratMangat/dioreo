...
//
// 🔴 THE INSTRUMENT EVERYTHING ELSE WAS VERIFIED THROUGH HAD NOTHING VERIFYING IT. portal/ui/harness/
// stubs fetchJson so the real components render with no Mongo and no OAuth, and six realms' design
// work was checked against it. If a stub's payload drifts from what portal/api/ actually returns,
// the harness keeps rendering the old shape happily and every judgement made in front of it is made
// against a lie — the page looks finished and is measuring a fiction. That already happened once
// today in miniature: /api/analytics was stubbed with fixture arrays under the API's key names, and
// the dashboard rendered a complete-looking page reading "no boot recorded / 0 alerts / 0 users" on
// a fixture set holding 303 boots and 998 alerts.
//
// This EXECUTES the stub rather than pattern-matching it. Two regexes agreeing proves the regexes
// agree; calling the thing and inspecting what comes back proves the shape.
...
// What each route PROMISES: the top-level keys of its 200 payload. These are written as shorthand
// object literals throughout portal/api/, so the keys are plain identifiers.
...
// Run the stub the way the browser does: window + FIX, then evaluate its module body with the two
// import lines removed (it imports nothing it uses at module scope).
function loadStub() {
    global.window = global;
    global.document = { documentElement: { dataset: {} } };
    global.location = { search: '' };
    require('../docs/superpowers/mockups/2026-08-23-portal-interactive/assets/fixtures.js');
    // The stub narrates every call for a human at a browser console; in a test run that is noise
    // between the lines that matter.
...
// THE GATE'S OWN FALSIFIER. A payload-key scan that matched nothing would compare {} against {} and
// pass forever, which is indistinguishable from every route agreeing.
...
// 🔴 AWAITED, AND THAT IS NOT A DETAIL. The first version passed an `async` function to a
// synchronous check(), so every assertion inside became an unhandled rejection that nothing counted
// — the suite printed a tick for /api/changeset while the stub was actually answering it with
// {ok:true} and the route promises {changesets}. A vacuous pass that LOOKS like a green line is the
// worst failure mode a gate has, and this file caught it in itself twice: once on the route scan
// finding zero routes, once here.
