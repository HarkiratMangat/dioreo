// scripts/dynamicPool.test.js
// Regression test for the Dynamic Profile Colors pool and the retired accent styles in
// utils/accentColor.js. Added 2026-08-13 00:02 EDT.
// Run: `node scripts/dynamicPool.test.js` (also via `npm test`).
//
// ⚠️ WHY THIS EXISTS. The pool draws STORED SWATCHES first and falls back to a per-source accent
// colour only where none are saved — and the fallback is the entire reason this design was chosen over
// a straight swap. `*Palette` fields are written ONLY by the View Colors panel, so a user who has never
// opened `/colors` has none: measured on prod at the time of writing, 12 of 18 preference documents
// (67%) had zero stored palettes. Drop the fallback and Dynamic Profile Colors silently becomes "use
// the preset" for most people — an option that appears to do nothing, with no error anywhere.
//
// ⚠️ AND THE RETIRED STYLES ARE A SILENT FAILURE OF THEIR OWN. 'banner' and 'displayName' can no longer
// be selected, but `accentColorStyle` is a plain String with no enum, so nothing stops an old value
// existing. Unnormalized, it would resolve against a source the UI can no longer pick.
...
// --- paletteFields is the shared field-name rule. accentColor.js and colorPalette.js BOTH depend on
// it now, which is why it lives in colorExtract (neither can import the other without a cycle).
...
t('there is exactly ONE definition of the palette field-name rule', () => {
    // ⚠️ Two hand-rolled copies of `guild${Kind}Palette` would drift silently, and a wrong field name
    // reads as "no swatches saved" rather than as an error — the pool would just quietly shrink. Read
    // from source because there is no output that can show it.
    const fs = require('fs');
...
// --- The pool itself. buildDynamicColorPool needs a live interaction, so the swatch-reading half is
// exercised through the REAL exported helper against a fake prefs document.
//
// ⚠️ THESE CASES USED A LOCAL COPY OF THE EXPRESSION AT FIRST, AND WERE THEREFORE VACUOUS. Deleting
// the guild half of the read and deleting the malformed-entry guard both left the suite green,
// because the suite was testing its own re-implementation. `storedSwatchesFor` was extracted out of
// an inline closure specifically so this file can reach it. **Never re-inline it.**
const swatchesOf = storedSwatchesFor;

t('swatches are read from BOTH the global and the server field pair', () => {
    // Harkirat's spec: "from both global & server profiles". Reading only one pair halves the pool and
    // nothing reports it.
    const prefs = {
...
t('malformed palette entries are dropped, never pushed as undefined', () => {
    // A palette restored from a cache written before the wire format gained its flags can hold entries
    // without a hex. An undefined in the pool becomes an undefined accent colour on a real embed.
...
t('the pool is de-duplicated', () => {
    // The same colour in both the global and server palette of one source would otherwise weight the
    // random pick toward it for no reason a user could see.
...
t('display name colors are NOT a pool source', () => {
    // Excluded on purpose: they are two exact values picked in Discord's own UI, not colours extracted
    // from an image, and the style that surfaced them was retired in the same change.
    const fs = require('fs');
    const path = require('path');
