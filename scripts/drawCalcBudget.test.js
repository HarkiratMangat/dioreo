// scripts/drawCalcBudget.test.js Regression test for /draw calculator's two panels' COMPONENT BUDGET. Modeled on scripts/colorPanelBudget.test.js (read first, per the plan -- same shape, same reasoning). Run: `node scripts/drawCalcBudget.test.js` (also via `npm test`).
//
// ⚠️ WHY THIS EXISTS. Components V2 allows 40 components per message, COUNTED RECURSIVELY. Exceeding it is a hard COMPONENT_MAX_TOTAL_COMPONENTS_EXCEEDED send failure -- a real production crash this repo has already taken once (see .claude/rules/rendering-and-ui.md). The design spec sizes a single combined panel at ~31 components with "almost no headroom on precisely the section most likely to grow", which is exactly why this is a two-stage panel and exactly why the worst case needs asserting rather than remembering.
...
// Worst case for Stage A: a mythic draw (upgrade toggle renders) with a live 2X event (entitlement select renders too) and the upgrade actually included.
...
// Worst case for Stage B: a mythic draw, upgrade included, ALL SIX 2X entitlements available (so both the cheapest and least-waste passes have real work to do and are likely to differ), and a client present so the mentionCommand footer line renders too.
...
t('cheapest and least-waste CAN genuinely differ (so the panel branch that renders both is real, not dead code)', () => {
    // 11000 is one of the shortfalls scripts/cpPackages.test.js's own "least-waste never overshoots more than cheapest" check is pinned against -- reusing a known-differing value here rather than assuming the specific worst-case draw scenario produces one (it happens not to: 11,510 CP with all six entitlements available lands both passes on the identical combo).
