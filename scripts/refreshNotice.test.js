// scripts/refreshNotice.test.js Regression test for buildRefreshNotice in utils/colorPaletteView.js -- the ephemeral follow-up the "Refresh Colors" button sends. Added 2026-08-12 21:52 EDT, rewritten 23:47 EDT for Harkirat's spec. Run: `node scripts/refreshNotice.test.js` (also via `npm test`).
//
// ⚠️ WHY THIS EXISTS. This message has three states, two per-profile lists and a decision about which list the PRESSED source belongs in — and every combination is invisible until somebody presses the button while their account happens to be in exactly that state. It lived inline in handlers/colors.js's refresh handler, where none of it could be exercised without a live interaction, a Mongo document and a Discord round trip. It was extracted specifically so the branches could be read off as a function of their inputs.
//
// ⚠️ AND THE FAILURES ARE ALL COSMETIC-LOOKING, which is why they survive. A missing "Server Profile" row does not error; it just means a user is told nothing happened to colours that did change, and goes looking for a button to press — the exact complaint that produced this rework (Harkirat 2026-08-12 21:39 EDT: "it's just unintuitive to make the user press it for global colors and then switch to server colors and press it again").
//
// ⚠️ THE COPY IS HARKIRAT'S, SPECIFIED TO THE CHARACTER 2026-08-12 23:41 EDT. These cases pin the SHAPE, not his exact sentences, except where a specific phrase was the point of a revision — those are asserted verbatim so a well-meaning rewording fails here instead of shipping.
...
// Labels and emoji are read from their real sources rather than written out, so a rename does not silently make these assertions test nothing.
...
t('any refresh at all reports "New colors found", not a per-source verdict', () => {
    // ⚠️ THE HEADLINE IS ABOUT THE PRESS. Reporting it per-source is what made the old message insist nothing had happened while it had quietly updated three other pages.
...
t('the PRESSED source lands in the profile it actually resolved from', () => {
    // Not the view the panel is in: a source with no server override resolves to the global image even in the server view. Getting this wrong files a global refresh under "Server Profile".
...
t('the pressed source is listed ONLY when it actually changed', () => {
    // It gets refreshed unconditionally (that is what the button targets), so listing it regardless would claim new colours for a source that produced the same ones.
    const m = notice({ changed: false, refreshed: [{ kind: 'banner', isGuild: false }] });
...
t('the reassurance line closes the refreshed state', () => {
    // Unconditional by design: the doubt that started this rework was "did it do the others?", so the message answers it every time rather than only when it can enumerate a leftover.
...
t('the three states are mutually exclusive', () => {
    // A refresh outranks an accent clear, which outranks nothing. Two headlines in one message would mean the state machine has a hole.
...
t('sources are labelled, never printed as raw keys', () => {
    // `decoration` renders as "Deco"; a raw key leaking through is the tell that SOURCE_META was bypassed.
...
t('an unknown source degrades to its key instead of throwing', () => {
    // handlers/colors.js passes whatever the custom_id carried; a message that throws would take the whole follow-up with it after the panel has already been edited.
...
t('every emoji in the notice is read at RENDER time, never frozen at require()', () => {
    // utils/emojiMap.js rewrites its own values at boot to whichever application is running, so a value captured when this module loaded would render a PROD id on the dev bot forever. Proven by mutating the map and re-rendering. See [[project_emoji_require_time_capture]].
