...
//
// 🔴 WHY THIS EXISTS. Harkirat decided 2026-08-30 22:45 EDT that the portal's two rendering modes collapse into
// one: the "ahead" rendering is scrapped, the conformed rendering survives, and redesigns get rebuilt fresh on it.
// The conformed side is photographed everywhere — portalDiff writes `pt-` on every run and an A/B artifact is
// published per Part. The AHEAD side is photographed NOWHERE. `?conform=1` is set only by the harness reading its
// own query string (portal/ui/harness/stub.js), so the moment the 57 `conforming()` sites and 51 `data-conform`
// blocks are deleted, the only record of what those 108 forks were protecting is prose plus git archaeology.
//
...
// `app.css:1372` is Armory's tier board; `app.css:2940` is a 78ch measure spanning six selectors. A session working
// Season would naturally shoot Season and silently lose five realms' only record.
//
// USAGE
...
//
// 🔴 THE FALSIFIER, and it is the trap table's own. §0.10: "IDENTICAL READINGS ACROSS VARIANTS THAT MUST DIFFER =
// NEVER ARRIVED" — an unauthenticated walk once reported the same reading on three views and that read as a stable
// measurement when it was the signature of measuring the door. A capture pair whose two sides are identical looks
// exactly like "this realm has no stand-downs" and is indistinguishable from "the query parameter never arrived".
// So the arrival of the flag is asserted DIRECTLY, on the root element's `data-conform` attribute, independent of
// any pixel: present in ON, absent in OFF, on every single page. A run where that assertion fails writes nothing.
// With arrival proven, identical pixels become a real finding about the realm rather than an instrument failure.
...
// Same instant portalDiff pins to, and for the same reason: ?today= does not travel the season countdown
// (COMPANION 16.31a), so two captures seconds apart can never be compared. The value must stay at the mockup's
// own F.today or the two sides desynchronise — see the plan's 0.6a.
const FROZEN = Date.parse('2026-08-24T18:41:00Z');

// Season's Board and Repairs live behind tabs and carry conform blocks of their own (.bcols at app.css:3073 is
// Board's). A capture of the default view alone would leave them unrecorded.
const SURFACES = [
...
async function shoot(page, { realm, view }, on) {
    ...
    // The mockup persists five UI keys and one page is reused across every shot here; without this a toggle
    // opened in shot 1 silently changes what shot 12 measures. Same reason portalDiff clears them.
    ...
    // The harness nests two `main` elements — its page wrapper and the Shell's — so querySelector returns the
    // outer one whose scrollHeight is the viewport. Take the largest content height of every candidate; a max
    // cannot be fooled by a wrapper. Same defect portalDiff records, and it clipped every capture to 888px twice.
