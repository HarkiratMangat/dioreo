// scripts/createPlaceholderLoadouts.js One-off/re-runnable seeder for weapons that already have a badge assignment (see scripts/applyBadgesBulk.js's "unmatched" report) but no actual loadout data yet -- Harkirat wants these weapons live and searchable in the bot NOW (name + badges visible via /gunsmiths search/list and autocomplete) even before attachments/Gunsmith code/a real image exist for them. Creates exactly one placeholder build per weapon (imageKey/attachments/buildName are all required schema fields -- see models/Loadout.js -- so there's no way to have a loadout with truly empty content; a single clearly-labeled "Coming Soon" build is the least-special-cased way to satisfy that while still rendering something honest). Safe to re-run: skips any weaponKey that already has a doc for that mode instead of creating a duplicate.
...
// weaponName kept in the same ALL-CAPS style as every other MP loadout in the DB (an artifact of the original builds.xlsx migration, see CLAUDE.md's MP loadout system section) for consistency.
