...
//
// 🔴 WHY IT EXISTS. `portal:sweep` printed seven percentages and nothing else, and the percentage is the metric §0.7d retired — it is a pointer to code worth reading, never a score. But the sweep is the FIRST artifact of a session, so a number standing alone at the top of the log is read as a grade no matter what the plan says. A realm closes on the ENUMERATION: are its regions exactly the cited set. This prints the other half of that sentence, so the two arrive together and neither can be read as the whole answer.
//
// ⚠️ IT COUNTS ROWS, NOT DIFFERENCES, AND THE TWO ARE NOT THE SAME NUMBER. One ledger row can cover several regions (Season's `+16px` cascade offsets the whole page) and several rows can cover one. So the shortfall printed beside it is a PROMPT — "this many regions have no row naming them" — and not a defect count. Saying so in the output is the point: an instrument that does not state its coverage lets every reader assume it covers everything.
import fs from 'fs';
