...
| **`reference/`** | On-demand reference docs: `deployment-and-ops.md` (stack, GCP VM/systemd/alerting, version tagging, **the local dev bot** — `Dioreo (Dev)`, `.env.dev`, local Mongo, `--watch`, emoji/data cloning; added 2026-07-26 13:45 EDT), `platform-constraints.md` (accepted **platform** limits — Components V2, the Discord client, system dependencies; renamed from `known-issues.md` 2026-08-06 08:15 EDT once the real open defects were split out to `db-deferred-list.md`, since most of what it held were facts rather than bugs — and it carries a header warning that an entry is evidence, not a verdict, and must be re-tested before being cited), `design-history.md` (redesigns that already happened), `commit-and-branch-naming.md` (the Conventional Commits subject format + branch/PR-title convention). Read when ops/history detail is needed, before designing one of the surfaces, or before writing any commit subject or branch name. ⚠️ **The unifying idea is "look this up to do the thing correctly" — a legend, not a notebook.** `design-history.md` sits here awkwardly by that test and is queued to fold into `DEVLOG.md` (see `db-deferred-list.md`). | When ops setup or a flagged issue changes. | Claude |
