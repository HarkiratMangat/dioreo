...
| **`reference/`** | On-demand reference docs: `deployment-and-ops.md` (stack, GCP VM/systemd/alerting, version tagging, **the local dev bot** — `Dioreo (Dev)`, `.env.dev`, local Mongo, `--watch`, emoji/data cloning; added 2026-07-26 13:45 EDT), `known-issues.md`, `design-history.md` (redesigns that already happened), `design-ideas.md` (the forward-looking counterpart — ideas rejected on TIMING rather than merit, each with the condition that would make it right; added 2026-08-02 22:53 EDT), `commit-and-branch-naming.md` (the Conventional Commits subject format + branch/PR-title convention). Read when ops/history detail is needed, before designing one of the surfaces, or before writing any commit subject or branch name. | When ops setup or a flagged issue changes. | Claude |
