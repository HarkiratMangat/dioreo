...
If you administer a Discord server, you can use `/server` to decide where the Bot answers publicly and where it answers only to the person who asked. Those choices are stored in a separate `GuildSettings` record keyed to the **server's** ID, not to yours, and **a record is only created once an admin actually saves a rule.**
...
Neither record is linked to your `UserPreference` record or used to personalise anything — the allowlist entry only exists for someone the owner has explicitly granted access to, and the announcement record's only personal data is who wrote it. You can read both schemas yourself: [`models/AdminUser.js`](../../models/AdminUser.js), [`models/Announcement.js`](../../models/Announcement.js).
...
We also keep a small **performance-timing log** (`RenderTiming`) while the "View Colors" feature is under active development: how long a colour-panel render or an animated preview took, which page was involved, and your Discord user ID and server ID so a slow render can be traced back to a specific case while we investigate it. It holds no colours, no images, and nothing about what you look like — only timing numbers and which button was pressed. Unlike the alert log above, this one is **not deleted automatically** — it is a small, deliberately-kept diagnostic record for an active investigation, not routine operational noise, and we will prune it by hand once that work is done.
...
| **MongoDB Atlas** (MongoDB, Inc.) | **Stores your preference record, the per-server settings described in §2.1a, plus the operational alert log described in §2.4** | **🇨🇦 Azure Canada Central (Toronto)** |
