...
| **MongoDB Atlas** (MongoDB, Inc.) | **Stores your preference record, the per-server settings described in §2.1a, the operational alert log described in §2.4, the interaction records described in §2.4b, and the admin allowlist / announcement / change log records described in §2.1b** | **🇨🇦 Azure Canada Central (Toronto)** |
