...
|---|---|---|---|
| **Some call sites are not linked, and the trigger is not isolated.** `mentionCommand` is linked from 1 of the 10 files that call it; `fetchWithTimeout` from none | graph | Retracted 2026-09-14 23:35 EDT: the earlier row blamed the slash-string argument. The tool's source (`detect_url_in_args` in `pass_parallel.c`) adds its fake Route edge AFTER the normal `CALLS` edge is emitted, so it cannot remove a caller; the same real files (`commandMentions.js`, `invite.js`, `calendar.js`, `help.js`) in a 4-file repo and a 64-file repo link all 7 callers; a from-scratch rebuild of this repo's index changes nothing. Not yet tried: bisecting the full repo by directory | `search_code` |
| **A call with a slash- or URL-shaped string argument also mints a fake `Route`**, via `HTTP_CALLS` | graph | **135 of 200 `HTTP_CALLS` edges** here are not portal routes: Discord slash commands, regex literals (`/503/`), `/bin/sh`, `mongodb://` URIs, test titles, Markdown prose. Source-confirmed; already reported upstream as #1248 and #598 | nothing; `get_architecture`'s routes list and `cross_service` traces are unreliable here |
...
`check_index_coverage` reports `no_recorded_issue` for every file involved, because nothing failed to parse: no parse error is recorded for an unlinked call. ⚠️ The fake-route census is source-confirmed; the unlinked-caller cause is unknown; the callback and module-object causes rest on one or two examples each, so no rate is claimed for them.
