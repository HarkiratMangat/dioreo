...
|---|---|---|
| Where is X defined | `search_graph` with `query: "X"` | **24 of 24** real definitions first, ~250 B per answer against up to 8,171 B for `rg -w`. A bare `name_pattern` also returns every destructuring import site as a `Variable` row, which is why 2026-09-08 scored ties. The 25th sample, `parseCol`, is not a JavaScript function at all: it is browser script inside the `MORPH_JS` template string in `scripts/buildLegalPages.js`, so the graph was right and the grep 'definition' was text in a string |
| Who calls X | `trace_path` with `include_tests: true`, then `search_code` for `"X("` | `sendV2Payload`: 35 callers including both test files. `mentionCommand`: `trace_path` links 2 callers in 1 of 10 calling files; `search_code` names all 10 containing functions. The unlinked calls sit in the same functions whose other calls resolve (`calendar.buildContainer` has 8 linked callees), and `check_index_coverage` reports `no_recorded_issue` for those files. **Cause found**: every unlinked call passes a slash-shaped string (`mentionCommand(client, '/settings')`), and each is filed as an HTTP call instead; see *Why the graph and grep disagree* |
...
**Why the graph and grep disagree** (2026-09-14 23:17 EDT). Each cause was confirmed on the graph itself, and the grep side was checked for its own errors:

| Cause | Side | Evidence | What sees it |
|---|---|---|---|
| **A call that passes a slash- or URL-shaped string becomes an HTTP call.** The cross-service linker files `fn(x, '/settings')` as `HTTP_CALLS` to a `Route` named after the string, and no `CALLS` edge is written | graph | every one of `mentionCommand`'s 13 unlinked call sites has an `HTTP_CALLS` edge to a slash-named Route from the exact function `search_code` names; its 2 linked calls pass a variable. **135 of the 200 `HTTP_CALLS` edges** here are not portal routes: Discord slash commands, regex literals (`/503/`, `/yes/i`), `/bin/sh`, `mongodb://` URIs, test titles, even `-# 🔸 **/autobuild** url:https://…`. `fetchWithTimeout`, called with GCP metadata URLs, shows 0 callers. Reproduced on a three-file repo (2026-09-14 23:20 EDT): with no HTTP server in the project the call links normally, but add one unrelated `app.get('/other', …)` and `mention(client, '/settings')` becomes a call to `__route__ANY__/settings` (and `'/help'` to `__route__ANY__/help`). Diors-Builds has the portal's HTTP server, so every slash-string argument in the bot code is exposed | `search_code`. And `get_architecture`'s routes list and `cross_service` traces carry these fake Routes |
| **Cross-file callers attach to the importing file's destructured `Variable` node**, not to the `Function` | graph model | 42 of 50 sampled no-caller functions have linked calls onto a same-named import `Variable`. `trace_path` resolves through it by name; a degree or `NOT EXISTS` query on the `Function` node cannot | `trace_path`, `search_code` |
| **A function passed as a value gets no edge**: `.map(normAtt)`, `.sort(byHueDistance)`, `process.on('SIGINT', releaseLock)`, `run: openFormatGuide` | graph | only 16 `CALL_REFERENCE` edges in the whole repo; `trace_path(normAtt)` returns 0 | `search_code` for the bare name |
| **A call through a module object links to nothing**: `logger.noteHotpatch()`, `require('./logger').currentCommit()` | graph | only `DEFINES` edges reach either; `trace_path(noteHotpatch)` returns 0 | `search_code` |
| **Test files are excluded by default** | graph default | `include_tests: true` returns them | `trace_path` with the flag |
| **A name inside a string, a comment or a test message counted as a use**, and **browser JS inside a template string counted as a definition** | **grep (mine)** | `clamped` counted as used because a test message says "was not clamped"; three `buildSyntheticInteraction` "misses" were comments; `parseCol` lives in `MORPH_JS` | reading the hit, or the graph |

`check_index_coverage` reports `no_recorded_issue` for every file involved, because nothing failed to parse: the calls were misfiled, not dropped. ⚠️ The first cause rests on 13 call sites plus the 200-edge census; the callback and module-object causes rest on one or two examples each, so no rate is claimed for them.
...
## 🔴 context-mode's startup sweep deletes a store that is in use (measured 2026-09-13 13:18 EDT)

The first time a context-mode server touches its store in a session, `getStore()` opens it and then runs `cleanupStaleContentDBs(contentDir, 14)`, which deletes every `.db` in the SHARED content directory whose `-wal` is non-empty and more than an hour old — the WAL a killed server leaves behind. The server keeps its descriptor on the deleted file and searches it for the rest of its life; every later index through it is lost; the path stays empty until something else opens it. Reproduced against 1.0.169's real server: a store with a WAL from a SIGKILLed writer, backdated two hours, one `ctx_stats` — gone, inode still held. It struck this repo on 2026-09-11 and 2026-09-13. **Prevented by** `~/.claude/hooks/context-mode-wal-guard.mjs` (global PreToolUse on `mcp__plugin_context-mode_context-mode__.*`, checkpoint-TRUNCATE before the sweep can run, proofs beside it). **Measured by** `npm run index:health` — exit 4 is a server reading a deleted store, and only a reconnect from `/mcp` or a new session fixes that; nothing can make a running server reopen a file. Ruled out on the way: the CLI replacing a store alone, beside a live holder, or over a stale WAL; codebase-memory, whose CLI indexes in place. Upstream: mksglu/context-mode #1024 (open). `ctx_doctor` stays green throughout — it never checks which file a server holds.
