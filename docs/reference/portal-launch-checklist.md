...
- [x] **A DEV hostname, so the portal is reachable without being at the Mac.** ✅ **LIVE 2026-08-28 12:0x EDT — `https://dev-portal.dioreo.app` answers 200 from the public internet and `/api/review` still 401s from outside, so the door holds.** Tunnel `dioreo-dev` (`e1622a09-…`) runs on the Mac from `~/.cloudflared/dioreo-dev.yml`; the tracked copy is `scripts/cloudflared-dev-config.yml`. Harkirat authorized it from his phone — `cloudflared tunnel login` hands the certificate back through Cloudflare's own relay rather than a localhost redirect, so the browser that opens the link need not be the machine that is waiting.
  ...
  - ✅ **DONE 2026-08-28 12:0x EDT — the whole flow works over the tunnel.** Harkirat registered `https://dev-portal.dioreo.app/auth/callback`; `PORTAL_PUBLIC_URL` in `.env.dev` now reads `https://dev-portal.dioreo.app` (that file is gitignored, so this line is the record of it) and the portal was restarted. Verified end to end over HTTPS: `/auth/login` redirects to Discord with the tunnel's own callback, consent completes, and the portal renders signed-in with `/api/review` returning **200** and the real staged op from the dev database. ⚠️ **`localhost:8787` still serves the portal**, but a sign-in started there now lands its session on the tunnel host, because the redirect is built from `PORTAL_PUBLIC_URL`.
