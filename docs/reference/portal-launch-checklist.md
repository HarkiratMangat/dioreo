...
- [ ] **A DEV hostname, so the portal is reachable without being at the Mac.** ✅ **Harkirat registered `https://dev.portal.dioreo.app/auth/callback` on the `Dioreo (Dev)` application 2026-08-28 11:5x EDT, alongside the existing `http://localhost:8787/auth/callback`** — both can be registered at once, so either way of running the dev portal keeps working. What remains is a second `cloudflared` tunnel **on the Mac**: `cloudflared` is installed (`/opt/homebrew/bin/cloudflared`) and has **never been connected to the Cloudflare account** — there is no `~/.cloudflared/`, so `cloudflared tunnel login` (a browser sign-in Harkirat has to complete) is the one blocking step. Everything after it — creating the tunnel, writing the config, `cloudflared tunnel route dns` to create the DNS record, starting it, pointing `PORTAL_PUBLIC_URL` at the new origin — needs nothing further from him. ⚠️ **Nothing is done at name.com**: `dig NS dioreo.app` returns Cloudflare's nameservers, so name.com holds only the registration. ⚠️ **`wrangler` cannot do this part** — it is authenticated but its token carries `zone (read)`, and it has no DNS-record command in any case. Full filing, including the two things to decide first: `docs/db-deferred-list.md`.
