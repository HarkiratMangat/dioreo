...
~~**Cross-cutting blocker for Phase 2, confirmed against memory (`project_web_admin_portal.md`):** `models/Announcement.js` has `expiresAt` but **no `startsAt` field** — this was already flagged during the original portal design ("Scheduling does not exist today") and filed in `docs/db-deferred-list.md`. The mockup's `SCHEDULED` pill state cannot be computed without this schema field existing first. **This is a hard Phase-2-before-Phase-3 dependency: adding `Announcement.startsAt` must happen before Broadcast's state-pill work can be finished, not after.**~~ **STALE — corrected 2026-08-22 14:11 EDT (Session A, Task 2.6).** `startsAt` was already added to `models/Announcement.js` on 2026-08-21 09:31 EDT, the day *before* this audit was written — wired end-to-end (`core/ops/announcements.js`'s `validatePost`/invert, `portal/ui/broadcast.js`'s Add form, `broadcast.logic.js`'s op builders). This claim was never independently re-verified before being written down; it was carried forward from an earlier memory note that predated the field's addition. **What's still true and still real:** the `LIVE`/`SCHEDULED`/`EXPIRED` state-pill *rendering* genuinely does not exist — `portal/ui/broadcast.js`'s Manifest still hardcodes `stateOf=${() => 'live'}` — so Phase 3's job here is unchanged (build the pill logic from data that already exists), it just isn't blocked on a schema change first. `startsAt` is not a per-user field, so `docs-audit`'s `privacy-inventory` check does not apply — confirmed by reading that check's scope (it only tracks `discordId`/`userId`-carrying fields).
...
## Addendum — 2026-08-22 (Session A, Phase 1)

Every §7 item resolved with real evidence — the local dev portal booted (`node --env-file=.env.dev portal/server.js`, local Mongo, `Dioreo (Dev)`), a `PortalSession` seeded directly in the dev database (owner `discordId`, dev-only, no OAuth secret involved) so a real browser could reach the gated realms without a live Discord login, and every screenshot below taken in real Chrome via `claude-in-chrome` (the in-app browser pane proved unreliable for horizontal-scroll interactions this session and was abandoned partway through, per Harkirat's own direction).

1. **Armory — resolved.** Real screenshot against the actual dev-DB catalogue (133 loadouts, not a fixture). The realm is *structurally* further along than this document's own uncertainty implied: Rack's category counts, Coverage's six flag chips, a wired **Live Preview** panel (confirmed calling the compose-UI spec's `GET /api/armory/preview` endpoint — that endpoint is live, not just designed), and a real Manifest table all render and return correct counts (133 of 133 shown). But visually it is exactly as plain as everywhere else: Rack's category chips are uniform grey, not the mockup's colored left-edge bars, and there is no dupe-tag or dashed-no-image treatment. Net: **no Phase 2 functionality gap here beyond what §3 already scoped** — this is 100% a Phase 3 styling job, now with real data to style against instead of a guess.
2. **Board — resolved, empty-state only.** Real screenshot: the four columns render with the mockup's exact job names (Draft/Staged/Blocked/Ready) and a live "Commit 0 of 3" affordance under Ready. No populated changeset exists in this fresh dev database, so the mockup's card-based visual treatment (tier badges, colored edges) remains unconfirmed in a populated state — **this is the Phase 2/3 boundary the plan itself already draws, not a new gap**; Phase 3 will populate and style it as part of the redesign.
3. **Commit & diff flow — deliberately not re-verified further.** §6's confirmation that the underlying logic (`gateCommit`, `columnFor`, `blockedReason`) is real and tested stands untouched. Manufacturing a tier-3 changeset purely to screenshot its confirm UI would be scope creep of exactly the kind the compose-UI spec's own audit log (F3/F4) already warned against avoiding. Left for Phase 3, which owns building the populated visual treatment (plan Task 3.3).
4. **Analytics — resolved, and the gap is sharper than "unverified."** Rendered `06-access-and-analytics.html`'s Analytics half directly (it lives further down the same single-page mockup as Access, not behind a separate nav click — the top nav's "Analytics" tab is inert markup in this static mockup file): a Health/Usage/Timing tab switcher, KPI stat tiles with sparklines (Errors 24h, RAM Peak 7d, Commands 24h), and a filterable, tier-chip-labeled event river. The live portal's Analytics tab is confirmed to still be exactly the raw monospace `/bot analytics`-style text export described in §7. **This is a missing-dashboard-feature gap, not a missing-style gap** — the two are different programs, not one under-styled version of the other. Flagged for whoever scopes Phase 3's Analytics work: it may warrant its own brainstorming pass, exactly as the plan's Task 3.3 already anticipates.
5. **`toManifestRows()`'s hardcoded `state:'live'` — resolved, and it is NOT masked by overflow.** Queried the live DOM directly (`document.querySelectorAll('tbody tr')` against all 38 Season Manifest rows, draft rows included): every single row's state pill is `class="stt live"` / text `LIVE`, with zero variance. This is independently, unambiguously visible — a real device does not need to scroll past the header overflow to see it, because the column itself renders correctly, it just always says the same wrong thing.
6. **Tablet-width (768–1024px) rendering — resolved, both directions, with a sharper finding than "broken at this width."** Tested at 820px. Season's header (which carries the extra Track/Board toggle) overflows and needs horizontal scroll at 820px, exactly as at phone width — this is content-dependent, not exclusively a sub-640px problem. Broadcast's header (no toggle) fits cleanly at 820px with zero clipping — so the overflow bug's trigger is "how many nav elements this realm's header carries," not a single global breakpoint failure. On the mockup side, `01-season-spine.html` at 820px does not use the shipped code's horizontal-top-bar shape at all — it renders a **left icon rail** (Season/Armory/Broadcast/Ops, stacked vertically) with a colored, shape-coded timeline (solid=live, dashed=staged, red=conflict) and a live staged-changes drawer with T1/T2/T3 tier badges. Neither version has "adapted" to 820px in the CSS sense (§2.1's own nuance: the mockups' breakpoints don't fire until 1180–1240px), but the mockup's actual navigation SHAPE was never a horizontal bar to begin with. **New finding, not previously named as its own line item:** Phase 3 is not just "add media queries to the existing top bar" — it may need to decide whether the shipped bar survives as a concept at all, since the approved mockup's information architecture is a left rail, not a bar that needs to learn to wrap.

**Two additional checks from Task 1.3's falsification pass, both cleared:**
- **Keyboard reachability of `:focus-visible`** — confirmed with a real 3-key Tab sequence on the live Season page: a visible gold focus ring lands on the Broadcast nav link exactly where tab order predicts. The rule is real and reachable, not just present in the stylesheet. §4's "Global rule exists and is real" claim is now doubly confirmed.
- **Broadcast's empty state** — independently re-screenshotted this session (rather than trusting the original audit's screenshot): unchanged, "Nothing is currently showing" / empty Airtime, `0 of 0 shown`. Confirms §3.5's characterization still holds.

**Noticed, explicitly left unscoped (not chased to a conclusion — Phase 1's job is the six §7 items, not an open-ended sweep):** nobody has cross-checked whether any realm's *behavior* (not visuals) matches `/manage`'s own equivalent semantics for the same mutation — e.g., whether Season's bulk-edit op selection matches `/manage`'s bulk semantics exactly. Also noticed in passing: the Armory Coverage panel currently flags 123 of 133 loadouts as `near-duplicate` — a strikingly high rate that may indicate `findDuplicateLoadouts`'s matching logic is broader than intended, but this is a data-quality/logic question outside this document's mockup-vs-live scope and is named here only so it isn't lost.

**No defect found in any existing numbered finding (§2, §3) during this pass** — everything found this session confirms, sharpens, or adds to what was already there; nothing already-frozen was overturned.
