...
A second systemd unit on the existing `e2-micro`. **The 90–125 MB resident figure is an ESTIMATE with no measurement behind it** (Node baseline + Mongoose + 17 compiled schemas + an HTTP layer), against roughly 600 MB of headroom (bot ~112 MB and Ops Agent ~127 MB, both dated 2026-07-17 — **re-measure before committing**). Reached through `cloudflared`, which is outbound-only: no inbound firewall rule, and no dependence on the VM's external IP, which the ops doc explicitly warns changes on stop/start.
