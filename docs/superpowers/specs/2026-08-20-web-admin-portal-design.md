...
**`utils/logger.js` is reusable, and the portal must use it — but not for free.** Measured: **2 local files, zero npm dependencies beyond Node builtins**, so the import is genuinely cheap. ⚠️ **Two things about it are NOT ready for a second process, and an earlier draft of this section asserted otherwise.** `patchConsole()` takes **zero parameters** and `SERVICE_CONTEXT` is a module-level `const` hardcoded to `service: 'dioreo-bot'`, so `patchConsole({ service: 'dioreo-portal' })` is accepted and silently discarded and portal errors would group under the bot in Error Reporting. And `LOG_FILE` defaults to `<repo>/logs/app.log`, which **both systemd units would open independently from the same working directory**, interleaving partial writes into the NDJSON the Ops Agent parses. **Both are small, and both belong in plan 3 Task 1 rather than being assumed:** give `patchConsole` an optional `{ service }` override, and have the portal set `DIORS_LOG_FILE` to its own path.
...
| `invert` | `(change) → op \| op[]` | pure — undo, replayable from either surface. **An array when one change needs several ops to reverse it** — reverting a `scope:'all'` draws purge restores two independent arrays, and collapsing that into one op silently drops half of it. `commitSet` flattens before applying, so the whole reversal stays in one transaction. |
...
**The portal's capability list is CONSERVED against `utils/manageActions.js` by a test, in both directions.** ⚠️ Earlier wording said "derives … never declares its own", and that is not what the design does: `core/ops/*.js` writes `action: 'draws:addnew'` as a string literal, which is a second copy of the key vocabulary. Guarding a copy with a conservation test is legitimate and deliberate — the op needs the key at definition time, and a runtime lookup would invert the dependency — but calling it "derived" invites a reader to assume drift is structurally *impossible*, when in fact it is *caught*. Those are different guarantees and the weaker one is what exists. The op `type` vocabulary and the registry's `page:id` keys are one table, with a conservation test asserting it in both directions — the same shape `scripts/manageActions.test.js` already enforces. A portal with its own action table recreates exactly the bug that registry was built to kill, across two runtimes, where drift is harder to see.
...
| H8 | A revoked admin keeps working | `invalidateAdminCache()` on revoke, **and** re-check per request. ⚠️ **Re-checking does not bypass the 60s TTL, and an earlier draft implied it did** — `hasCommandAccess` reads `getAdminPermissionsMap()`, which serves its cache for up to 60 seconds. What re-checking buys is that the ceiling becomes **60 seconds instead of the 12-hour session**; the explicit `invalidateAdminCache()` on revoke is what collapses it to ~0. Access can also end a live session outright |
...
**Broadcast.** Now showing renders the live set exactly as Discord sends it, in the order it sends them. ⚠️ **`models/Announcement.js` has no ordering field** (`text`, `createdAt`, `createdBy`, `expiresAt`, `color`), so "order" today means `createdAt`. An earlier draft said "slot order", implying a field that does not exist; deliberate ordering would be a new field and a schema change, filed beside `startsAt` rather than assumed here. Airtime puts every announcement on a time axis, which is how "this has been up for nineteen days with no expiry" becomes visible instead of forgotten.

> 🔴 **The realm ↔ page ↔ scope join, stated once here because four partitions of the same domain exist and nothing else maps them.** There are **5 realms** (UI), **7 registry pages** (`ACTIONS_BY_PAGE`), **8 permission scopes** (`MANAGE_PAGE_SCOPES`, which includes `season` — a pseudo-page with no registry entry), and **6 core entities** (`core/ops/*`). Without this table two implementers build incompatible authorization from the same sentences.
>
> | Realm | Pages it covers | Gate |
> |---|---|---|
> | Season | `draws` · `calendar` · `patchnotes` · `seasondraft` · `season` | visible if the user holds **any**; each page's controls gated individually |
> | Armory | `loadouts_mp` · `loadouts_dmz` | same |
> | Broadcast | `announcement` | same |
> | Access | — | **owner only**, via `isOwner()`. Not a grantable scope, exactly as `/bot access` |
> | Analytics | — | the `bot` command token; **revert additionally re-checks the change's own page** |
>
> **A realm is visible if the user holds ANY page in it, and every control inside is gated on its own page** — never all-or-nothing, because a `calendar`-only admin must still reach the calendar. **There is no `portal` permission token and none is added**: holding any `manage.*` scope, or `bot`, is what admits you. The door's admit check is `isAdmin(userId)`; each realm and each control re-checks its own scope server-side.
...
| `PortalSession` | **new** | **`sessionHash`** — SHA-256 of the cookie value; the raw id is never stored — plus `discordId`, `createdAt`, `lastSeenAt`, `userAgent`, `revokedAt`. Backs the live-session list and remote sign-out |
| `Changeset` | **new** | `ops[]`, `authorId`, **`realm`**, `state`, **`exportedAt`** (the tier-3 gate), `createdAt`, `committedAt`. ⚠️ **No `snapshot` field, deliberately** — an earlier draft listed one, which would have been a duplicate store: a tier-2 op's inverse is already captured per-op on `ChangeLog.inverse` at commit time, and a second copy could disagree with it. Reverting a committed set means replaying its rows' inverses. |
...
1. ✅ **RESOLVED 2026-08-20 18:35 EDT — TRANSACTIONS WORK ON M0. The highest-risk premise in this design is closed.** Probed against the real cluster (`atlas-u27ti0-shard-0`, MongoDB 8.0.29) inside a throwaway `portal_txn_probe` database that never touched `diors-builds` and was dropped afterwards: a deliberate mid-transaction throw left **0 of 2 documents** behind, and a clean commit persisted **2 of 2**. Rollback and commit are both genuinely atomic. §5's tier-2 model and plan 1 Task 5 stand as written. Original wording kept below. ~~**Mongo transactions on the M0 free tier.**~~ The changeset model depends entirely on §6's atomic commit. The cluster is a three-node replica set on 8.0.29, so transactions should be available — but *should* is not *verified*. Run a real two-document transaction and confirm it commits and rolls back. **If it fails, §5's tier 2 needs a different commit strategy and the design changes materially.**
2. ✅ **RESOLVED 2026-08-20 18:20 EDT.** Measured live: **969 MB total, 579 MB used, 390 MB available**, `diors-bot` at **131 MB**, disk 24 GB free of 30. Not the ~600 MB this spec originally derived from month-old figures. A 90–125 MB portal fits, leaving ~265–300 MB. **Re-measure immediately before installing the unit** — plan 3's stop-and-move-to-Cloud-Run threshold is 250 MB and this is not a generous margin. ~~**VM memory headroom.**~~ The 112 MB / 127 MB figures are dated 2026-07-17. Re-measure live before adding a second resident process to a 1 GB box.
3. ⚠️ **CHECKED 2026-08-20 18:20 EDT — `cloudflared` is NOT INSTALLED** on `diors-builds-bot`. Nothing is broken; plan 3 Task 7 Step 2 installs it. Recorded so nobody assumes it is already there. ~~**`cloudflared` on the VM.**~~ Confirm it installs, runs as a systemd unit and survives reboot alongside `diors-bot`.
4. ✅ **RESOLVED 2026-08-20 18:32 EDT — the concurrency model works exactly as designed.** Probed against the real dev database on a live `newDraws` element: an update whose filter carried a **correct** prior-value assertion modified 1 document; the **same filter re-run against the now-stale value** modified 0, which is the conflict signal §6.1 depends on; the subdocument `_id` survived the positional write; and the change reverted cleanly. **Element-identity concurrency is verified, not assumed.** ~~**Subdocument `_id` stability across a positional update.**~~ §6.1's whole concurrency model rests on it. Verify that an `arrayFilters` update preserves `_id` and that a prior-value assertion actually rejects a stale write.
...
**Frontend testing had no story at all.** The core's tests are plain node scripts; Preact components had nothing. **Answer: the render functions stay pure and are tested as data** — a component takes state and returns a tree, so a test asserts the tree, with no DOM and no browser.

⚠️ **That collides with the no-bundler decision, and the collision must be resolved before plan 3 Task 5.** `package.json` declares no `"type"`, so Node treats `.js` as **CommonJS**, while a browser module doing `import { html } from './vendor/preact.mjs'` is **ESM** — a CJS `require()` cannot load it. **Resolution: the pure logic each component depends on — `bandClass`, `laneFor`, `tierOf`, filter and sort predicates — lives in `portal/ui/*.logic.js` as CommonJS importing no Preact, and the `.js` component modules are ESM importing both Preact and their sibling logic file.** Every assertion in plan 3's example test already targets exactly those functions. The browser never loads a CJS file and Node never loads an ESM one.

Anything needing a real browser is verified through **the in-app browser preview tools** — `preview_start`, then `read_page` / `read_console_messages` / a `computer` screenshot. Named here because plan 3 says "the preview tooling" twice without ever saying what it is.
...
**F19 — every premise this design rested on is now measured, and one of them nearly derailed it for a reason that had nothing to do with the design.** All six checked: transactions work on M0 (probed on the real cluster in a throwaway database — 0 of 2 after a rollback, 2 of 2 after a commit); element-identity concurrency behaves exactly as §6.1 requires, including rejecting a stale prior-value assertion; capacity is 0.15% used; the render path imports clean and yields production emoji ids; the VM has 390 MB available; `cloudflared` is not installed. **The near-miss is the finding worth keeping**: the transaction probe was pinned to `.env.dev` by a guard I wrote to keep it safe, and the local database is a standalone, so it would have failed and been read as "M0 does not support transactions" — closing the highest-risk question with a confident wrong answer, produced by a safety measure.

### The reader test — 28 findings, and what it says about the three passes before it

A fresh agent with **no context from the design session** was given the spec and two plans, told to verify claims against the source rather than accept them, and asked where the documents are wrong. It returned **28 findings**. Every one I spot-checked was correct, and several were fatal.

**F20 — the three earlier falsification passes all shared a blind spot: they audited the DESIGN and not the CODE the design would produce.** F1–F19 found wrong counts, a wrong concurrency model, a wrong capacity claim and an inverse that stored a summary. What none of them did was *read the plan's own code as code*. A reader who did found, in the first file: a **circular `require` that fails on load with a TypeError** — `core/ops/index.js` required its entities above `module.exports`, so `registerEntity` was `undefined` when they ran. **Task 1 could not have completed.** Three passes had "reviewed" that file.

**F21 — `recordChange()` cannot do what §4 and H6 promise, and its own header says so.** It returns `undefined` (a `.catch()`-terminated chain), so `commitSet`'s `changeIds` were always `[undefined]` and revert could never find its row. It takes no `session`, so audit rows written inside a transaction commit **outside** it — a rollback would leave rows claiming changes that never happened, the exact inverse of the gap H6 says is closed. And its header documents it as deliberately fire-and-forget, which is right for a Discord handler and wrong here. The spec asserted "the caller cannot opt out"; the caller opted out by default.

**F22 — the §4.1 conservation test was mathematically incapable of passing.** Draws has **ten** mutating actions; the plan wrote **seven** op types over a 1:1 `Map`. `addreturning`, `purgenew` and `purgereturning` mapped to nothing, and the map could not have held them if they had. The plan even said "and the three purge scopes" in a later task while modelling one.

**F23 — three of seven inverses were destroyed on the way back in.** `commitSet` validates before applying, including inverses — and the bulk validators re-parsed `payload.text || ''`, overwriting the structured `parsed` data an inverse carries. Undo for bulkDelete, purge and bulkReplace would have silently restored **nothing**. F/P3 congratulated itself for storing the full prior array; the array was stored and then thrown away four lines later, because the test asserted what `invert()` *returned* and never round-tripped it through `validate()`.

**F24 — the Revert button was a privilege escalation, and the file it edits explains why in a comment.** The `bot_` router prefix proves *some* `/bot` access, not ownership or page scope; `handlers/bot.js` says exactly that at the top, which is why every other mutation there re-checks independently. An analytics-only admin could have reverted changes on pages they hold no scope for.

**F25 — three claims about other people's code were simply false.** `patchConsole()` takes zero parameters and `SERVICE_CONTEXT` is a hardcoded const, so the portal's logging service override was a silent no-op. `scripts/buildLegalPages.js` has **no `module.exports`**, so the contrast gate R6 rests on could not be imported. `handlers/router.js` never imports `ALLOWED_ADMIN_ID`, so Task 0b instructed an edit to a file with nothing to edit. **All three were checkable in one grep and none of the three passes ran it.**

**F26 — a new top-level directory fails a CI gate nobody remembered.** `docs-audit`'s `top-level-dirs` is an **ERROR** check; `core/` and `portal/` appeared in neither `CLAUDE.md` nor `docs/README.md`. The first commit of plan 1 would have turned CI red.

**F27 — four partitions of the same domain existed with no map between them**: 5 realms, 7 registry pages, 8 permission scopes, 6 core entities. The reader's verdict was that two implementers would build incompatible authorization from the same sentences, and it was right — §8.2 now carries the join table.

**F28 — smaller, all real:** `page: 'draws'` hardcoded in a module described as generic, which would have broken audit filtering *and* the two-message `canRevert()` that P8 added; `_id` discovered by reading an array tail inside a retrying transaction; a `Changeset` TTL with no state predicate, deleting committed sets; `snapshot` specified in §11 and absent from the model; `sessionId` vs `sessionHash`; "28 tests" against a real 42; a capacity paragraph that double-counted a zero-byte cost and ignored the retention cap already in the code; a CJS test importing browser ESM; the tier-3 gate defined in `season.js` while spanning three realms; and two worktree-executed tasks depending on a gitignored directory.

**What this changes about how the remaining work should be checked.** Reading a plan is not testing it. **The cheapest defect-finding move available was `node -e` on the plan's own snippets and `rg` on the claims about existing code**, and it was not made until a stranger made it. Everything above is fixed in place; nothing was deferred.
