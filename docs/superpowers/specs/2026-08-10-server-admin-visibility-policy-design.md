...
1. **Guild-installed Dioreo is not an "external" app.** Once a server installs the bot, "Use External Apps" no longer governs it, so admins lose the native force-ephemeral lever precisely in the configuration v3 is launching. ⚠️ **UNVERIFIED — this is the premise that sizes the feature, and it must be tested before the sizing is quoted anywhere as fact.** Test: in 𝔇𝔯𝔢𝔞𝔪𝔩𝔞𝔫𝔡 (where the dev bot *is* guild-installed), disable **Use External Apps** for a role and run a public Dioreo command as a member holding it. Public output → gap is real. Ephemeral output → gap 1 does not exist and this feature rests on gaps 2 and 3 alone.
   - ❌ **RESULT, 2026-08-10 18:04 EDT — GAP 1 DOES NOT EXIST.** *(Appended rather than rewritten: this spec is `frozen`, and the paragraph above pre-registered both outcomes, so recording which one happened completes it instead of editing it.)* Harkirat ran exactly this test in 𝔇𝔯𝔢𝔞𝔪𝔩𝔞𝔫𝔡 with a member who does **not** have Dioreo (Dev) user-installed — so the invocation could only have come from the guild-installed copy. `/colors` was public; **Use External Apps** was disabled for the channel; the retry came back **ephemeral**. A guild-installed app *is* governed by that permission. Per the pre-registration, the feature rests on gaps 2 and 3 alone, and every live document that quoted gap 1 was corrected in the same pass.
...
6. Gap 1's Use External Apps test, above. — ❌ **DONE 2026-08-10 18:04 EDT, and it refuted gap 1.** See the result appended under that gap.
