...
| **§8** `portalContrastAudit()` cannot see fallback chains | **Still open, deliberately.** It is a gap in a build tool, not in the portal, and fixing it means teaching the audit to compute contrast on rendered elements rather than token pairs. Filed in `docs/db-deferred-list.md` rather than folded into a design phase. |
