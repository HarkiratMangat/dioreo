...
| **1** | **SEASON** | ◐ in flight | `222b67a` `99bd908` `26e4af0` | 🔴 **READ §0.44, §0.45 AND §0.46 BEFORE TOUCHING THIS — they are why this row was reopened from a ☑ that was false.** ⓪ **Run `npm run portal:diff -- --realm season` first.** ✅ Landed: the recessive Manifest (all 7 realms) · `.rephits li` · the label pass's two-cycle · shared-prefix stemming · the record's baseline and its picture-led banner row with the dot finally dead · `Stage deletion` → `Remove` · **the context-strip band the shell never had, so the Track is above the fold** · the duplicate Track header · the window range · the expiring-banner check · `TL.days`' NaN on real data · the origin-derived OAuth redirect (+5 tests) · `no-cache` at the origin · the clock rebuilt onto one right-aligned axis. 🔴 **STILL OPEN, with evidence:** ~~the overview scrubber~~ **that one was MY sampling error, retracted — I measured eight minis and wrote "every one of 37"; the full set runs 3→894px and it works** · **Events and Playlists auto-collapse on real data**, hiding 20 of 39 items, never adjudicated · **Board and Repairs have never been diffed** (the tool takes no view argument) · **375×812 never diffed at all**, and his original instruction named both widths · 4 export formats against the mockup's 5 · the subtitle measure · the ADD row's alignment · the identity showing a raw id. ⚠️ **The clock, the band, the one-line draft state and the picture-led banner row are MY design calls, made after he refused to answer a pop-up — *"USE YOUR DESIGN KNOWLEDGE"*. The clock is the most likely rejection** |
| **2** | **ARMORY** | ☐ open | | |
| **3** | **BROADCAST** | ☐ open | | |
| **4** | **ACCESS** | ☐ open | | |
| **5** | **ANALYTICS** | ☐ open | | |
