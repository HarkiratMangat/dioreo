...
| **5** | **ANALYTICS** | ◐ **in flight — four root-cause groups closed and committed 2026-09-01 22:11 EDT; ⑤ real-server, ⑥ reader test and ⑦ are owed.** portal:inventory 22/14 → 16/10. ① CASCADE was the masthead stat block on all five views and is GONE — every view now reports `section.panel top 204→204 (+0)` with only a height difference, which is composition, not an offset. ⚠️ **The first four commits of this Part did not touch this plan, which is the §L same-commit rule broken exactly as Part 4 broke it** — recorded rather than quietly repaired. **① converge 69 → 58 mismatches of 62 design nodes** (WORDS 31→37, which is the four-stat masthead Harkirat chose, and STYLE 9→8). **③ inventory 22/14 → 16/10, every remaining row cited in `local/difference-ledger-analytics.md` (20 rows).** **⑤ real-server pass RUN 2026-09-01 22:17 EDT:** `portal:realwalk` walks all five views clean (favicon 404 only) against `node --env-file=.env.dev portal/server.js`, and the live page proves the level fix on real data — INFO 90 grey · **CAUTION 52 orange**, matching Mongo's counts for the live 7-day window exactly, where before this pass all 52 painted as info. ✅ **④ CLAIMED — `npm test` exits 0 at `96e08bd`, 2026-09-02 11:14 EDT, the first full-suite run of the pass.** ⚠️ **It took three runs and the first two were not noise.** Run 1 caught a real temporal dead zone (`RIVER_COLUMNS` read `LEVEL_TAG` before its declaration — safe at runtime, and I had written a comment arguing exactly that). Run 2 hit the documented load-dependent `portal:states` flake on `"export strip open"`, which passes when its registry runs alone, plus two files that are not mine (`.claude/hooks/instructions-loaded-audit.*`, which appeared mid-session and entered this branch through `git add -A`). Run 3 green. 🔴 **AND THE SUITE WAS GREEN OVER A BROKEN DRAWER FOR FIVE COMMITS** — see row 5. A green suite bounded nothing about the interaction tier, because nothing in it opened one. **⑥ RUN 2026-09-02 — two agents, findings folded in; ⑦ is his.** 🔴 **Agent A found that `b908b8d` had deleted `EventDrawer`, `eventRows` and `EVENT_NOTE`** as collateral in a block removal aimed at `worstAck` — 71 lines where ~15 were intended — and it survived `node --check`, the build, ten scoped gates and a full green `npm test`, because nothing in the suite opens the drawer. It surfaced from a `ReferenceError` in a browser console while registering this realm's states. **`portal/fixtures/states/analytics.json` is new and is what found it**: four states reached by hand this pass and now repeatable. Agent B found a QUOTATION IN MY OWN PROMPT that does not exist in `review.html`, and the fixture wall behind Part 6a. Both reports are folded into `REVIEW-PROMPT.md`. 🔴 **UPDATED 2026-09-01 22:47 EDT — ① AND ② ARE NOW ENUMERATED, WHICH IS THE CLOSE TEST.** converge **57 of 62** grouped into seven groups and portalDiff's printed regions mapped one by one, every one to a cited ledger row, in `local/difference-ledger-analytics.md`. ⚠️ The tool prints the largest 14 of 41 regions; the rest are fragments of the same ones and it does not enumerate them — a limit of the instrument, stated rather than claimed away. **The UX-copy audit is walked**, and produced three fixes: the count line said "11 of 11" over a capped 100-row river of a collection holding thousands (the defect `manifest.js:163` already records for Armory, on the realm whose subject is counting), Revert and Reverse were both in use four inches apart, and the no-match state named neither the action nor the total on any realm. **And `ABSENT mk span.sp` was a real gap**: Shell has carried a `meta` slot since Broadcast hand-rolled one, four realms use it, and Analytics was the only one that did not — its five view-meta lines were MOVED there from a duplicated inner panel header, verified on all five views. 🔴 **⑤ RE-RUN 2026-09-01 22:48 EDT AGAINST A RESTARTED DEV PORTAL**, because the running process still carried the pre-`riverTotal` API and a clean walk against a stale binary certifies nothing. All five views walk clean (favicon 404 only). On the real database the level panel draws **CAUTION 52 orange above INFO 90 grey** — caution now sorts ABOVE info, which is the `levelOrder` half of the fix, visible only where real data has both. `riverTotal()` returns **1394**. ⚠️ **The Health meta reads "0 commands"** on the real server and that is CORRECT, not a defect: `AnalyticsEvent` data ends 2026-08-27 and the figure is a rolling 24-hour aggregate. It is the coverage limit this Part already states — the real-server pass certifies REACHABILITY, never the correctness of a populated aggregate. 🔴 **AND A NOUN IN THIS ROW'S OWN SCOPE WAS MISSED AND THEN CLOSED — the drawer opened from a table row.** The cell named three unmeasured interaction surfaces; the first pass closed the tiles and the level rows and did not check the third, and no reference or conservation check could have seen the omission because the scope that named it is prose. The completeness sweep's scope-conservation pass caught it. Closed 2026-09-01 22:28 EDT: every river row opens an event drawer carrying the fields the table has no room for — level, whether it pinged, the memory reading, the page and action behind a change — with a Reverse action on a change that is not yet undone. 🔴 **AND IT EXPOSED A SHARED-SURFACE DEFECT: `Manifest`'s clickable rows were MOUSE-ONLY on every realm that passes `onRowClick`** — no `role`, no `tabindex`, no key handler. Armory's rows escaped it only by accident, through a rename input that happens to sit inside them. The design had already found and fixed exactly this on its own event tables and wrote the reasoning at `analytics.html:540`. Fixed in the component, verified on the running page: a row is `role="button"`, takes focus, and Enter opens the drawer. | | **🔴 START AT `docs/superpowers/plans/ANALYTICS-PROMPT.md`** — tracked, written 2026-09-01 19:10 EDT by the Access session, and it supersedes this cell for anything operational. `npm run portal:status` prints **44KB · 1 handler**; the 46.1KB here is the file's **46,107 bytes** relabelled, so the two are one file in two units and **the tool wins**. **FIVE views — Health · Usage · Timing · Reach · Search — and the names are identical on both sides**, so `--view` works on every one. Near-static reference: composition, not an interaction tier. 🔴 **AND THE REALM IS NOT NEAR-STATIC — that framing came from `portalStatus`'s `/addEventListener|onclick=/` over the MOCKUP page, which cannot see the twelve handlers `analytics.html` wires by property assignment, one of them a drawer opened from a table row.** ⚠️ This cell and the prompt stated that conclusion from the same tool run, so they read as two sources and are one. **Two findings are measured, not grepped**: the masthead is four stats against three (the design tones its error stat `warn` unconditionally, and `.stat.bad` styles nothing), and `LEVEL_ROW` cannot emit `caution` while the dev database holds **306 of 1000 alerts at that level and zero at `warn`** — 30.6% painted as the grey no-severity tier, on a panel whose own paragraph explains three tiers and which draws one. 🔵 **A third, the data-built classes, was checked and CLEARED** — `.rivk.change/.alert/.boot` all have rules on both sides and the `|| key` fall-through is documented as deliberate; an earlier version of this cell sent the next session at it |
| **6a** | **REVIEW** | ◐ **in flight — the FUNCTIONAL objective is DONE and proven; the pixel half is open.** | `2a5b8059` `3a6803ca` `d3f184b9` | 🔴 **THE COMMIT PATH IS WALKED, 2026-09-02 22:50 EDT — `portal:status` no longer reads `never` for this realm.** `scripts/portalReviewWalk.mjs` (`npm run portal:reviewwalk`) drives the REAL dev server over HTTP: stage a tier-1 `draw.add`, see it on the board with its tier, a field-level diff and an OPEN gate, **commit it and prove BOTH the live document and a `ChangeLog` row moved** inside `commitSet`'s transaction (actor, action, target and a non-null `inverse` all asserted), discard a second and prove nothing moved and no audit row was written, then stage a tier-3 `draw.purge` and prove the gate CLOSES with a reason in words and the server answers **409**, with nothing destroyed. **31/31.** It records a NEW receipt kind, `commitwalk`, because `portal:realwalk` walks a realm's VIEWS and Review has none — the gap was structural, not neglect, which is why a near-neighbour instrument existing is what kept it unmeasured. 🔴 **AND ITS FIRST RUN FOUND A LIVE DEFECT ON THE MOST DANGEROUS BUTTON IN THE PORTAL.** `POST /api/revert/:changeId` could not address ANY change minted since ids became `#N` (2026-08-23) — two faults at one seam, either fix alone insufficient: `portal/ui/analytics.js` interpolated the raw id so `#` began a URL FRAGMENT and the id never left the browser (404, null body), and `httpUtil.js`'s `segment()` never decoded, so an encoded `%231` was looked up literally (404, "no such change"). `core/revert.js` was never at fault and proved it by reverting the same change directly. Fixed both; `scripts/portalRevertId.test.js` asserts both halves and **was falsified against the pre-fix code before being wired into `npm test`**. 🔴 **AND THE FIXTURE QUESTION IS ANSWERED, WHICH CHANGED EVERY NUMBER BELOW.** Every resting measurement this realm has ever produced compared an **empty mockup** against a **populated portal** — `review.html:109` returns early on an empty store, the store is `sessionStorage`, and every instrument clears it on load. Unseeded **4.7% / 15 regions**, largest 1168×576, `div.rvnone` against `div.rvdet`: the empty state against the board, a third of the fold, read as a design difference. **Seeded: 2.9% / 21 regions**, mk 25 → 28 nodes and 530 → 720px. The mockup now seeds from its own `F.sampleOps` behind **`?demo=1`** — not a new decision: COMPANION §15 already requires "seeded on request and never automatically", and `?demo=1` is the flag `review.html`'s own do-not-ship comment names. No visible copy; the flag changes DATA, never design. portalDiff/portalAudit/portalConverge learn **`--mk-query`**, general to any mockup-side state flag. ⚠️ **Every figure recorded for this realm before 2026-09-02 22:50 EDT is a comparison of two different datasets and must be re-measured seeded.** ☐ **OPEN:** the 21 seeded regions (ledger `local/difference-ledger-review.md`, gitignored — state the path), ② SHAPE 60 / ③ WORDS 2 / ④ STYLE 8 re-run seeded, the overlay tier (no overlay on either side has ever been opened on this realm), ⑥ the reader test and ⑦ Harkirat. ⚠️ **`npm test` has NOT been run on this branch** — ④ is not claimed. | failure mode: correctness. 🔴 **"ZERO handlers — nothing to open" WAS FALSE AND CAME FROM A BLIND METRIC.** `portalStatus` counted only `addEventListener|onclick=`; `review.html` wires **7** handlers by property assignment (6 `.onclick`, 1 `.oninput`). The tool was fixed 2026-09-01 23:08 EDT and now reads **18KB  7h**. 🔴 **AND REVIEW IS THE COMMIT SCREEN** — `--triggers` gives mk 11 · pt 15 with EIGHT portal-only controls (Discard ×4, Discard all, Commit 4 changes, Export it now), so the realm the plan called static has the highest-consequence interaction tier in the portal. 🔴 **THE MOCKUP IS A STUB OF AN EMPTY BOARD.** `review.html:245` draws `<button … data-demo-only>Load a sample changeset</button>` and `:342` says "The review screen is empty, so there is nothing to confirm" — the design deliberately declines to populate itself, and the UX-copy audit (F4) says that button and its neighbouring copy "must not ship". **So a naive conformance pass drives the portal toward emptiness.** Resolve the fixture question BEFORE the first audit. Resting, measured 2026-09-01 23:08 EDT: converge **40 mismatches of 25 design nodes** (⚠️ 40 > 25 because EXTRA/ABSENT rows count alongside paired ones — it is not a ratio) · inventory **mk 32 sigs · pt 55**, ONLY-IN-MOCKUP 7 / ONLY-IN-PORTAL 30 · portalDiff **4.7%, 15 regions**, portal 96px taller. ⚠️ **Nearly double the elements at 4.7% is the tell**, and the diff sees only 888px of a 984px page. 🔴 **AND THE PRIMARY OBJECTIVE IS FUNCTIONAL, NOT PIXEL — Harkirat, 2026-09-02 10:46 EDT.** `portal:status` reads `review … realwalk · never`: **the only screen where staged work becomes real has never been exercised by any instrument.** The portal's Review is real code (`/api/review`, `/api/changeset/<id>/commit`, `commitSet`'s Mongo transaction with its audit write) — it is the MOCKUP that is a stub. So Part 6a stages, commits, discards and forces a gate refusal against the real server FIRST, and compares pixels second. **Start at `docs/superpowers/plans/REVIEW-PROMPT.md`.** |
| **6b** | **HOME** | ☐ open | | failure mode: composition. Target is COMPANION, **not** the mockup. ⚠️ **"ZERO handlers" was the same blind metric** — `index.html` wires **2** by property assignment; the tool now reads **21KB  2h**. 🔴 **HOME HAS NO PROMPT AND CANNOT SHARE REVIEW'S.** Its failure mode is composition against COMPANION where 6a's is correctness against a mockup that declines to populate itself — one document covering both would blur the two things this plan separates on purpose. Written down rather than silently half-done 2026-09-01 23:08 EDT |
...
| 🔴 **BOTH · THE COMMENTS, AS CLAIMS TO TEST** | Added 2026-09-02 10:38 EDT. Measured across Part 5: **eight defects, every one of them the code violating a rule its OWN adjacent prose states** — a total summed from a six-row sample under a comment reading *"never the sample length"*; classes concatenated eight lines beneath *"THE CLASSES ARE LITERALS, NOT CONCATENATED"*; a gate proven by a designed fixture directly under a comment forbidding exactly that. Writing the rule down discharges the felt obligation to check the line beneath it, so the author reads a comment as handled where an outsider reads it as a claim. **The comments are a ready-made list of assertions nobody has re-checked — the highest-yield thing to hand a reader, and free.** Memory: `feedback_comment_is_a_receipt` |
