...
| **3** | **BROADCAST** | ◐ **all seven §L conditions run 2026-09-01 except the seventh, which is Harkirat's.** ① both views stack identically, `+0px`, ① CASCADE empty on the resting page and CITED (`b.crumb-sep`) under every `--open`. ② diff **0.2% · 16 regions · both pages 1258px** — the regions are exactly the cited set. ③ inventory's six lists: 1 · 3 · 1 · 1 · 0 · 4, **every row cited or settled**, none open. ④ machine floor green — `npm test` exit **0**, `portal:reverse-orphans --ci` clean after a re-record the PILL fix earned. ⑤ **real-server walk PASSES**: past the door, `Delivery queue` 5 rows / `Airtime` 9 rows, distinct readings, one favicon 404. ⑥ owed — the reader test on Part 4's carriers. **Not closed by me.** | *(this branch, unmerged)* | **39KB · 5 handlers.** ② SHAPE **20 → 8** and **22 → 8**, ③ WORDS **1 → 0**, ④ STYLE **9 → 4** on the two views. 🔴 **The largest defect was invisible to every gate and to the percentage: `.stt.saved` painted `--ink2` on `--ok` at 1.34:1** where the design draws near-black at **11.29:1** — found by cropping the two captures into an aligned band and looking, which is where 7 of Armory's 17 also came from. Its cause is a stylesheet-level one: `.stt.saved{background:var(--c,var(--ok))}` guaranteed a fill three lines above a colour fallback still written for the unfilled case. 🔴 **And the same component's `PILL` map emitted `stag`/`sched`/`exp`/`conf` against stylesheets defining only `.stt.saved`/`.stt.staged`/`.stt.conflict`** — `season.logic.js` returns exactly `live`\|`staged`\|`conflict`, so **every staged and every conflict row on Season had been rendering its state chip with no shape at all**, against COMPANION §0.0's law. Armory's `RANK_KEY` defect, in the same file, carried in the same ratchet baseline. ✅ **The interaction tier ran in full** (Harkirat's call, not the derivative shortcut): `--triggers` 17 · 17, and `--open` on the Post form and the Export drawer. The Post form was an **inline `div.panel` where the design opens a shell drawer** — 49 SHAPE pieces mockup-only and the view panel 291px taller; ported, **49 → 21**. ✅ Also closed: the Airtime ruler's Track-borrowed `<b>`, the `never` cell's `<b>`, Posted's cell kind (`td.d` `--ink3` → `td.nums` `--ink2`), `.ncell` as a `<span>` (3 of the 4 mockup pages write one; only `season.html` writes a `div`), the inert `.sev.warn` modifier, the export scope label, a comment naming a `portal/ui/rail.css` that does not exist, and **`portalRealWalk` defaulting to Season's view names on every realm** — which made this realm's first walk fail on two views it has never had. 🔵 **CITED FLOOR:** `docs/reference/portal-decision-ledger.md`'s Broadcast table, ten rows, each with a falsifier. 📄 `local/broadcast-triage.md` · `local/difference-ledger-broadcast.md` (both gitignored — state the paths). ⚠️ **Season's own figures in row 1 do NOT reproduce at HEAD** (12.0% / 135 regions / +224px on the default view, measured twice, before and after this branch's changes, with the changes stashed) — filed in `docs/db-deferred-list.md`, not this Part's work |
