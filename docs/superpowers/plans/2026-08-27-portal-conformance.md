...
| **2** | **ARMORY** | ◐ resting pass substantially closed 2026-09-01, overlay PARTIAL | `0a9fe45` `5d5c3df` (branch `feat/armory-portal-conformance`, unmerged) | **99.6KB · 13 handlers · 48 data-attrs.** ① CASCADE: 2 real defects found and fixed (`id="mhAdd"` missing; `loadTClosed()` default), page-height gap +2172px→+533px. ②/③/④/⑤ triaged against the code, not by inspection — 5 new ledger rows, each with its evidence. **NOT run: `portalRealWalk.mjs --realm armory`** (harness/mockup are both fixture-driven and can both disagree with production — Season's own row calls this pass "owed" even after far more work). **NOT done: a published A/B artifact, a difference-ledger file.** Overlay tier: budget claim (≤4 calls) falsified after 1 of 2-3 surfaces — see ARMORY-PROMPT.md's own falsifier note. Genuine open items filed in db-deferred-list.md: 7 missing category-filter chips (real feature gap, needs a design call) and 2 unopened overlay surfaces + 2 small findings (breadcrumb, empty whobtn). **Not closed by me — Harkirat decides, same as Season.** |
