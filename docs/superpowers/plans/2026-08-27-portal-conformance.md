...
| **2** | **ARMORY** | ◐ **resting pass worked across ALL FOUR views 2026-08-31** — the previous pass had only ever audited the default view, so Repairs, Compare and Bulk & export were unlooked-at while this row read *resting pass closed*. Overlay tier partial. **Not closed by me — Harkirat decides.** | `0a9fe45` `5d5c3df` `e31d8d7` `b6d448e` + this session (branch `feat/armory-portal-conformance`, unmerged) | **96KB · 14 handlers** (`portal:status`, not the 99.6/13 this row used to quote from a second measurement). **Four views, measured for the first time:** Tier board 10497→11029 (+532px, was +574) · Repairs 8817→9363 (+546, was +587) · Compare 9504→9013 (−491) · Bulk & export 8678→9182 (+504). ⚠️ **EVERY ① ROW ON THIS REALM IS `top +0, h ±N`** — the panel starts at the same y on both sides and differs in HEIGHT, so §0.7c's *fix the first ① alone* (written for a cascading TOP offset) points at a symptom here: ① and the page-height delta are the same fact, produced by ②/④ content. Written down because reading the rule literally costs a pass. 🔴 **THE LARGEST DEFECT WAS DEAD CSS, AND NOTHING HAD EVER REPORTED IT:** `RANK_KEY` emitted `t-t3`/`t-t4`/`t-t5`/`t-none` while BOTH stylesheets key the Armory tier block on `.trow.t-top3`/`.t-top4`/`.t-top5`/`.t-unranked` — so four of the five tier rules had never applied once, taking with them the graded 20/17/15/14/13px marks and the `--bc-dim` card fade, which is the entire reason the board reads as tiered. Same class as the missing `id="mhAdd"` the previous pass found. ✅ **Seven more fixed:** two view headers where every design draws one (Rack/Coverage/Compare each rendered their own `.ph`; `Shell` has carried a `meta` slot since Broadcast needed one and Armory never passed it — `portal:probe --sel .ph` now reports `w=1160 h=59 x=99 y=274` on both sides with every property agreeing) · the export strip offered 2 of the 4 scopes `/manage` has, so `This view`, `Category` and `First five` were built and `mh-take-n` now reads **4 formats** exactly like the design's · a Manifest `mode` chip whose every non-default value was guaranteed to render an empty table, and which produced the `MP ×2 / DMZ ×2 / All ×2` duplication `--triggers` reported · the Manifest count dividing by the rows handed in rather than the catalogue ("125 of 125" over 133 builds) · the tier rows' accessible name fusing to "Best in category18one weapon per category" (PASS 6's class — htm drops a whitespace-only text node that spans a newline, so the separator has to be same-line) · the lead stat reading "MP builds" where the design says "MP builds shown" · need-repair using the `bad` tone where armory.html:21 uses `warn`. 🔬 **AND ONE INSTRUMENT DEFECT: `portalAudit --triggers` printed `season` as a hardcoded literal**, so every run on every other realm named the wrong subject above a correct listing — §0.5a R1's own failure, in the tool that enforces it. **`--triggers` ONLY-IN lists went 6+6 → 3+3**; the three surviving pairs are the identity chip (a PRIVACY decision, per Season's row) and the two `ADD_KEY` hints (deliberate, commented in the code), all three now cited. 🔵 **REGRESSION CHECKS PASS** after changes to shared `manifest.js`: Broadcast height-identical at **+0px** with ③ WORDS **0**, Season unchanged at its cited **+16px** cascade. 🔵 **RESOLVED — the prior pass's "inconclusive" `--open-sel` result was not ambiguous:** Armory's build editor is an INLINE panel in `.bed`, not an overlay, so clicking a row REPLACES the rack and a SHORTER page is the correct outcome. There was no overlay to open. ✅ **ALL THREE OVERLAYS ARE NOW OPENED AND TRIAGED.** The bulk-delete `Confirm` opens on BOTH sides with `--open-sel "td.ra button"`, and every difference it shows is downstream of a decision already recorded IN CODE — `scripts/portalOpsReach.test.js:35` carries `loadout.delete` → `loadout.bulkDelete` as a checked equivalence with its reason. **Zero new FIX rows from it.** ⚠️ The diff makes that read as an over-gated destructive action; it is not one to lower on a diff's say-so. The build editor turned out not to be an overlay at all. 🔴 **A SECOND PASS, 2026-09-01, DRIVEN BY LOOKING RATHER THAN BY A SECTION — six more defects, none of which any audit section had named.** A panel inside a panel (Shell already draws `section.panel` and every view opened its own, so 23px of padding applied twice and `.rack` measured 1114px at x=122 against 1160 at x=99) · the Category column printing the stored enum `AR` beside a chip reading `Assault 35` · the Manifest naming itself twice · two legends where the design draws one · the chip count folded into the label string, which wrapped the toolbar and pushed `+ Add build` onto its own row · and the stat row ending on `staged` instead of on the two problem figures. **Tier board +532 → +480px, ① +128 → +109, ③ WORDS 16 → 12.** ⚠️ **This is §0.7d working exactly as written** — the diff found the surfaces, reading the code around them found the causes, and the six that mattered came from cropping the two captures into aligned bands and looking at them. ⚠️ **And one triage rule needed a boundary:** the Build/Category column swap reported SYMMETRICALLY, which §0.7c classifies as a pairing artifact; it was a real reorder. Symmetry is evidence of an artifact only when the two elements are INTERCHANGEABLE. 🔴 **AND A THIRD PASS FOUND THE ONE THAT MATTERED MOST, FROM A PHONE.** Harkirat looked at the A/B on his phone and asked what was different about the ROWS' BACKGROUND. Answer: the mockup's table rows are `#0F1418` (`--desk`) and the portal's were `#171E24` (`--raised`) — the design's manifest panel is the ADJACENT SIBLING of its view panel, so `.panel + .panel{background:transparent}` fires and the table reads as a well cut into the page. A single `p.hint` between the two panels broke the selector, on every row of a 125-row table, **with both stylesheets carrying the identical rule.** ⚠️ **THIS IS THE STRONGEST ARGUMENT IN THIS DOCUMENT FOR §0.7d.** ④ STYLE reported it as ONE `section.panel backgroundColor` row among 149 and I read past it twice; the reverse-orphan sweep saw a rule that HAS elements; a class-signature diff sees nothing at all. **A structural instrument cannot see a selector that fails because of what sits BETWEEN two elements** — only the rendered pixel can, and only a person asking about it made anyone sample the pixel. 🖼️ **A/B ARTIFACT PUBLISHED: https://claude.ai/code/artifact/d2a35dd7-d409-44ea-ba96-4c14a16850fa** — both full-page captures (mk 10,541px · pt 11,073px) with a lockstep flip that HOLDS the scroll offset across the swap, because a flip that jumps to the top compares two different parts of the page. Not a reconstruction: two running pages at one frozen instant. 📄 **`local/difference-ledger-armory.md` IS WRITTEN** (gitignored — state the path): the 9.6%/906-region figure enumerated against a cited floor, where the floor is one repeated row difference counted 125 times. 🔴 **What is genuinely left is the RESIDUAL ROW HEIGHT and it is CITED, not open** — the `.detcell` + `.thumb` pair is ~370px of the +532, and both classes are defined in the DESIGN's own stylesheet; its Armory table just does not emit them. 📄 **Triage: `local/armory-triage.md`** (gitignored — state the path), four buckets over all four views; every CITED row is now in `docs/reference/portal-decision-ledger.md` with a falsifier. |
