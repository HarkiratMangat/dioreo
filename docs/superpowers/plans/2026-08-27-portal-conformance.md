...
| **4** | **ACCESS** | ☐ open | | **46KB · 8 handlers** per `npm run portal:status`, three views (`By admin · By scope · Sessions`). ⚠️ **This cell read "47.7KB · 7 handlers · 15 data-attrs" until 2026-09-01 and all three were wrong** — the tool prints 46/8, `rg -o 'addEventListener\|onclick=' access.html \| wc -l` returns 8, and **nothing in `scripts/` prints a per-realm data-attribute count at all**, so the third figure had no producing instrument (`access.html` carries 12 distinct `data-*` names / 30 occurrences; `access.js` carries none). Caught by the §0.5c reader test on Part 4's prompt, which documented the disagreement while this row went uncorrected — **§L's own opening rule is that a row whose status is wrong is worse than no row**, and it applies to figures, not only to ticks |
