...
| **1** | **SEASON** | ◐ in flight | | ① LOCATE done → `local/locate-season.md` · ② SWEEP triaged · ③ MEASURE done for masthead, identity (collapsed **and** open), the record, Board and Repairs. **Five fixes landed**: the recessive Manifest (COMPANION §16.35, all seven realms), `.rephits li` (§16.36), the dead pre-`.srec` stylesheet region, `Stage deletion` → `Remove`, and the record's sixth title 8px out of its row. **Still owed: ④ WALK · ⑥ UX-COPY · ⑤ CLOSE**, plus `lbl-cut` with no rule, `.bar.stemmed` and `.ruler span.masked` unported, `.zhint` unemitted, `.rise` unemitted, the expanded editor's control order, and Repairs' three-of-six checks |
| **2** | **ARMORY** | ☐ open | | |
| **3** | **BROADCAST** | ☐ open | | |
| **4** | **ACCESS** | ☐ open | | |
| **5** | **ANALYTICS** | ☐ open | | |
