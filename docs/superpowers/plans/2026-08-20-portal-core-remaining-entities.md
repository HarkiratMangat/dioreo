...
## Audit log — second pass, against the implementation (2026-08-21 09:45 EDT)

The handoff at `local/handoff/2026-08-21-portal-plan2-handoff.md` recorded that this plan's own draft code had already been proven wrong once for draws (Task 6 integration, plan 1), and directed the next session to cross-check every entity's op design against its real handler before implementing — not trust the plan text above. Doing that for all five entities surfaced defects the first audit pass (Q1-Q6) did not catch, because Q1-Q6 reasoned about the *shapes* of the ops in the abstract; this pass ran the plan's own code paths against the real handler line by line.

**Q7 — calendar's own draft implementation (Step 3 above) repeats the exact bulkReplace bug Q1-era draws already paid for.** The code block in Task 2 does a wholesale `SeasonalData.updateOne(DOC, { $set: { [PATH]: op.payload.parsed } })` for "Replace" — but `handlers/manage/calendar.js`'s real `bulkAddOrReplaceCalendar` has always used `upsertEventsByTitle` (a fuzzy-title merge, `utils/bulkMerge.js`) for its replace mode, never a wholesale overwrite. The shipped `core/ops/calendar.js` uses the merge. Also found: single add/edit never `toTitleCase`s the title (draws does; calendar never has), add/edit/bulkAdd all need to resort the array by date afterward (same reasoning as draws' own resort fix), "Delete Multiple" collects pasted titles not ids, and "Banners" needs `utils/calendarBannerCache.js`'s cache/clear calls, not a plain field `$set`.

**Q8 — loadouts' own draft implementation used `computeWeaponKeyAndBuild()`, which is the wrong function.** That helper is `/autobuild`'s auto-naming utility — it invents its own `buildName` ("Build N") and `imageKey` from the weapon name, discarding whatever the admin actually typed into those two fields. The real `/manage` add/edit handlers derive `weaponKey` via a plain `weaponName.toLowerCase().replace(/\s+/g,'')` inline. Also found: "Add Multiple" is already an upsert by `{weaponKey, mode, buildName}` in the real handler (a blind `createDocument()` per block would duplicate on re-run); "Replace Multiple" reuses the *exact same* modal and handler function as "Add Multiple" (`manageActions.js`'s `loadoutsActions()` opens `buildLoadoutsBulkAddModal` for both ids) — there is no wholesale-replace behaviour for loadouts at all, so the draft's `deleteMany({mode})` + recreate would have wiped every build of that mode not mentioned in the paste; "Delete Multiple" fuzzy-matches weapon names, never ids; and `orderAttachmentsBySlot` (named in the plan's own Interfaces line) is never called anywhere in `handlers/manage/loadouts.js` — it belongs to `/autobuild`'s vision pipeline, and calling it here would silently reorder every admin-typed attachment list.

**Q9 — the plan's own Interfaces line for patch notes omits a real mutation.** `handlers/manage/patchnotes.js`'s `handlePatchSeasonPick` → `modal_patch_editseason_{id}` flow edits any past entry in place, with no equivalent in the plan's five named op types (`setDateInfo`/`setUrls1`/`setUrls2`/`addSeason`/`purge`). It was invisible to a registry-only read: its custom_id embeds a Mongo `_id`, which `utils/manageActions.js`'s group/action split can't represent, so it is not a key in `ACTIONS_BY_PAGE` at all — the same reason the announcement per-row buttons aren't in the registry either (noted in Task 6's own text, but not generalized to patch notes). `core/ops/patchnotes.js` adds `patchnote.editSeason`. Also found: the real handler uses `parseReleaseDateTime` (admin-local-clock-aware) everywhere, never `parseAdminDate` — every other admin date field is UTC-0-only, but patch notes' release date is the documented exception (`design-decisions.md`).

**Q10 — the plan's Interfaces line for season names only four op types and is missing three real, registered actions.** `utils/manageActions.js`'s `SEASONDRAFT_ACTIONS` has `settitles`/`bulkdraws`/`bulkcalendar` alongside `promote`/`discard` — five real page actions — but the plan's Interfaces line names only `setTitlesDeadlines`/`promoteDraft`/`discardDraft`/`startNew`. Had Task 7's own conservation test (`EVERY mutating action on EVERY page resolves to an op`) actually run against an implementation built only from that line, it would have failed on `seasondraft:settitles`/`seasondraft:bulkdraws`/`seasondraft:bulkcalendar` immediately — the gap was only visible by reading `ACTIONS_BY_PAGE.seasondraft` directly, not the plan text. `core/ops/season.js` adds `season.setDraftTitlesDeadlines`/`season.bulkDraftDraws`/`season.bulkDraftCalendar`.

**Q11 — `season.restoreSnapshot`'s own invert() would have thrown, breaking its OWN forward apply().** `core/changeset.js`'s `commitSet` calls `impl.invert(...)` immediately after every successful `apply()` — inside the same transaction, to populate `ChangeLog.inverse` — not only when a user actually clicks revert. An op whose `invert()` throws (the plan's own text describes `restoreSnapshot` as carrying "no action key" and being unreachable, which reads as "it never needs a real inverse") would therefore fail its own apply every time, rolling back the write that had just succeeded. Fixed by making `restoreSnapshot` self-symmetric: its `apply()` snapshots exactly the fields it is about to overwrite, and its `invert()` returns another `restoreSnapshot` carrying that snapshot — reverting a revert of the highest-blast-radius op in the system stays correctly revertible with no second op type and no throw path.

**Q12 — two of the plan's own draft *tests* assumed contracts the real functions don't have.** `calendar.add`'s test checked a field named `is2XCP`; the real `SeasonalData.calendar[]` schema field is `isDoubleCP` — a name that does not exist on the model at all would have passed a test asserting nothing real. `announcement.post`'s test passed an absolute date string (`'2026-09-01'`) as `expiresAt`; `utils/announcement.js`'s `computeExpiresAt()` only parses blank/"never"/a whole number of days, so that fixture would have failed as an unparseable *expiry*, never reaching the start-after-expiry check the test claimed to prove. Both were corrected in the shipped test files (`scripts/calendarOps.test.js`, `scripts/announcementOps.test.js`).

**Not found (this pass):** no case where a real handler's mutation had no home in any of the five entity files once the registry and the handler were both read; no defect in `core/mongo/document.js`'s contract under real use (loadouts/announcements).

**Scope note:** this pass built and tested `core/mongo/document.js` and all five `core/ops/*.js` files (Tasks 1-6's Steps 1-4). The handler-refactor steps (Steps 5-7 of Tasks 2-6: snapshot the real modals, wire `handlers/manage/*.js` onto these ops, click-test on the dev bot) and Task 7 (retire `registerUndo`, open `core/revert.js`'s `ON_CORE`) are **not done** — `core/revert.js`'s `ON_CORE` is deliberately left at `Set(['draws'])` rather than extended, because extending it before the handlers are wired would make `canRevert()` report the wrong reason ("predates revert support" instead of "not yet supported") for every current change on these pages.
