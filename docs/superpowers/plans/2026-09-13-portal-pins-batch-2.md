...
DO:
 ...
 2. The Broadcast composer's banner-image and repeat inputs per §10.3, bound to agent A's fields — rows 1 and 8 included, which also touch `portal/ui/broadcast.logic.js` (both op builders) and `handlers/manage/announcements.js:42` (the Discord edit payload).
...
| 3 | The cards come before the table: at 1440 the table starts ~1,265px down. `.dcard.lc{max-width:none}` (`app.css:2674`) beats `.cmpcards .dcard{max-width:360px}` (`:1753`), so a fifth or a single card stretches to 1,274px | **P1** | Table first. Whether the cards are dropped or kept behind a "Show cards" toggle is Harkirat's call, asked by the critique session with the screenshot (`armory.js:719` already says the Discord render moved to the editor); this row is updated with his answer. Fix the max-width override either way |
