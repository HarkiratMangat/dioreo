...
- [ ] **Step 2 — ledger:** manifest widths by role · the add button · Broadcast's state column and StatePill · HeadsUp · admin traffic (2026-09-02, masthead stats) · Armory's columns (sortable, Stage deletion) and its single New build chip (2026-09-11) · every `span.sp`/`p.chint`/`p.pnote`/`.hint` row · the whole "Decided 2026-09-14 — portal pins batch 2, design board 2" section and the seven rows it retired or amended.
...
|---|---|---|---|---|---|
| `portal/ui/tokens.css` | ✏️ | | | ✏️ slot hues, radii, button inset (§10.4 rule ②) | |
| `DESIGN.md` | | | | ✏️ the tokens §10.4 rule ② adds | |
...
GATES: npm test · the new armory.logic test · node scripts/portalReverseOrphans.mjs --ci · node scripts/portalStates.mjs --ci · the plan §10.4 refinement-contract rules that apply to the drawer and composer (C3 heights and centre lines, C4 edges, C5 label gaps, C9 action runs, C12 no truncation), measured with getComputedStyle/getBoundingClientRect and listed in RETURN · live getComputedStyle checks on port 8788 for every §10 row, and chrome-devtools captures compared against the board's G8 and G9 states.
RULES: as agent A.
...
| C14 | Look at 1282 AND on the phone viewer he reviews from | nothing clips a control or an edge at phone width; a phone layout itself stays not a priority (§10's rules) — the board's sideways-scrolling sections are a board mechanic, not a portal spec | 14:11 EDT |
...
| 2 | The weapon line | **P1** | **Header row, 52px:** grid `32px auto minmax(0,1fr) auto` — the checkbox that selects every build of the weapon, then the weapon name (`--t-md` 600), the category word in the category colour (micro caps, .16em) followed by **"• N BUILDS"** (a 3px dot, `--ink3` micro caps) — on the left, because the right side is for actions (13:42 EDT) — then a 1px rule and the outline tier / META / TOXIC tags. **A 4px category accent edge on the header only**, replacing spec §6's square category chip (*"the square color chip is mostly redundant and useless since you added the left side accent border"*, 09:44 EDT), and a category-tinted gradient on its left third; builds carry the accent only on hover (10:56 EDT). The whole header toggles its group; its collapse button at the right is a boxed icon (C3, C4) using **fold / unfold** glyphs, distinct from the sort chevron (11:35 EDT) |
