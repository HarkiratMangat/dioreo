...
- [x] *(2026-09-13 19:26 EDT — 10 + 12 + 8 rows; one ledger row for Harkirat's method decision)* Write each surface's findings into §10 of this file, replacing its *Not yet written* line — one row per finding, `finding · severity · what the build must do`. A decision Harkirat makes goes in the ledger as a row.
...
| 3 | The drawer states rules §6 retired: "Defaults to Standard Build" (`armory.js:394`, fallback `:352`), "counted against nine" / "a different count is flagged" (`:406`), "is not ten" (`:431`). The same `Standard Build` fallback renders in the edit title (`:580`) and Compare (`:864`, `:914`) | **P1** | Coverage copy reads "2 or fewer attachments is flagged". The code hint is computed from the filled slots: "Code is 2× attachments: 5 → 10 characters". The `Standard Build` fallback goes from all five sites |
...
| 3 | The cards come before the table: at 1440 the table starts ~1,265px down. `.dcard.lc{max-width:none}` (`app.css:2674`) beats `.cmpcards .dcard{max-width:360px}` (`:1753`), so a fifth or a single card stretches to 1,274px | **P1** | Table first. The cards go behind a "Show cards" toggle or are dropped (`armory.js:719` already says the Discord render moved to the editor). Fix the max-width override either way |
...
| 7 | At 390 wide the panel overflows (empty state 435 vs 364, from a 448px suggestion button) and six columns in 332px make a 2,571px table | **P1** | Below ~640px, a stacked field-by-field view (or two columns inside an `overflow-x:auto` container); suggestion buttons wrap |
...
| # | Finding | Severity | What the build must do |
|---|---|---|---|
| 1 | **A blank "Ends" posts an announcement that never ends, while the drawer says "blank = the server's 60-day default, not never".** The portal sends `expiresAt: null`, which `alreadyNormalized` (`core/ops/announcements.js:13`) passes through untouched. Measured with the real `validate`: portal-shaped blank → `null`; `/manage`-shaped blank (`expiry: ''`) → 60 days out | **P1** | Send the raw expiry the op expects (`expiry`: blank · a whole number of days · `never`) or resolve the default before staging, so blank means 60 days on both surfaces. Raises the existing `[P2 · S]` "EXPIRY DATE where the design takes blank, days, or never" item in `docs/db-deferred-list.md`: it is not only that "never" is inexpressible — the blank case is wrong today. Verify by staging a blank-end post and reading a date 60 days out in Review |
| 2 | Banner image — the input | **P1** | A field "Banner image (optional)" directly under Text, before the date row: an https URL input (no upload; the op has no pipeline). Hint: "An https:// link. Discord shows it full-width under the text." Beside it a 56px thumbnail using Season's `<img onError>` broken-link detector; a link that does not load warns "That link didn't load as an image — players would see a broken embed", and does not block. The op's own error "The banner image needs to be a full https:// URL." shows inline on a non-https value |
...
| 4 | The composer has no preview. The mockup's composer rendered a Discord card (`broadcast.html:194`), `DiscordCard` is already imported in `broadcast.js:8`, and with an image the preview is the only place its placement can be judged | **P1** | Render the announcement as Discord will — heading line, body, banner image — beside or under the fields, plus one portal line under the card: "Shown up to 3 times, a day apart". This is one of the four composition changes Harkirat has not reviewed (linksee anchor #4), so the build shows it to him before keeping it |
...
| 8 | **Any edit wipes a banner and a repeat count.** The edit apply writes `bannerImageUrl: payload.bannerImageUrl \|\| null` and `repeatCount ?? null` (`core/ops/announcements.js:113`), but neither `buildBroadcastEditOp` (`portal/ui/broadcast.logic.js:12`, wired to the Manifest's inline edits at `broadcast.js:450`) nor `/manage`'s Discord edit (`handlers/manage/announcements.js:42`) sends them. Session 1 shipped the fields; nothing edits them yet, so today the loss is latent — it becomes real the moment the composer can set them | **P1** | Both edit builders carry `bannerImageUrl` and `repeatCount` from the row, in the same change that adds the inputs. Verify by staging a text edit on a record that has both and reading them unchanged in Review's preview |
...
Run /impeccable critique on the New Build drawer and Compare (§4), on the harness with chrome-devtools; each run's two agents are the only agents you dispatch. The Broadcast composer is critiqued with /design-critique.
