...
DO:
 ...
 2. The Broadcast composer's banner-image and repeat inputs per §10.3, bound to agent A's fields — rows 1 and 8 included, which also touch `portal/ui/broadcast.logic.js` (both op builders) and `handlers/manage/announcements.js:42` (the Discord edit payload). The banner re-host (row 2) adds `utils/announcementBannerCache.js` and touches `core/ops/announcements.js`'s `apply`.
...
| 2 | Where paste-many lives | **P1** | A two-segment switch `One build · Paste many` on the drawer's title row, with the MP/DMZ switch moved up beside it so it governs both. In Paste many the left column becomes the textarea with a collapsed Format disclosure, and the right column swaps the record card for the parse preview (counts, rows, then `BulkOverwrites`). Title reads "New MP builds"; footer reads "Stage 3 builds · 1 update, 2 new", blocked as "Paste at least one block". Switching with a partly filled single draft carries it in as the first block. The Bulk & export view loses its paste |
| 3 | The drawer states rules §6 retired: "Defaults to Standard Build" (`armory.js:394`, fallback `:352`), "counted against nine" / "a different count is flagged" (`:406`), "is not ten" (`:431`). The same `Standard Build` fallback renders in the edit title (`:580`) and Compare (`:864`, `:914`) | **P1** | Coverage copy reads "2 or fewer attachments is flagged". The code is a plain field with a copy button and a quiet check when its length matches the attachments — no hint line and no split into digit–letter pairs (Harkirat, 2026-09-13 23:45 EDT: the pairs "look and read terribly"). The `Standard Build` fallback goes from all five sites |
| 4 | The draft preview passes `siblings=${[previewBuild]}` (`:479`), so the card always reads "Build 1 of 1" | **P1** | Row 1 Weapon · Category. Row 2 a read-only line computed from the real siblings: "Files as **Build 4 of 4** for ICR-1 · MP". Row 3 "Label (optional)", empty placeholder, hint "Leave blank and the card shows Build 4". The preview card uses the same number |
...
| 7 | The paste textarea (`:1045`) has no accessible name, only a placeholder | **P2** | Label it "Builds to paste" |
...
| 9 | Explanation prose outweighs the fields: MP-empty content is 1,406px against 608px visible; the badges policy and image-convention paragraphs are always open | **P2** | Intro becomes one line ("Weapon and category stage it; the rest can follow"); the policy and image paragraphs become one-line hints with a disclosure |
...
| 13 | `clarify` — the preview caption "The record you are writing" names the act, not the thing; the paste mode will need its own caption | **P3** | "Record preview" in One build; "What this paste will change" in Paste many |
...
**Board v4 amendments to §10.1 (2026-09-14 00:35 EDT), from Harkirat's comments on the design board.** Directives (his): the MP/DMZ switch sits first, then a rule, then the build-count switch, and the MP/DMZ switch keeps its current look · "Paste many" is renamed **Many builds** with a stacked-layers icon, not a copy-looking one · the build number leaves the title and becomes the first field of the build, beside Label · "Record" is jargon: the side panel is **In Discord** · the weapon and attachment fields are fuzzy searches · DMZ builds take up to nine attachment slots and carry no gunsmith code, so in DMZ the drawer shows all nine slots and no Code field (the real `DMZ-AK117-1` image has nine slots and no code) · the Many builds panel is redrawn after a `/design-critique` (`local/pins2-critique/design-board-v32-bulk-critique.md`) found its example paste was not the parser's format and two of its outcomes were invented. **Measured this session:** `Loadout.attachmentSlots` is empty on all 133 prod builds, but Cloudinary structured metadata carries the vision slots on 130 of 134 loadout images — 217 attachment names, none filed under two slots — because the 2026-07-21 backfill predates the 2026-07-24 line that also writes Mongo. The gunsmith code's digits agree with that metadata (1 Muzzle · 2 Barrel · 3 Optic · 4 Stock · 5 Perk · 6 Laser · 7 Underbarrel · 8 Ammunition · 9 Rear grip, checked on BAL-27 1–5, FFAR 1 1–3, ICR-1, DL Q33). **Drawn in design board v4 (`local/pins2-critique/design-board.html`), pending his review:** each attachment match shows its slot and how many builds use it, a slot already filled is greyed, and the picked name files into its slot; Rank is a switch — None · Best · Top 3 · Top 4 · Top 5 for MP, None · Best close · Best mid–long · Top 3 · Top 5 for DMZ (the stored `categoryRank` / `dmzRangeRank` values); Many builds numbers each block in an editor gutter, repeats the number on its result row, and leads with a tally of new · updated · saved with a warning · can't be read — the parser's own four outcomes. The catalogue backfill into Mongo is filed in `docs/db-deferred-list.md`.

**Board v5 amendments to §10.1 (2026-09-14 01:08 EDT), from Harkirat's second round of comments.** Directives (his): the switch reads **Add build · Bulk create** · Rank reads **Tier in AR** and the switch takes the selected tier's colour · the gunsmith code sits above the attachments · the autocomplete drops its "N builds" counts · section headings read as headings · the build number stops looking bolted on · Bulk create shows the real paste format and loses the numbered circles, keeping the line numbers and each build's coloured rail. **Answered by measurement:** a pasted code can fill the attachments. Across the 121 loadout images carrying both a code and slot metadata, a digit–letter pair names the same attachment on every build of the same weapon (0 conflicts across 413 weapon-pair keys), while the same pair on different weapons disagrees on 36 of 56 pairs — so the lookup is per weapon. Leave-one-out, 311 of 587 attachments are filled from the other builds of their weapon with 0 wrong, and 15 of 121 builds fill completely; a pair never seen before still names its slot from the digit and leaves the name to type, with a search limited to that slot. The paste format on the board is `formatLoadoutsAsBulkText`'s own output for real dev builds, which is also the portal's `BULK_EXAMPLE` (`portal/ui/armory.js:957`). **Drawn in the board, pending his review:** the build number as a display numeral locked into the left of the Label field; tier colours gold for Best (the portal's legendary gold) and the info blue fading through Top 3–5.
...
| 4 | Weapon, Build and Image always differ, and they count toward "N fields differ" | **P1** | Pin the identity rows as a header block outside the count; order the remaining rows differing-first. All rows still draw — `armory.js:703` keeps matching rows on purpose |
...
| 7 | At 390 wide the panel overflows (empty state 435 vs 364, from a 448px suggestion button) and six columns in 332px make a 2,571px table | **P1** | Below ~640px, a stacked view: one block per field, each build's value on its own line, differing values marked as in row 1; suggestion buttons wrap |
| 8 | Chips and column headers fall back to `Standard Build` (`armory.js:864`, `:914`); after §6 every build of a weapon reads the same | **P1** | Label every build with the derived "Build n of N"; the optional name is a second line only when set |
...
| 14 | `clarify` — the empty state says "Nothing to compare yet." then "Pick one weapon, or two."; the real limit is six columns, and the two uses are different questions | **P2** | Heading "Pick a weapon"; one line "Its builds line up field by field. Add a second weapon to set them side by side." The cap is stated where it bites (row 5), not here |

**Board v4 amendments to §10.2 (2026-09-14 00:35 EDT).** Row 2's "existing slot data" is the Cloudinary metadata named in the §10.1 note, not `attachmentSlots`, which is empty. Harkirat asked whether builds with different slot sets were considered: rows are every slot any shown build uses, in Gunsmith order, and a build lacking a slot Build 1 has shows a dashed **Not equipped** cell, distinct from a different attachment (raised and ringed) and from a slot neither uses (a dash) — drawn with the real BAL-27 and FFAR 1 builds in design board v4 (`local/pins2-critique/design-board.html`). The phone state is not drawn: phone is not a priority (Harkirat, restated on the board).
...
**Board v4 amendments to §10.3 (2026-09-14 00:35 EDT).** Directives (his): the length budget copies Analytics' fill bars (`.erow`) — other live posts, this post, what's left of 6,000 · the repeat visual carries no dates, because a player who does not open Dioreo for three days sees the second showing on day four. **Drawn in design board v4 (`local/pins2-critique/design-board.html`), pending his review:** the stepper sits beside one token per showing joined by "24h+" links; when the window cannot hold them all, the tokens past the end day go hollow in warn, which replaces row 3's hint and warning sentences · Never is a switch beside the Ends label that replaces the date with "Stays up until you remove it" (row 1's explicit never control), since a chip inside the field read as a copy button · the preview panel is headed **In Discord**.

**Board v5 amendments to §10.3 (2026-09-14 01:08 EDT).** Harkirat pointed the budget at the thin meter in Armory's Repairs cards (`.cmeter`), not Analytics' rows: one 4px track for the 6,000-character limit shared by every live post: the other live posts in a dimmed Broadcast pink and this post in full pink (Harkirat, once the shared total was clear), then "3,180 of 6,000 left". The v4 repeat tokens missed; the board now draws one small Discord card per showing with a "1 a day max" tag, and in a short window the cards past the end day turn orange after a stop line with "5 fit by Sep 20" — pending his review.
