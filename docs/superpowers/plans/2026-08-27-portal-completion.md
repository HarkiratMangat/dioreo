...
>
> ✅ **UPDATED 2026-08-27 15:2x EDT.** Also closed since: **F** (glyph — published as an Artifact and **rejected**: *"none for now, might revisit this in the future some day"*; the prototype stays) · **J** (account panel — 🔴 **it was ALREADY BUILT**, all seven of COMPANION §16.8's points are in `portal/ui/shell.js`) · **Q** (analytics payload — **measured at 42.1 KB**, and the fix had landed 2h20m *after* the 495 KB reading, so the row was stale rather than open). **O** (first boot) is **partly** done: boot, Mongo and the 401 gate are exercised; everything behind a signed-in session needs an OAuth sign-in, which is Harkirat's step. **Still open: P** (production) and the mobile pair **L/M**, deprioritised.
>
> 🔴 **And three defects that were on no list at all, found by opening the page:** two typefaces declared in `app.css` and never loaded (`--display`, `--title`), so every large figure rendered in the body face; an export strip with no right gutter because three grid properties were inert in a `display:block` parent; and Season's masthead stating one number three times.
