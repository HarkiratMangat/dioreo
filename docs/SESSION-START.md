...
>
> ℹ️ **THIS FILE DOES NOT POINT AT A SPECIFIC PLAN, ON PURPOSE — corrected 2026-09-07 11:53 EDT, at Harkirat's direction.** It used to carry a "FIRST ACTION — start at <plan/prompt file>" banner, rewritten every time work moved on, with each superseded version kept below as "history." That accumulated into six stacked, quickly-stale generations (2026-08-31 through 2026-09-07) in the one file guaranteed to reach every session. **Session-specific "what to work on next" state belongs in `.remember/remember.md`'s auto-injected LAST HANDOFF block, not here** — that carrier already exists, is already auto-loaded separately every session, and is *meant* to be rewritten each time, unlike this file. This file is the STANDING compliance layer: rules that stay true across plans, not pointers to which plan is current.
>
...
>
> 🔇 **THE MODE IS SILENT MODE, and it lives in `.claude/rules/silent-mode.md`** — no `paths:` frontmatter, so it loads unconditionally every session. It carries the working style AND the vocabulary (call · turn · run · prose-only turn · batch · mega-batch · checkpoint), each definition paired with the mistake it prevents. ⚠️ **No hook enforces it** — the guards are parked on `chore/silent-mode-guards-parked` until the 83-step workflow-compliance plan owns them, so that file is the whole mechanism. ⏰ **Update this line once `chore/silent-mode-guards-parked` merges, is retired, or the 83-step plan actually owns the guards** — flagged 2026-09-07 12:23 EDT, same reasoning as the worktree section above: it names a specific branch that will stop existing. ⚠️ **CORRECTED 2026-09-04 14:16 EDT: this pointed at a `MEMORY.md` section moved out on 2026-09-02, and named `claude/silent-mode-compliance`, a branch this same file records as deleted.** Two dead pointers in one sentence, in the auto-loaded file. Found by the §L ⑥ document agent.
>
