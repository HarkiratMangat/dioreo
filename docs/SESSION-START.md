...
> 🔴 **FIRST ACTION, updated 2026-09-07 00:52 EDT — START AT `docs/superpowers/plans/OWED-PROMPT.md`.** It carries the opener, the do-not list AND a compliance half (§4) built from the ten corrections Harkirat made on 2026-09-06. Four items are owed and they are really three — the harness delay knob and the three states registries are ONE piece of work. **Triage before you fix: five of nine filed items that day were already done and two more were refuted by a single measurement.** Narrative half: `local/handoff/2026-09-07-owed-items-handoff.md`. The previous FIRST ACTION is kept below as history.
>
> ⚠️ **STEP3-PROMPT.md is SUPERSEDED** — its work is finished. Do not work from it.

> 🔴 **(HISTORY) FIRST ACTION, 2026-09-06 20:46 EDT — START AT `docs/superpowers/plans/STEP3-PROMPT.md`.** Its §1 block is an OPENER, not a summary, and its §3 is the do-not list. It names the plan: `docs/superpowers/plans/2026-09-06-portal-step3-step4-completion.md`, which is TRACKED and is the plan for finishing the portal. `local/handoff/2026-09-06-portal-step3-exhaustive-handoff.md` is the narrative half and is gitignored. **The unit of work is the FILED ITEM, never the realm** — three plans were retired on 2026-09-06 and a per-realm loop is the first of them. **Before any pop-up asking Harkirat to decide, `ctx_search` `docs/reference/portal-decision-ledger.md`**: four settled items were surfaced to him as live work in one day. The previous FIRST ACTION below is kept as history.
...
> ## 🔴 ACTIVE BRANCH — `docs/build-out-handoff` (PR #186, OPEN, do not merge) · updated 2026-09-06 15:40 EDT
>
> ⚠️ **It read `feat/home-portal-conformance` until 2026-09-06 15:40 EDT, and that branch is merged.** This file warns twelve lines down that a stale branch name in the one auto-loaded carrier is the worst place for one, and it went stale again anyway. **Everything from 2026-09-06 after the build-out merged is on `docs/build-out-handoff`.**
>
...
>
> 🔴 **FIRST ACTION AS OF 2026-09-06 15:40 EDT — THE PORTAL'S WRITE PATH IS PROVEN AND THE PINS PROCEDURE BELOW IS HISTORY.** Harkirat signed in to `dev-portal.dioreo.app` and drove one change end to end: a draw staged at 15:21:38 EDT, committed at 15:22:08, landed in `seasonaldatas` (newDraws 4 → 5), then reversed (ChangeLog #26 add, #27 delete). **That closed four filed items at once** — `/api/review` had "executed in no environment, ever", a changeset had never received a `baseline` array (this one did: `[{count:4}]`), the dev redirect URI's hyphen-vs-dot question was unanswerable from a terminal, and the P1 first-real-boot item. **The first action now is step 3 of the four-step plan in `local/handoff/2026-09-06-refinement-plan-handoff.md`: the portal's design work, from a list that is finally true.** ⚠️ **69 items, not 107** — a two-agent supersession audit archived 38 that were already done or decided.
>
