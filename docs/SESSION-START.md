...
> 🔵 **THE MEMORY INDEX FITS AGAIN — this block used to tell you to Read the whole file and that instruction is now WRONG.** As of 2026-09-02 16:25 EDT `MEMORY.md` is **22,230 B over 151 lines**, inside both of the harness's limits (first 200 lines OR first 25,000 B, whichever comes first), so the whole index arrives on its own and there is nothing to go and fetch. The `@`-import that used to route around the truncation is **removed**, on its own stated condition — do not restore it, and do not read its absence as the old breakage. ⚠️ **The sentinel is still the test, and it is now the only one you need:** `MEMORY.md`'s last line is `<!-- MEMORY-INDEX-END -->`. See it in your context and the index is complete. Cannot see it and the file has outgrown a limit again — `Read` it and SAY SO, because nothing else will notice. 🔴 **What changed underneath, so you do not re-derive it:** SILENT MODE was an INSTRUCTION living in auto-memory and moved to `.claude/rules/silent-mode.md`, and 79 over-long index-line tails moved into the topic files they already point at. Nothing was deleted.
