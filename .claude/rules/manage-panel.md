...
*Loads when you touch `commands/manage.js`. The flat-command Components-V2 admin panel: page/action
mechanics, per-page accent colors, and the `/manage` admin-only lock. The per-user locks on
`/settings` + View Colors (author-lock, passive idle auto-disable, admin override) are in
`.claude/rules/settings-and-expiry.md`. All routing/DB-mutating submit logic lives in `index.js` — see
`.claude/rules/interaction-router.md`. Loadout/image specifics: `.claude/rules/loadouts.md` +
...
- `/manage` (admin-only, hidden via `setDefaultMemberPermissions(0)`) — the single admin data-entry
  ...
  - **`formatguide` (Draws/Calendar/Loadouts, added 2026-07-31 17:20 EDT, notes L189)** — NOT a
    modal, just an ephemeral text reply of that page's `BULK_FORMAT_GUIDES` entry (real syntax
    pulled from `utils/adminParser.js`'s own parsers, not hand-guessed). Handled directly in the
    `mng_act_` router, before the group-specific branches, since it needs no DB access. `loadouts_mp`/
    `loadouts_dmz` share one guide text (aliased in `manage.js`).
  ...
  - **Patch Notes operates on a single "current entry"** (`dateinfo`/`urls1`/`urls2`, replacing the
    ...
    entry in `.claude/rules/design-decisions.md`). `dateinfo` covers release date + additional info;
...
any of those `PRESET_ACCENT`s are ever re-picked, see the color map in `.claude/rules/design-decisions.md`).
Season has no page in `PAGES` at all (its two actions open their modal directly, see the Command
architecture in `.claude/rules/commands-overview.md`), so it needs no entry.
...
The gap: `/manage` (before this) checked `ALLOWED_ADMIN_ID` only once at invocation; none of the ~25 button/select/modal-submit handlers re-checked the clicker, so anyone who could see the message could press its buttons and mutate bot data. The centralized guard choke-point itself lives in `index.js`'s `interactionCreate` (see
`.claude/rules/interaction-router.md`); this is the `/manage` half of the panel-locks work.
