...
⚠️ **SEVEN MENTIONS OF `index.js` SURVIVE ON PURPOSE — they are correct, do not "finish the job" on them.** The `process.on('unhandledRejection')` and `uncaughtException` nets really are still in `index.js` (referenced from `bot/lifecycle.js` and `utils/logger.js`); `utils/interactionContext.js` and `scripts/migrateBuildsToMongo.js` describe where code *used to* live, which is history, not a stale pointer.
