...
#
# 🔴 THE SILENT HALF IS THE POINT. An advisory that fires on the wrong command gets filtered, and a filtered
# advisory protects nothing — rg-flag-guard next door paid for that lesson five times. So every case below
# comes in a pair: a command that must produce the nudge, and the nearest command that must not.
#
# ⚠️ Run the way the HOOK runs — a non-interactive shell. This machine's interactive aliases (`find`->`bfs`,
# `git`->`rtk`) make probes succeed where the real hook fails, and that is how a BSD-find bug nearly escaped
# a second time (see reference_enforcement_hooks).
set -u
...
# ── the heredoc carve-out: PROSE ABOUT a search is not a search ──────────────
# Written as a single string so this file's own body cannot be mistaken for the case it describes.
HD='python3 - <<PYEOF
...
# ── §0.5a R7: the guard must not swallow the failure it was written for ──────
# A hook whose matcher never matches passes every run and certifies nothing. The FIRES block above is that
# proof; this asserts the harness wiring too — malformed input must not crash it into a false silence.
