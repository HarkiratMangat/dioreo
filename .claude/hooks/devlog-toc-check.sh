...
#
# WHY (2026-07-28 17:35 EDT): `docs/DEVLOG.md`'s Part A table of contents is maintained by hand, with a "**Keep in sync** when you add an entry" note. It was found **15 entries behind the body** on 2026-07-28 16:20 EDT — every 2026-07-27 and 2026-07-28 entry missing — the same asymmetry already measured for the DEVLOG itself (machine-checked 22/22, attention-dependent 8/22). A note asking politely is not a mechanism.
#
# Harkirat caught the second half of the problem the same day: the TOC used vague qualifiers ("later", "afternoon", "late afternoon") instead of the timestamps the body headings carry. That made it ambiguous, made it ROT (insert one entry and every "later" below it silently shifts meaning), and defeated the TOC's own stated purpose — its header says to jump by SEARCHING the entry text, which cannot work when the TOC text doesn't match the heading text.
#
# The rule is exact mirroring: **every DATED Part A TOC line is its body heading with `## ` stripped.**
#
# ⚠️ CHANGED 2026-07-28 20:45 EDT — the comparison logic MOVED to `scripts/docs-audit.mjs` (check id `devlog-toc`) and this file now just calls it. Two reasons, both learned the hard way:
#   1. It was the only copy. A hook runs solely inside a Claude session on this Mac, so a PR opened any other way was unchecked. The audit also runs in CI, on every PR, forever.
#   2. Its exemption for the intentional non-dated TOC pointer ("Earlier milestones", which has no body heading and was very nearly deleted by a naive regeneration) now lives in ONE place with a self-test proving it, instead of being re-derived in each copy.
...
# ⚠️ FAILSAFE (2026-07-29 02:00 EDT). This gate DELEGATES to scripts/docs-audit.mjs so there is one implementation of the TOC rule -- but delegation replaced a working standalone check with a single point of failure. If the audit is deleted, renamed, or throws, a bare \`exit 0\` here would silently retire the gate, which is the exact class of failure this whole enforcement layer exists to stop. So an audit that cannot RUN is itself reported. Deliberately NOT a duplicated copy of the comparison logic: two copies drift, and the drift is silent too.
