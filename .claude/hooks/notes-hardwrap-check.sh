...
# rules that get violated three times are not rules, they are wishes — the standing strategy in
# memory `reference_enforcement_hooks` is that a checkable rule becomes a machine gate. This is that
# gate. It exists because I could not be trusted to remember.
#
# WHAT IT CHECKS. Only edits to the notes file, and only the text being written. An HTML comment
# whose `<!--` and `-->` sit on different physical lines is reported. That is the exact shape of the
# violation: one logical paragraph split across several source lines.
#
# ITS FAILURE MODE, STATED (per the rule that every guard must declare one). It is advisory and never
# blocks — a false positive must not be able to stop work. It inspects only the incoming string, not
# the file on disk, so it cannot see a violation that was already committed; the audit script covers
# the tree, this covers the keystroke. A genuinely multi-line comment containing a real bulleted list
# IS legitimate (Harkirat's own carve-out), so bullet-only line breaks are not reported, and anything
# it does report still needs a human judgement rather than a reflex reformat.
...
# Find comments that open on one line and close on another. Continuation lines that are pure list
# items (leading "-" or "*" or "(N)" bullets) are Harkirat's explicit exception and don't count.
