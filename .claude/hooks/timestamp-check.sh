...
if [ -n "$placeholder" ]; then
  ...
  if [ "$mode" = "pre" ]; then
    # 🔬 AUTOFIX PROBE, added 2026-09-01 19:24 EDT. Harkirat: "how exactly are you going to make it
    # durable when the hooks already exist?" — and he is right that a fourth prose restatement is not
    # a mechanism. Denying is correct but costs a whole round trip every time, and this branch has
    # been firing for weeks. The only remedy that removes the cost is SUBSTITUTION: hand the harness
    # a corrected tool_input instead of a refusal. Whether PreToolUse honours `updatedInput` in this
    # build is NOT documented anywhere reachable (the CLI ships as a binary) and no hook in this repo
    # has ever tried it, so this emits BOTH the substitution and the deny fallback and lets the
    # observed behaviour settle it. If the write lands corrected, updatedInput is honoured and the
    # deny can go. If it is still denied, it is not, and the honest conclusion is that no mechanism
    # exists at this layer.
    # 🔴 THIS BRANCH USED TO DENY, AND DENYING IS WHY IT KEPT COSTING A WHOLE ROUND TRIP. Harkirat, 2026-09-01 19:10 EDT: "the timestamp injection has been a hook into EVERY tool call you make for WEEKS now... and yet nearly every time you edit a file and input a timestamp, it triggers the denial... LIKE ARE WE DEADASS???" He was right that a fourth prose restatement of the rule is not a mechanism — three surfaces already carried it and the placeholder still got typed. A refusal detects the defect and then hands the work back; a SUBSTITUTION detects it and finishes the job, because the hook already knows the one value the write was missing.
    # ⚠️ `updatedInput` IS HONOURED BY THIS BUILD — measured 2026-09-01 19:16 EDT, not assumed. The CLI ships as a binary so the capability is not greppable; a scratch write carrying `19:xx` landed on disk reading `19:16`. If a future build stops honouring it the fallback below still denies, so the failure mode is the old behaviour rather than a placeholder reaching disk.
    # ⚠️ THE FALLBACK EXISTS FOR A CAPABILITY REGRESSION, AND NO CONTENT CAN SIMULATE ONE. Every placeholder the detector finds is by construction one the substitution can repair — the two use the same pattern — so there is no input that reaches the deny path, and a branch no test can enter is a branch that rots. `TS_NO_AUTOFIX=1` is a test-only seam that stands in for the day `updatedInput` stops being honoured; it is what makes the deny path a checked claim rather than a hope. Naming it here because a seam whose purpose is undocumented gets deleted as dead code.
    ...
    now_hm=$(date '+%H:%M')
    fixed=$(printf '%s' "$payload" | jq --arg t "$now_hm" '
      .tool_input |= with_entries(
        if (.value | type) == "string"
        then .value |= gsub("(?<d>[0-9]{4}-[0-9]{2}-[0-9]{2}[ \t]+)[0-9xX?]{1,2}:[0-9xX?]{2}"; .d + $t)
        else . end)
      | .tool_input' 2>/dev/null)
    # The fallback is not decoration: if jq fails, or the substitution changes nothing, an `allow` would pass the placeholder straight through — a fix that silently does not fix is worse than the refusal it replaced.
    # ⚠️ THE FIRST VERSION OF THIS GUARD REJECTED ITS OWN CORRECT OUTPUT. It re-matched the placeholder pattern against the fixed text to prove the slot was gone — but `[0-9xX?]{2}` is an alternation, so a repaired `19:20` matches it just as happily as `18:xx` does, and every substitution was discarded as unfixed. The check has to ask whether a placeholder CHARACTER survives, which is the same two-step the detector above already uses: find the date+slot shapes, then keep only the ones carrying x/X/?.
