...
#
# ==========================================================================
# ⚠️ REBUILT 2026-08-20 12:16 EDT -- Harkirat: "nearly every session has skipped reading the model recommendation file or given pre-emptive recommendations since they rely purely on what's inside of the hook." Two separate defects, and the second one was this hook's fault:
#
...
#
# THREE MODES, chosen by reading the transcript, because a nudge that never stops is a nudge that gets tuned out -- and this one was re-injecting ~1.4KB of table on EVERY prompt for a decision made ONCE:
...
# MEASURED 2026-08-20 12:24 EDT, not estimated -- the first draft of this comment asserted "~14.5KB becomes ~5.8KB" and both numbers were invented. Real figures, from running the old and new hooks: old 2,284 B on EVERY prompt · new 3,666 B once, then 1,267 B. Across a session: 5 prompts 11,420 -> 8,734 (23%), 10 prompts 22,840 -> 15,069 (34%), 20 prompts 45,680 -> 27,739 (39%). It carries MORE content than before and still costs less from the second prompt on. ⚠️ Re-measure rather than trust these if you edit the blocks; they are a dated measurement, not a maintained invariant. Fail-open in every ambiguous case (no transcript, unreadable, no jq) -- showing the grid unnecessarily costs tokens; hiding it costs a wrong model for a whole session.
# ==========================================================================
#
...
# ---------------------------------------------------------------------------
# Read the harness payload. `[ -t 0 ]` so a manual `bash self-check.sh` from a terminal does not hang forever on a `cat` with no stdin -- the hook must stay runnable by hand for debugging.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# THE TABLE, as data -- the ONLY copy. The printed grid is rendered from it below, so the validator and the thing it validates against cannot drift apart. A validator carrying its own second copy of the cells is how a gate starts confidently passing wrong answers.
# ---------------------------------------------------------------------------
...
# The PRINTED table is rendered from cell_for() rather than written out a second time. Two copies of twelve cells is how a validator starts quietly disagreeing with the grid it validates against -- and this hook exists because a remembered copy of this exact table was wrong twice in one session.
tbl_row() {
...
# The derivation pattern, named once so the hook and self-check.test.sh cannot pin different shapes. Deliberately loose on the SEPARATOR (·, x, ×, |, comma, "and") -- pinning one punctuation mark would fail a CORRECT answer that used a different bullet, and the axis words plus the cell are what carry the meaning. The transcript is JSONL and an assistant message is one physical line, so a line-oriented grep reaches inside it without parsing the JSON.
...
# The /rename string, e.g. "Sonnet5-H · Hotpatch build · Aug 20". Added 2026-08-20 13:35 EDT -- the FIRST_ACTION text has always asked for this ALONGSIDE the model derivation, but until now nothing detected it, so stating the derivation alone silently satisfied the WHOLE gate (both halves) and the rename string went unrequested. ⚠️ UNLIKE DERIV_RE this is deliberately STRICT on the separator (a literal "·", not the loose punctuation class DERIV_RE allows) -- the derivation is a typed sentence that can vary in phrasing, but the rename string is a fixed, copy-pasted format. A loose separator here would match ordinary prose that happens to mention a model name near a date (verified live: "the release is scheduled for Aug 20" false-matched under a loose version before this was tightened).
RENAME_RE='(Sonnet|Opus|Fable|Haiku)[0-9.]*-[A-Za-z]{1,4}[ ]*·[ ]*[^·]{2,60}·[ ]*(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) [0-9]{1,2}'

# ---------------------------------------------------------------------------
# Find the most recent derivation in the transcript.
# ---------------------------------------------------------------------------
# A SessionStart hook with matcher `compact|fork` drops this marker; consume it once and force a fresh derivation. FORK is why it exists: a fork COPIES the transcript, so the previous session's derivation is still greppable and would wrongly suppress the re-recommendation on the one case Harkirat named as genuinely wanting a new model pick ("only time i'd switch model mid session is if i fork or the actual session drastically pivots", 2026-08-20 12:16 EDT). `compact` is belt-and- braces alongside the in-transcript boundary check below -- a session RESUMED from a compact and an in-session /compact are different code paths and only the marker covers the first. Consume-once, same shape as bot/lifecycle.js's .restart-reason. Fully swallowed: a marker problem must never be able to break the prompt.
...
PREMISE="" ; DELIB="" ; STATED="" ; STALE=0 ; RENAME_GIVEN=0
...
# derivation back verbatim, so its own output MATCHES ITS OWN DETECTOR -- the hook's echo lands in the transcript and becomes evidence for the next firing, which is the classic "a hook whose block message quotes its own detector suppresses itself forever". Found by the completeness sweep 2026-08-20 12:58 EDT, one commit after the escape token was introduced for the neighbouring case. ⚠️ MG-EXAMPLE -- the per-line escape, added 2026-08-20 12:54 EDT after this gate fired a FALSE
    ...
    if [ -n "$RCAND" ]; then
        RENAME_LINE="${RCAND%%:*}"
        RENAME_GIVEN=1
        if [ -n "$COMPACT_LINE" ] && [ "$COMPACT_LINE" -gt "$RENAME_LINE" ] 2>/dev/null; then RENAME_GIVEN=0; fi
    fi
fi
...
EOF
# Bash 3.2-safe placeholder substitution -- the heredoc stays single-quoted so nothing inside it is subject to expansion, and only this one marker is replaced.
...
# ---------------------------------------------------------------------------
# Mode selection. Fail-open to the full grid: showing it needlessly costs tokens, hiding it costs a wrong model for an entire session.
# ---------------------------------------------------------------------------
# MODEL_OK drives the mismatch-correction check ONLY, and stays independent of the rename string -- a wrong pick must be flagged whether or not the rename has been given yet. CURRENT drives the "already given, stay quiet" short-circuit and REQUIRES BOTH: the FIRST_ACTION text has always asked for the rename string alongside the model recommendation, but until 2026-08-20 nothing checked for the rename half, so stating the derivation alone silently satisfied the whole gate. See RENAME_RE's own comment above for why that went unnoticed for as long as it did.
MODEL_OK=0
if [ -n "$STATED" ] && [ -n "$EXPECTED" ] && [ "$STALE" -eq 0 ] && [ "$FORCED" -eq 0 ]; then MODEL_OK=1; fi
CURRENT=0
if [ "$MODEL_OK" -eq 1 ] && [ "$RENAME_GIVEN" -eq 1 ]; then CURRENT=1; fi
SELFCHECK="$FIRST_ACTION"
[ "$CURRENT" -eq 1 ] && SELFCHECK="$ALREADY_DONE"

if [ "$MODEL_OK" -eq 1 ] && [ "$STATED" != "$EXPECTED" ]; then
...
"$GRID")"
    ...
    # A compact landed after the derivation. The context that held it is gone, so re-derive rather than carry a number nobody in the current window can justify.
