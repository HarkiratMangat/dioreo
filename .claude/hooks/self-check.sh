...
#
# Was an inline `echo` in .claude/settings.json until 2026-08-11 21:07 EDT. Promoted to a script for
# one reason: it now has to CARRY the model-selection grid, and a lookup table does not survive being
# JSON-escaped onto one line of a settings file.
#
# ⚠️ WHY THE GRID IS IN HERE RATHER THAN POINTED AT. Measured the hard way this session. Every layer
# already said "use the grid": docs/SESSION-START.md (auto-loaded, names the file), the MEMORY.md
# index line (auto-loaded, names the axes), and this hook (fires every turn, names the requirement).
# All three were in context, and the recommendation was STILL made from a remembered shape of the
# table -- twice, wrongly, in opposite directions. Harkirat: *"so you just ignored our new model
# recommendation system?"*
#
# The failure mode is precise and worth stating: a POINTER preserves the concept and drops the
# DISCRIMINATING VALUES. "premise-risk x deliberation-load grid" is enough to feel informed and not
# enough to be correct -- it carries neither axis DEFINITION nor the retired-cell rule, and those two
# facts are exactly what both wrong picks turned on. A fourth layer of prose saying "really do open
# it" would have failed the same way. So the table is present at the moment of the decision instead.
#
# ⚠️ THIS IS A SECOND COPY of content owned by the `reference_priority_tier_system` memory, which is
# duplicated state and can drift (see `feedback_no_duplicated_state_in_prose`). Accepted knowingly:
# the alternative is a pointer, and a pointer is the thing that measurably failed. The memory is named
# below as canonical so a reader knows which side wins, and `self-check.test.sh` pins the values that
# actually discriminate, so silent drift fails the suite rather than a session.
#
# ⚠️ `hookEventName` is REQUIRED. A hook emitting hookSpecificOutput without it is SILENTLY DISCARDED
# -- runs, exits 0, prints valid JSON, reaches nobody. Two hooks were dead this way for weeks.
set -euo pipefail
