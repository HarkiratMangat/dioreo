...
#
# ⚠️ (b) is the case that already failed once for real. The first live run reported 29 of 60 lines missing from a file whose bytes were IDENTICAL, because rg parsed a markdown bullet's leading `-` as a flag. A conservation checker that invents data loss is worse than none — it would train everyone to dismiss the one run that matters. That regression is pinned by the RENAME case below, whose fixture content deliberately starts with `- `.
#
...
# ---- (b) it does NOT invent data loss ---- A rename preserves every byte, so a correct checker is silent. This is the rg-leading-dash regression: without `-e`, every "- ..." line reads as missing and this fires on a clean move.
...
# ---- (c) cost controls ---- The stamp: a second identical run must short-circuit. Proving it means the FIRST run reported and the SECOND is silent on unchanged state -- the property that keeps this off every message.
...
# The claim gate: in stop mode with no completion claim in the transcript, silence regardless of findings. Without this the hook would fire on every assistant message and be disabled within a day.
...
# ---- pass 3: the angle set difference ---- That transcript contains no `config/dior`, so that angle must be reported as un-taken.
a "un-taken angle is reported"        "ANGLES NOT TAKEN" yes "$out_claim"
a "names the external-trees angle"    "external-trees"   yes "$out_claim"

# ...and an angle whose signature IS present must NOT be reported. This is the half that catches a detector matching nothing -- a registry where every angle always reports is a registry nobody reads.
CLAIM3=$(mkrepo d7 delete)
tr_ang="$TMP/t-angles.jsonl"
# ⚠️ Must be a real `"name":"Bash"` tool_use. This fixture used a plain `"type":"user"` line until 2026-08-06 09:33 EDT, and it passed only because detection was an unscoped whole-transcript grep — the very bug that let the gate suppress every angle after one fire. Once scoping landed, this case correctly began reporting the angle as un-taken and the assertion failed. The test was asserting the defect, which is exactly what the docs-only exemption did in the sibling sweep's suite.
...
# ---- 🔴 SELF-POISONING: the failure that made this gate switch itself off ---- An angle's detector string appearing in PROSE — the hook's own block message, or this file's own source when someone Writes/Reads it — must NOT count as having run the angle. Measured against the live transcript 2026-08-06 09:31 EDT: a whole-transcript grep marked ALL FIVE angles covered on a session where most had never run, i.e. the gate suppressed itself permanently after one fire. Detection is scoped to `"name":"Bash"` lines; these two cases pin both directions of that.
POISON=$(mkrepo d8 delete)
tr_poison="$TMP/t-poison.jsonl"
# The detector string appears ONLY in assistant prose (exactly how the hook's own output arrives). ⚠️ The filler is a SEPARATE JSONL LINE, not another block in the same message. JSONL puts a whole assistant message on ONE line, so a Bash call sharing a message with the prose makes that line match `"name":"Bash"` and drags the prose through with it — which broke this test's own premise on the first run. Scoping to Bash lines excludes messages containing no tool call at all; it cannot separate prose from a command inside the SAME message. That limit is stated in the hook.
...
# ---- two bases: a v3 branch must not be diffed against main ---- The Stop wiring passes a literal `main` because it fires long before any --base flag is typed. A branch cut from v3-pre-release diffed against main reports the entire v3 delta as "changed".
V3="$TMP/v3"
mkdir -p "$V3/docs"; git -C "$V3" init --quiet -b main
# ⚠️ Must clear the hook's 45-char sampling floor. A 4-char "base" line meant ZERO lines were sampled, so the conservation check correctly found nothing and this assertion failed against a fixture that could never have triggered it — a test that cannot fail for the right reason.
...
out_v3=$(run "$V3" "" pr)
# docs/v3only.md belongs to v3-pre-release, not to this branch. With the base wrongly resolved to main it would appear in the diff; with it correctly resolved to v3-pre-release it must not.
...
# ---- 🔴 UNCOMMITTED WORK: the highest-likelihood miss in the whole design ---- `$BASE...HEAD` sees committed history only, so a session that edits files and claims "done" WITHOUT COMMITTING produced an empty diff and total silence. That is the default state mid-session and precisely when a premature "done" is most likely. Found 2026-08-06 09:44 EDT by asking what the gate's PRECONDITIONS are, an angle neither earlier pass took.
UNC="$TMP/uncommitted"
...
# ---- the stamp must be SESSION-scoped, not repo-state-scoped ---- It lives in .git/ and persists across sessions. Without the session in the fingerprint, a NEW session inheriting an unchanged repo got silence on its first claim — exactly when pass 3 is worth most, because a fresh session has taken zero angles.
...
# ══ PASSES 4 & 5 — change-shaped demands + the derivation requirement ══════════════════════════════ These exist because pass 3's registry asks the SAME question every session. Each proof below pins a demand to the property that triggers it, and — just as importantly — pins that it stays QUIET when that property is absent. A demand that fires on everything is the failure mode four hooks here warn about, so the negative cases are the load-bearing half.
...
# ---- derivation: demanded on a wide change when no derivation ran ---- shellcheck disable=SC2086
out=$(run "$(mkfiles wide $many)" "$TR" pr)
a "wide change demands DERIVE THE ANGLES" "DERIVE THE ANGLES" yes "$out"

# ---- ...and is SATISFIED when the transcript carries a real sequential-thinking tool_use ---- shellcheck disable=SC2086
