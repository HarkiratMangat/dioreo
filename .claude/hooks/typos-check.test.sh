...
#
# The important one is the LAST case. This hook opens with `command -v typos || exit 0`, so if the binary is missing it goes permanently silent and looks exactly like "no typos found" — a vacuous pass, which is the failure mode feedback_not_checkable_is_usually_unexamined is about. The suite therefore FAILS LOUDLY when `typos` is absent rather than skipping, because a green suite that proves nothing is worse than a red one.

HOOK="$(cd "$(dirname "$0")" && pwd)/typos-check.sh"
pass=0; fail=0

# ⚠️ CI IS EXEMPT, THE DEV MACHINE IS NOT — split 2026-08-02 18:04 EDT, after CI failed here. The distinction is real, not a convenience: this hook only ever runs inside a Claude Code session on Harkirat's Mac, so `typos` missing THERE means the gate is silently dead and must fail loudly. On the CI runner the hook never executes at all, so demanding the binary would be enforcing a dependency nothing uses — the fastest way to get a suite disabled is to make it fail for a reason that does not matter. CI still runs every other case; only the binary requirement is relaxed.
...
# Project vocabulary must not fire — that is what _typos.toml is for, and if this starts failing it means the config stopped being picked up (the hook writes its probe INSIDE the repo precisely because typos resolves config relative to the file it checks).
a "project vocabulary is silent"  silent "The loadout accent color for the DMZ changelog devlog."

# The probe file must never be left behind: it is written into the repo root, so a leak would show up as an untracked file and could end up committed.
