...
printf 'CODEBASE-MEMORY NUDGE — this is a text search over the CODE corpus that codebase-memory-mcp already has indexed as a graph.\n\nFor "where is X defined", "what calls X", "what does X call", or "what breaks if I change X", the graph answers what rg structurally cannot: rg finds the STRING, the graph returns the CALLERS.\n\n  search_graph({ project: "Applications-Claude-Code-Diors-Builds", query: "X" })   # the definition; a bare name_pattern also returns every import site\n  trace_path({ function_name: "X", direction: "both", depth: 3 })\n  get_code_snippet({ qualified_name: "..." })            # exact source, precise ranges\n\nMEASURED 2026-09-14 22:53 EDT on 0.10.8, over a seeded random sample of 25 functions: search_graph with query put the real definition first for 24 of 25 in about 250 bytes, against up to 8 KB for rg -w. ⚠️ For callers, pass include_tests:true to trace_path and also run search_code for \"X(\": a call that passes a slash- or URL-shaped string is filed as an HTTP route, and a function passed as a callback or called through a module object gets no edge (mentionCommand: 1 of 10 files linked), and search_code finds all of them. The dead-code filter gives candidates only (utils/: 115 candidates, 7 truly unused). semantic_query takes ONE keyword per call. Re-measured 2026-09-14 23:08 EDT.\n\nrg stays correct for a filename, a config value, a non-repo path, or a literal in a file you already have open. This is advisory and blocks nothing.' \
