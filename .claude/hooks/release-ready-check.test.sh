...
r(){ local raw; raw="$(RELEASE_CHECK_FILES="$1" printf '{"tool_input":{"command":"gh pr merge 1 --squash"}}' | RELEASE_CHECK_FILES="$1" bash "$HOOK")"
     [ -z "$raw" ] && { echo SILENT; return; }; printf '%s' "$raw" | jq -r '.hookSpecificOutput.permissionDecisionReason // "SILENT"'; }
a(){ local n="$1" needle="$2" want="$3" files="$4" out; out="$(r "$files")"
